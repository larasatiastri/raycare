...
class Input_hasil_lab_manual extends MY_Controller {

    // protected $menu_parent_id = 2;           // deprecated, pake $menu_tree
    protected $menu_id = '9df9dd3baaac9726263e7378595bfc63';                  // untuk check bit_access
    ...
    {
        $assets = array();
        $config = 'assets/tindakan/input_hasil_lab_manual/index';
        ...
        $data = array(
            ...
            'js_files'       => $assets['js'],
            'content_view'   => 'tindakan/input_hasil_lab_manual/index',
            );
    ...
    {
        $assets = array();
        $config = 'assets/tindakan/input_hasil_lab_manual/view';
        $this->config->load($config, true);
        $assets = $this->config->item('assets', $config);
        ...
        $data = array(
            ...
            'content_view'              => 'tindakan/input_hasil_lab_manual/view',
        );
    ...
    {
        $assets = array();
        $assets_config = 'assets/tindakan/input_hasil_lab_manual/add';
        ...
        $data = array(
            ...
            'css_files'      => $assets['css'],
            'js_files'       => $assets['js'],
            'content_view'   => 'tindakan/input_hasil_lab_manual/add',
            'flag'           => 'add',
        );
    ...
    {
        $result = $this->hasil_lab_m->get_datatable(1);
        $output = array(
        ...
        foreach($records->result_array() as $row)
        {
            ...
            $action = '<a title="'.translate('View', $this->session->userdata('language')).'" href="'.base_url().'tindakan/input_hasil_lab_manual/view/'.$row['id'].'" class="btn default"><i class="fa fa-search"></i></a>';
        ...
        if($array_input['command'] == 'add'){
            ...
            foreach ($array_input['item'] as $key => $item) {
                ...
                $data_detail = array(
                    ...
                    'hasil'                     => $item['hasil'],
                    'nilai_normal'              => $item['nilai_normal_manual'],
                    'satuan'                    => $item['satuan'],
            ...
            foreach ($array_input['hasil_lab'] as $key => $hasil_lab) {
                if($hasil_lab['url'] != ''){
                    ...
                    $path_dokumen = '../cloud/'.config_item('site_dir').'pages/tindakan/input_hasil_lab_manual/images/'.str_replace(' ', '_', $id_hasil);
                    ...
                    //die_dump(config_item('base_dir').config_item('user_img_temp_dir_new').$temp_filename);

                    copy(config_item('base_dir').config_item('user_img_temp_dir_new').$temp_filename, $_SERVER['DOCUMENT_ROOT'] .config_item('site_img_hasil_lab_manual').$real_file);
                }
            }
        }
        redirect('tindakan/input_hasil_lab_manual');
