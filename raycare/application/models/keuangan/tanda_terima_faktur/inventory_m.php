...
class Inventory_m extends MY_Model {
	...
	public function get_datatable_item($gudang_dari, $gudang_ke)
	{	
		$datatable_item = array(
			...
			'(SELECT SUM(inventory.jumlah)FROM inventory WHERE inventory.gudang_id = "'.$gudang_dari.'" AND inventory.item_id = item.id
				AND inventory.item_satuan_id = item_satuan.id)' => 'stock',
			'(SELECT SUM(inventory.jumlah)FROM inventory WHERE inventory.gudang_id = "'.$gudang_ke.'" AND inventory.item_id = item.id
				AND inventory.item_satuan_id = item_satuan.id)' => 'stock_tersedia',
		);
