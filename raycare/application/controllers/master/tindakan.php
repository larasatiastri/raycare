...
class Tindakan extends MY_Controller {
    ...
    protected $menu_id = '21caf90c61f8e02c29af3e02b1601718';                  // untuk check bit_access
    ...
    {
        $assets = array();
        $config = 'assets/master/tindakan/index';
        $this->config->load($config, true);
        $assets = $this->config->item('assets', $config);
        
        // die(dump( $assets['css'] ));
        $data = array(
            'title'          => config_item('site_name').' | '.translate('Master Tindakan', $this->session->userdata('language')), 
            'header'         => translate('Master Tindakan', $this->session->userdata('language')), 
            ...
            'js_files'       => $assets['js'],
            'content_view'   => 'master/tindakan/index',
            'flag'           => 2,
        );
    ...
    {
        $assets = array();
        $assets_config = 'assets/master/tindakan/add';
        ...
        $data = array(
            ...
            'content_view'   => 'master/tindakan/add',
        );
    ...
    public function edit($id)
    {
        ...
        $assets = array();
        $config = 'assets/master/tindakan/edit';
        $this->config->load($config, true);
        ...
        $form_data=$this->tindakan_m->get_by(array('id'=>$id,'is_active'=>1));
      //  $form_data2=$this->poliklinik_tindakan_m->getdata($id)->result_array();
        ...
        $data = array(
            ...
            'css_files'      => $assets['css'],
            'js_files'       => $assets['js'],
            'content_view'   => 'master/tindakan/edit',
            'form_data'      => $form_data,
           // 'form_data2'     => $form_data2,
    ...
    {        
        ...
        foreach($records->result_array() as $row)
        {
            ...
            $action = '<a title="'.translate('Lihat', $this->session->userdata('language')).'"  name="view[]" data-action="view" data-id="'.$row['id'].'" class="btn grey-cascade search-item"><i class="fa fa-search"></i></a>
                        <a title="'.translate('Edit', $this->session->userdata('language')).'" href="'.base_url().'master/tindakan/edit/'.$row['id'].'" class="btn blue-chambray"><i class="fa fa-edit"></i></a>
                        <a title="'.translate('Poliklinik Tindakan', $this->session->userdata('language')).'" href="'.base_url().'master/tindakan/poliklinik_tindakan/'.$row['id'].'" class="btn yellow"><i class="fa fa-list-alt"></i></a>
                        <a title="'.translate('Hapus', $this->session->userdata('language')).'" data-confirm="'.translate('Anda yakin akan menghapus data tindakan ini?', $this->session->userdata('language')).'" name="delete[]" data-action="delete" data-id="'.$row['id'].'" class="btn red"><i class="fa fa-times"></i> </a>';
    ...
    public function save()
    {
        ...
        {  
           // $data = $this->cabang_m->array_from_post( $this->cabang_m->fillable_add());
            ...
            $data['harga'] = $this->input->post('harga1');
            $data['keterangan'] = $this->input->post('keterangan1');
            $data['is_active'] = 1;
            $tindakan_id = $this->tindakan_m->save($data);
            ...
            if ($tindakan_id) 
            {
                $flashdata = array(
                    "type"     => "success",
                    "msg"      => translate("Data tindakan berhasil ditambahkan.", $this->session->userdata("language")),
        ...
        elseif ($command === 'edit')
        {
            ...
            $poliklinik_id = $this->tindakan_m->save($data, $this->input->post('pk'));
    ...
    public function deleteajax()
    {
        ...
        } else
        {
            ...
            $msg = "Tindakan sudah dikembalikan";
        }
       
        // save data
        $user_id = $this->tindakan_m->save($data, $id);
       ...
       if($tindakan_id!='')
        {
            ...
            $data1['cabang_id']=$this->session->userdata('cabang_id');
            $data1['is_active'] = 1;
    ...
    public function add_harga($poli_tindakan_id)
    {
        $data_poli_tindakan = $this->poliklinik_tindakan_m->get_by(array('id' => $poli_tindakan_id), true);
        $data_poli = $this->poliklinik_m->get_by(array('id' => $data_poli_tindakan->poliklinik_id), true);

        $data = array(
            'id'     => $poli_tindakan_id,
            'data_poli_tindakan' => object_to_array($data_poli_tindakan),
            'data_poli' => object_to_array($data_poli)
        );

        $this->load->view('master/tindakan/add_harga', $data);
    }
