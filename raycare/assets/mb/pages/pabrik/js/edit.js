...
(function(o){
    ...
    var initForm = function(){
        ...
        $.each(formsCP, function(idx, form){
            ...
            $('a.del-db-cp', form.section).on('click', function(){
                var id = $(this).data('id');

                handleDeleteDbContactPerson(form, id);
            });
        ...
        $.each(formsAlamat, function(idx, form){
            ...
            $('a.del-db', form.section).on('click', function(){
                ...
                handleDeleteDbAlamat(form, id);
            });

            $('a.edit-subjek', form.section).on('click', function(){
                var id = $(this).data('id');
                //alert(counter);
                handleEditSubjekAlamat(id);
            });
            ...
            $('select.kota', form.section).on('change', function(){
                var id = $(this).data('id');
                //alert(counter);
                handleSelectKecamatan(id);
            });

            $('select.kecamatan', form.section).on('change', function(){
                var id = $(this).data('id');
                //alert(counter);
                handleSelectKelurahan(id);
            });
    ...
    var handleDeleteDbContactPerson = function(form, id){
        //alert(id);
        bootbox.confirm("Are you sure to delete this contact person?", function(result) {
        
            if (result==true) {
                $('div#contact_person_' + id).addClass('hidden');
                $('input#is_delete_cp_' + id).val(1);
            }
        });
    }

    var handleDeleteDbAlamat = function(form, id){
        //alert(id);
        bootbox.confirm("Are you sure to delete this Alamat?", function(result) {
    ...
    var handleEditSubjekcp = function(counter){
        ...
        $btnSaveSubjekcp.on('click', function(e){
