...
class Kasir_titip_uang_m extends MY_Model {

	protected $_table      = 'kasir_titip_uang';
	...
	protected $datatable_columns = array(
		...
		'kasir_titip_uang.rupiah'				=> 'rupiah',
		'kasir_titip_uang.rupiah_terima'		=> 'rupiah_terima',
		'kasir_titip_uang.rupiah_sisa'			=> 'rupiah_sisa',
		'kasir_titip_uang.subjek'				=> 'subjek',
		'kasir_titip_uang.keterangan'			=> 'keterangan',
		'kasir_titip_uang.is_lunas'				=> 'is_lunas',
		'kasir_titip_uang.created_by'			=> 'created_by',
	...
	protected $datatable_columns_pilih_kasir_titip_uang = array(
		//column di table  => alias
		'kasir_titip_uang.id'			=> 'id',
		'kasir_titip_uang.nama'			=> 'nama',
		'kasir_titip_uang.tempat_lahir'	=> 'tempat_lahir',
		'kasir_titip_uang.tanggal_lahir'	=> 'tanggal_lahir',
		'kasir_titip_uang.is_active'		=> 'active',
		'kasir_titip_uang_alamat.alamat'	=> 'alamat'
	);
	...
	public function get_datatable_history_kasir_titip_uang($status, $date)
	{
		...
		$status_kasir_titip_uang = '';

		if($status == 1) 
		{
			$status_kasir_titip_uang = 1;

		} elseif ($status == 2) 
		{

			$status_kasir_titip_uang = 0;
		...
		if($status == 0) 
		{
			...
			$this->db->where('left(tanggal_kasir_titip_uang, 10) <', $date);
			$this->db->where('kasir_titip_uang.is_active', 1);
			$this->db->where('selesai', $status_kasir_titip_uang);
			$this->db->or_where('left(tanggal_kasir_titip_uang, 10) >=', $date);
			$this->db->where('selesai', $status_kasir_titip_uang);
			$this->db->where('kasir_titip_uang.is_active', 1);
			$this->db->or_where('left(tanggal_kasir_titip_uang, 10) =', $date);
			$this->db->where('selesai', $status_kasir_titip_uang);
	...
	{
		$format = "SELECT MAX(SUBSTRING(`no_member`,16,4)) AS max_no_member FROM `kasir_titip_uang` WHERE SUBSTRING(`no_member`,14,2) = DATE_FORMAT(NOW(), '%y')";	
		return $this->db->query($format);
	}
}
