...
class Permintaan_po extends MY_Controller {
    ...
    {        
        ...
        foreach($records->result_array() as $row)
        {
            ...
            if($row['tipe'] == 1)
            {
                ...
                $info = '<a title="'.translate('Info', $this->session->userdata('language')).'" data-item="'.htmlentities(json_encode($item)).'" class="pilih-item" data-id="'.$row['transaksi_id'].'" name="info"><u>'.$jumlah.' item</u></a>';
            ...
            if($row['tipe'] == 2)
            {
                ...
                $info = '<a title="'.translate('Info', $this->session->userdata('language')).'" data-item="'.htmlentities(json_encode($item)).'" class="item-unlist inline-button-table" data-id="'.$row['transaksi_id'].'" name="info"><u>'.$jumlah.' item</u></a>';
    ...
    public function save()
    {
        ...
        if ($command === 'add')
        {  
            $data_user_level_persetujuan = $this->user_level_persetujuan_m->get_data_order($this->input->post('user_level_id'), 1)->result_array();
            ...
            if (!empty($user_level_persetujuan) && $array_input['tipe_item'] == 1)
            {
                ...
                $divisi_posisi = $this->user_level_m->get($user_level_persetujuan[0]['user_level_menyetujui_id']);
                $data_status = array(
                    'id'               => $id_status,
                    ...
                    'tipe_persetujuan' => 0,
                    'status'           => 1,
                    'user_level_id'    => $user_level_persetujuan[0]['user_level_menyetujui_id'],
                ...
                $user_level = $this->user_level_m->get(7);
            ...
            } elseif(empty($user_level_persetujuan) && $array_input['tipe_item'] == 1) {
                ...
                $divisi_posisi = $this->user_level_m->get(3);
                $data_status = array(
                    'id'              => $id_status,
                    'transaksi_id'    => $no_permintaan,
                    'transaksi_nomor' => $nomor_permintaan,
            ...
            if ($array_input['tipe_item'] == 2)
            {
                ...
                $divisi_posisi = $this->user_level_m->get(14);
                $data_status = array(
                    'id'              => $id_status,
                    'transaksi_id'    => $no_permintaan,
                    'transaksi_nomor' => $nomor_permintaan,
                ...
                sent_notification(14,$nama_user,$no_permintaan);

                $user_level = $this->user_level_m->get(14);
            ...
            foreach ($items as $item) 
            {
                ...
                if ($item['account_id'] != "" && $item['nama'] != "" && $item['jumlah'] != "")
                {
                    ...
                    {    
                        ...
                        $level_id = $this->session->userdata('level_id');
                        if($level_id == 8 || $level_id == 19 || $level_id == 18 || $level_id == 20 || $level_id == 10 || $level_id == 29){
                            $nama_user = $this->session->userdata('nama_lengkap');
                            sent_notification(1,$nama_user,$no_permintaan);
                        }
                        ...
                        $item_sub_kat = $this->item_sub_kategori_m->get_by(array('id' => $item['item_sub_kat_id']), true);
                        ...
                        $data_pembelian =  $this->pembelian_m->get_by(array('id_permintaan' => $no_permintaan, 'tipe_pembelian' => 1, 'supplier_id' => $supplier->id, 'divisi_setting_id' => $item_sub_kat->divisi_setting_id), true);
                        ...
                        $supplier_item = $this->supplier_item_m->get_by(array('supplier_id' => $supplier->id, 'item_id' => $item['account_id'], 'item_satuan_id' => $item['satuan']), true);
                        ...
                        $data_jual_item = array(
                            ...
                            'item_satuan_id_primary' => $satuan_primary->id,
                            'jumlah_pesan'                 => $item['jumlah'],
                            'jumlah_konversi'        => ($item['jumlah'] * $konversi),
                            'jumlah_diterima'     => 0,
                            'jumlah_belum_diterima'     => ($item['jumlah'] * $konversi),
                            'status'                 => 1,
                            'is_active'        => 1,
                            'jumlah_disetujui'       => ($item['jumlah'] * $konversi),
                            'disetujui_oleh'       => 0,
                            'harga_beli'                  => (count($harga_item) == 0)?0:$harga_item[0]['harga'],
                            'tanggal_kirim'          => date('Y-m-d', strtotime($item['item_tanggal_kirim'])),
                            'created_by'             => $this->session->userdata('user_id'),
    ...
    public function listing_alat_obat($status_so_history = null)
    {
       ...
       //die(dump($records));
        ...
        foreach($records->result_array() as $row)
        {
            ...
            $inventory = $this->inventory_m->get_by("item_id = $item_id AND gudang_id = 'WH-05-2016-001' OR item_id = $item_id AND gudang_id = 'WH-05-2016-002'");
            if(count($inventory))
            {
                $stok = 0;
