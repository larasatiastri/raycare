...
mb.app.supplier.add = mb.app.supplier.add || {};
(function(o){
    ...
    var addFieldsetPhone = function(form)
    {
        ...
        $('input.address_radio', $newFieldset).uniform();
    ...
    var addFieldsetAlamat = function(form)
    {
        ...
        // $('a.search_alamat', $newFieldset).attr('href', baseAppUrl + 'modal_search_alamat/' + counter);
    ...
    var addFieldsetEmail = function(form)
    {
        ...
        $('input#radio_primary_email_id_1').prop('checked', true);
        $('input[name$="[is_primary_email]"]').val('');

        $('input#primary_email_id_1').val(1);
    ...
    var handleCheckItemDetail = function() {
        var value = '';
        $.each($('.check-item'), function() {
            if ($(this).is(':checked')) {
                value = value + ',' + $(this).val();
                $('input#item_id_detail').val(value);
            }
        })

        var item_id = $('input#item_id_detail').val(),
            item_id = encodeURIComponent(item_id.substring(1));
        ...
        $.ajax ({ 
            ...
            { 
                $('#multi_select_item').empty();
                $.each(data, function(idx, key){
                    $('#multi_select_item').append($("<option></option>").attr("value",key.id).text(key.nama + ' "' + key.kode + '"'));
                });
            },
            complete : function() {
                Metronic.unblockUI();
    ...
    var handleSelectItemBerdasarkanKode = function() {

        $('select[name="items_berdasarkan_kode[]"]').on('change', function() {
            
            if ($(this).val() != null) {

                var value = $(this).val().toString();
                    value = encodeURIComponent(value.substring(1));
                
                $tableItemDetail.api().ajax.url(baseAppUrl + 'listing_item_detail/' + value).load();
            
            } else {

                $tableItemDetail.api().ajax.url(baseAppUrl + 'listing_item_detail').load();
            }
    ...
    o.init = function(){
        ...
        handleDatePickers();
        handleSelectItemBerdasarkanKode();
        // handlePilihPasien();
...
// initialize  mb.app.supplier.add
$(function(){
    mb.app.supplier.add.init();
});