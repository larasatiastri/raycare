...
class Surat_pengantar extends MY_Controller {
    ...
    {       
        ...
        $this->load->model('klinik_hd/surat_pengantar_m');
        $this->load->model('klinik_hd/surat_pengantar_diagnosa_m');
    ...
    {
        $assets = array();
        $config = 'assets/klinik_hd/surat_pengantar/index';
        $this->config->load($config, true);
        $assets = $this->config->item('assets', $config);
        
        // die(dump( $assets['css'] ));
        $data = array(
            'title'          => config_item('site_name').' | '.translate('Surat Pengantar', $this->session->userdata('language')), 
            'header'         => translate('Surat Pengantar', $this->session->userdata('language')), 
            ...
            'js_files'       => $assets['js'],
            'content_view'   => 'klinik_hd/surat_pengantar/index',
            );
    ...
    {
        $assets = array();
        $assets_config = 'assets/klinik_hd/surat_pengantar/add';
        $this->config->load($assets_config, true);

        $assets = $this->config->item('assets', $assets_config);

        $data = array(
            'title'          => config_item('site_name').' | '.translate("Tambah Surat Pengantar", $this->session->userdata("language")), 
            'header'         => translate("Tambah Surat Pengantar", $this->session->userdata("language")), 
            'header_info'    => config_item('site_name'), 
            'breadcrumb'     => TRUE,
            'menus'          => $this->menus,
            'menu_tree'      => $this->menu_tree,
            'css_files'      => $assets['css'],
            'js_files'       => $assets['js'],
            'content_view'   => 'klinik_hd/surat_pengantar/add',
            'flag'           => 'add',
            'pasien_id'      => $pasien_id
        );
    ...
    public function view($id)
    {
        ...
        $assets = array();
        $config = 'assets/klinik_hd/surat_pengantar/view';
        $this->config->load($config, true);

        $assets = $this->config->item('assets', $config);
        
        $form_data            = $this->surat_pengantar_m->get($id);
        ...
        $data = array(
            ...
            'css_files'            => $assets['css'],
            'js_files'             => $assets['js'],
            'content_view'         => 'klinik_hd/surat_pengantar/view',
    ...
    {        
        $result = $this->surat_pengantar_m->get_datatable();
        ...
        foreach($records->result_array() as $row)
        {
            ...
            {
                $action = '
                    <a title="'.translate('View', $this->session->userdata('language')).'" href="'.base_url().'klinik_hd/surat_pengantar/view/'.$row['id'].'" class="btn btn-xs grey-cascade hidden"><i class="fa fa-search"></i></a>
                    <a target="_blank" title="'.translate('Cetak', $this->session->userdata('language')).'" href="'.base_url().'klinik_hd/surat_pengantar/cetak_pengantar/'.$row['id'].'" class="btn btn-xs default"><i class="fa fa-print"></i></a>
            ...
            $output['data'][] = array(
                ...
                '<div class="text-left">'.$row['nama_rs_poliklinik'].'</div>' ,
    ...
    public function save()
    {
        ...
        {  
            ...
            $max_no = $this->surat_pengantar_m->get_max_no($roman_month)->result_array();
            ...
            $data_surat_pengantar = array(
                ...
                'is_active'            => 1,
                'nama_rs_poliklinik'   => $array_input['nama_rs_poliklinik'],
                'nama_dokter'          => $array_input['nama_dokter'],
            );

            $surat_pengantar_id = $this->surat_pengantar_m->save($data_surat_pengantar);

            foreach ($array_input['diagnosa_awal'] as $diagnosa) 
            {
                $data_diagnosa = array(
                    'surat_pengantar_id' => $surat_pengantar_id,
                    'icd_code'  => $diagnosa['code_ast']
                );

                $surat_pengantar_diagnosa_id = $this->surat_pengantar_diagnosa_m->save($data_diagnosa);
                
            }  
        
            if ($surat_pengantar_id) 
            {
                $flashdata = array(
                    "type"     => "success",
                    "msg"      => translate("Data surat pengantar berhasil ditambahkan.", $this->session->userdata("language")),
        ...
        redirect("klinik_hd/surat_pengantar");
    }
    ...
    public function delete($id)
    {
        ...
        // save data
        $rujukan_id = $this->surat_pengantar_m->save($data, $id);
    ...
    function cetak_pengantar($id) 
    {
        $this->load->library('mpdf/mpdf.php');

        $data_pengantar = $this->surat_pengantar_m->get($id);
        ...
        $pengantar_diagnosa = $this->surat_pengantar_diagnosa_m->get_diagnosa($id)->result_array();
        $pengantar_diagnosa = object_to_array($pengantar_diagnosa);
        $form_kel_alamat  = '';
        $form_kec_alamat  = '';
        $form_kota_alamat = '';
        ...
        $body = array(
            ...
            'pengantar_diagnosa' => $pengantar_diagnosa
        );
        ...
        $mpdf->writeHTML($this->load->view('klinik_hd/surat_pengantar/print_surat_pengantar', $body, true));

        $mpdf->Output('surat_pengantar_'.$data_pengantar['no_surat'].'.pdf', 'I'); 
    }
