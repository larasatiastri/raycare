...
class Penjualan_obat_m extends MY_Model {

	protected $_table        = 'penjualan_obat';
	...
	protected $datatable_columns = array(
		...
		'penjualan_obat.id'      => 'id',
		'penjualan_obat.tanggal'      => 'tanggal',
		'penjualan_obat.no_penjualan' => 'no_penjualan',
		'penjualan_obat.nama_pasien'  => 'nama_pasien',
		'penjualan_obat.alamat_pasien'  => 'alamat_pasien',
		'penjualan_obat.grand_total'  => 'grand_total',
	...
	{
		$format = "SELECT MAX(SUBSTRING(`no_penjualan`,6,3)) AS max_no_penjualan FROM `penjualan_obat` WHERE RIGHT(`no_penjualan`,4) = DATE_FORMAT(NOW(), '%Y');";	
		return $this->db->query($format);
	}

	public function get_max_id_penjualan()
	{
		$format = "SELECT MAX(SUBSTRING(`id`,13,4)) AS max_id FROM `penjualan_obat` WHERE SUBSTR(`id`,5,7) = DATE_FORMAT(NOW(), '%m-%Y');";	
		return $this->db->query($format);
	}
