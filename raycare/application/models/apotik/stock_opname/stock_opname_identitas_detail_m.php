...
class Stock_opname_identitas_detail_m extends MY_Model
{
	protected $_table        = 'stok_opname_identitas_detail';
...
public function printpdf($id)
	{
		$format = "SELECT `item`.`kode` AS item_code, `item`.`nama` AS item_name, `stok_opname_detail`.`jumlah_hitung` AS counted_qty, `stok_opname_detail`.`jumlah_sistem` AS system_qty, `stok_opname_detail`.`id` AS id, `stok_opname_detail`.`stok_opname_id` AS stock_opname_id, `stok_opname_detail`.`item_id` AS item_id
	...
	{
		$format = "select * from inventory_identitas where inventory_id in (SELECT  id FROM inventory WHERE item_id =".$item_id."  AND gudang_id =".$warehouse_id." and item_satuan_id=".$item_satuan_id.")";	
		return $this->db->query($format);
	}
