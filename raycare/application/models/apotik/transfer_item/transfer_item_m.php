...
class Transfer_item_m extends MY_Model {

	protected $_table        = 'transfer_item';
	...
	protected $datatable_columns = array(
		...
		'transfer_item.ke_gudang_id'     => 'ke_gudang_id',
		'transfer_item.tanggal'          => 'tanggal',
		'transfer_item.no_transfer'      => 'no_transfer',
		'transfer_item.no_surat_jalan'   => 'no_surat_jalan',
		'transfer_item.keterangan'       => 'keterangan',
		'transfer_item.status'           => 'status',
		'transfer_item.diterima_oleh'    => 'diterima_oleh',
		'transfer_item.diterima_tanggal' => 'diterima_tanggal',
		'transfer_item.is_active'        => 'is_active',
		'transfer_item.created_by'       => 'created_by',
		'user.nama'                      => 'dikirim_oleh',
	...
	{	
		...
		$params['sort_by'] = 'transfer_item.tanggal';
	...
	public function get_datatable_history_terima($gudang_id)
	{	
		...
		$this->db->where('transfer_item.is_active',1);
		$this->db->where('transfer_item.`status`',2);
		$this->db->where('transfer_item.`ke_gudang_id`',$gudang);
	...
	{
		$format = "SELECT MAX(SUBSTRING(`id`,12,4)) AS max_id FROM `transfer_item` WHERE SUBSTRING(`id`,4,7) = DATE_FORMAT(NOW(), '%m-%Y')";	
		return $this->db->query($format);
	}

	public function get_nomor_transfer()
	{
		$format = "SELECT MAX(SUBSTRING(`no_transfer`,5,4)) AS max_no_transfer FROM `transfer_item` WHERE RIGHT(`no_transfer`,4) = DATE_FORMAT(NOW(), '%Y')";	
		return $this->db->query($format);
	}

	public function get_nomor_do()
	{
		$format = "SELECT MAX(SUBSTRING(`no_surat_jalan`,4,4)) AS max_nomor_do FROM `transfer_item` WHERE RIGHT(`no_surat_jalan`,4) = DATE_FORMAT(NOW(), '%Y')";	
		return $this->db->query($format);
	}
