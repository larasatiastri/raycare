...
mb.app.buat_invoice.add = mb.app.buat_invoice.add || {};
(function(o){
    ...
    function addItemRowPaket()
    {
        ...
        handleCountSubTotalItem($newItemRow);
        // handle delete btn
        handleBtnDelete( $('.del-this', $newItemRow) );
      
    };
    function addItemRowObat()
    {
        ...
        var 
            ...
            ;
        $('input[name$="[kode]"]', $newItemRow).focus();
        $('input[name$="[id]"]', $newItemRow).val('8');
        $('input[name$="[tipe_item]"]', $newItemRow).val('2');
        $('input[name$="[kode]"]', $newItemRow).val('8');
        $('input[name$="[name]"]', $newItemRow).val('Heparin');
        $('input[name$="[qty]"]', $newItemRow).val('1');
        $('input[name$="[harga]"]', $newItemRow).val('60000');
        $('select[name$="[tipe]"]', $newItemRow).val('2');
        $('input[name$="[sub_total]"]', $newItemRow).val('60000');
    ...
    function addItemRow()
    {
        ...
        $('input[name$="[diskon]"]', $newItemRow).val('');
    ...
    var handleDataTablePaket = function(){
        oTablePaket = $tablePaketSearch.dataTable({
            ...
            'language'              : mb.DTLanguage(),
            'ajax'                  : {
                'url' : baseAppUrl + 'listing_paket/1',
                'type' : 'POST',
            },          
            'pageLength'            : 10,
            'lengthMenu'            : [[10, 25, 50, 100], [10, 25, 50, 100]],
    ...
    var handleItemSelect = function($btn){
        $btn.on('click', function(e){
            ...
            var 
                ...
                $itemDiscIn    = $('input[name$="[diskon]"]', $row);
                ...
                $itemHargaIn.val($(this).data('harga')['harga']);
                $itemQtyIn.val(1);
                $itemDiscIn.val(0);
                $itemSubTotIn.val(1 * ($(this).data('harga')['harga']));
    ...
    var handleCountSubTotalItem = function($row) {
        var $itemQtyIn    = $('input[name$="[qty]"]', $row),
            $itemSubTotIn = $('input[name$="[sub_total]"]', $row),
            $itemHargaIn  = $('input[name$="[harga]"]', $row),
            $itemDiskonIn  = $('input[name$="[diskon]"]', $row);

        $($itemQtyIn).on('change keyup', function() {
            var disc = parseFloat($itemDiskonIn.val()),
                qty = $(this).val(),
                hrg = $itemHargaIn.val(),
                disc_nominal = parseFloat((disc/100) * hrg),
                sub_total = parseFloat((hrg - disc_nominal) * qty);


            $itemSubTotIn.val(sub_total);
        });

        $($itemDiskonIn).on('change keyup', function() {
 ...
 }(mb.app.buat_invoice.add));


// initialize  mb.app.home.table
$(function(){
    mb.app.buat_invoice.add.init();
});