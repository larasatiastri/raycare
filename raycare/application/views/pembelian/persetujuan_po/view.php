...
<div class="portlet light">
	...
	<div class="portlet-body form">
			<div class="row">
				...
				<div class="col-md-9">
				<div class="table-scrollable">
					<table class="table table-striped" id="table_detail_pembelian_view">
						<thead>
						...
						<tbody>
							<?php
								$pembelian_detail = $this->persetujuan_po_m->get_data_item_view($pk_value, $form_data_supplier[0]['id'])->result_array();
								...
								foreach ($pembelian_detail as $detail_item) {
									...
									$persetujuan = $this->persetujuan_po_m->get_data_persetujuan($pk_value, $detail_item['id_detail'])->result_array();
									...
									$checked = '';
									$color = '';
									$jumlah = $detail_item['jumlah_pesan'];
										if($persetujuan)
										{
											foreach ($persetujuan as $data_persetujuan) {
												if($data_persetujuan['jumlah'] != null)
												{
													$jumlah = $data_persetujuan['jumlah'];
												}
				  							if($data_persetujuan['status'] == 4 )
					    					{
					    						$checked = 'checked';
									...
									$sub_total = ($detail_item['harga_beli']-($detail_item['harga_beli']*$detail_item['diskon']/100))*$jumlah;
									...
									$info = '<a title="'.translate('Info', $this->session->userdata('language')).'" name="info[]" class="lihat-persetujuan" data-id="'.$detail_item['id_detail'].'" '.$disabled.'>'.$jumlah.'</a>';
											// die_dump($sub_total);
						...
						<tfoot>
							...
							<?php
								...
								$grand_total =  $tat - intval($data_head_detail[0]['pph_23_nominal']) + intval($data_head_detail[0]['biaya_tambahan']) ;
							?>
							<tr>
								<td colspan="9" class="text-right bold">PPH 23</td>
								<td colspan="2">
									<div class="input-group col-md-12">
										<span class="input-group-addon">
											&nbsp;Rp&nbsp;
										</span>
										<input class="form-control text-right" readonly type="text" value="<?=formattanparupiahstrip($data_head_detail[0]['pph_23_nominal'])?>" id="pph23" name="pph23">
...
<div id="popover_item_content" class="row">
    <div class="col-md-12">
    	<div class="portlet">
			<div class="portlet-body">
		        <table class="table table-condensed table-striped table-hover" id="table_data_persetujuan">
		            <thead>
		                <tr>
