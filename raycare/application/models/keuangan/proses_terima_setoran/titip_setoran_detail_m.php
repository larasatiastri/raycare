...
class Titip_setoran_detail_m extends MY_Model {
	 ...
	 */
	public function get_datatable($titip_setoran_id)
	{

		$join1 = array('permintaan_biaya', $this->_table.'.detail_id = permintaan_biaya.id', 'LEFT');
	...
	{
		$this->db->select("DATE_FORMAT(permintaan_biaya.tanggal,'%d %b %Y') as tanggal, permintaan_biaya.nomor_permintaan, permintaan_biaya.nominal_setujui, permintaan_biaya.keperluan, user.nama as nama_minta");
		$this->db->join('permintaan_biaya', $this->_table.'.detail_id = permintaan_biaya.id','left');
		$this->db->join('user', 'permintaan_biaya.diminta_oleh_id = user.id','left');
		$this->db->where($this->_table.'.tipe_detail',1);
		$this->db->where($this->_table.'.titip_setoran_id',$id);
