<?php
	...
	$class_detail = (count($data_invoice) == 0)?'col-md-9':'col-md-4';

	$supplier_data = $this->supplier_m->get_by(array('id' => $form_data[0]['id']), true);
?>
					...
					if(count($data_invoice) != 0){
						...
						<div class="col-md-5">
							<div class="portlet light bordered">
								<div class="portlet-title">
									<div class="caption">
										<span class="caption-subject"><?=translate("Detail Invoice", $this->session->userdata("language"))?></span>
									</div>
								</div>
								<div class="portlet-body">
									<div class="form-body">
									<?php
										...
										if(count($data_invoice) != 0){
											...
											foreach ($data_invoice as $key => $bon) {
												...
												<td style="vertical-align: top !important;" width="10px">'.date('d M Y', strtotime($bon->tgl_invoice)).'</td>
												<td><a class="fancybox-button" title="'.$bon->url.'" href="'.base_url().'/assets/mb/pages/keuangan/pembayaran_transaksi/images/'.$data_bayar->id.'/'.$bon->url.'" data-rel="fancybox-button"><img src="'.base_url().'/assets/mb/pages/keuangan/pembayaran_transaksi/images/'.$data_bayar->id.'/'.$bon->url.'" alt="Smiley face" class="img-responsive" ></a></td>';
												
												if($supplier_data->is_pkp == 1){
												
												$form_upload_inv .= '<td><a class="fancybox-button" title="'.$bon->url_faktur_pajak.'" href="'.base_url().'/assets/mb/pages/keuangan/pembayaran_transaksi/images/'.$data_bayar->id.'/'.$bon->url_faktur_pajak.'" data-rel="fancybox-button"><img src="'.base_url().'/assets/mb/pages/keuangan/pembayaran_transaksi/images/'.$data_bayar->id.'/'.$bon->url_faktur_pajak.'" alt="Smiley face" class="img-responsive" ></a></td>';
												}
