...
class Biaya extends MY_Controller {
    ...
    {       
        ...
        $this->load->model('reservasi/biaya/laporan_biaya_m');
    ...
    {
        $assets = array();
        $config = 'assets/reservasi/biaya/index';
        ...
        $data = array(
            ...
            'js_files'       => $assets['js'],
            'content_view'   => 'reservasi/biaya/index',
            );
    ...
    public function save()
    {
        ...
        {  
            ...
            $cabang_id = $this->pendaftaran_tindakan_m->save($data);
            //die_dump($this->db->last_query());           
            ...
            if ($cabang_id) 
            {
              ...
              redirect("reservasi/pendaftaran_tindakan/refresh",'refresh');
    ...
    {   
        ...
        $result = $this->laporan_biaya_m->get_datatable($id,$tggl);
       $result2 = $this->laporan_biaya_m->get_datatable_total($id,$tggl);
        //die_dump($result);
        $output = array(
            'draw'                 => intval($this->input->post('draw', true)),
            'iTotalRecords'        => $result->total_records,
        ...
        foreach($records->result_array() as $row)
        {
            ...
            $output['data'][] = array(
               '<div class="text-center">'. date('d',strtotime($row['tanggal'])).'<input type="hidden" id="total_'.$i.'" name="total['.$i.']" value="'.$total.'"></div>',
