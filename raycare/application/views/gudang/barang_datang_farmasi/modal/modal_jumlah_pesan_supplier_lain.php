...
<div class="modal-body">
    ...
    <table class="table table-condensed table-striped table-hover" id="table_jumlah_pesan_supplier_lain">
        <thead>
            <tr role="row">
                <th><div class="text-center"><?=translate('No.PO', $this->session->userdata('language'))?></div></th>
                <th><div class="text-center"><?=translate('Tanggal Pesan', $this->session->userdata('language'))?></div></th>
                <th><div class="text-center"><?=translate('Tanggal Kadaluarsa', $this->session->userdata('language'))?></div></th>
                <th><div class="text-center"><?=translate('Tanggal Kirim', $this->session->userdata('language'))?></div></th>
...
<script type="text/javascript">
    ...
    function handleDataTableModal(){
        ...
        oTable = $('#table_jumlah_pesan_supplier_lain').dataTable({
            'processing'            : true,
            'serverSide'            : true,
            'info'                  : false,
            'paging'                : false,
            ...
            'ajax'                  : {
                'url' : baseAppUrl + 'listing_jumlah_pesan_supplier_lain/'+ $('input#supplier_id_modal').val() + '/' + $('input#item_id_modal').val() + '/' + $('input#item_satuan_id_modal').val(),
                'type' : 'POST',
            },          
            'pageLength'            : 10,
            ...
            'columns'               : [
                ...
                { 'name':'pembelian_detail.jumlah_pesan jumlah_pesan','visible' : true, 'searchable': true, 'orderable': true },
