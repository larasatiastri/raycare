                                ...
                                <?php
                                    if($data_item['is_identitas'] == 1){
                                        ...
                                        foreach ($data_inventory as $inventory) {
                                            ...
                                            echo '<td class="text-center" style="width : 5% !important;">'.$i.'</td>'.$rowbn.$rowexpire.$rowdll1.$rowdll2.$rowdll3.' <td class="text-left" style="width : 16% !important;">'.$inventory['jumlah'].'</td><td class="text-left" style="width : 16% !important;"><div class="input-group"><input type="number" class="form-control jumlah" id="jml_'.$i.'" name="identitas_'.$item_id.'['.$index.'][jumlah_identitas]" value="0" min="0" max="'.$inventory['jumlah'].'"><span class="input-group-addon">&nbsp;'.$data_item_satuan['nama'].'&nbsp;</span></div><input type="hidden" id="jml_'.$i.'" name="identitas_'.$item_id.'['.$index.'][inventory_id]" value="'.$inventory['inventory_id'].'"><input type="hidden" id="jml_'.$i.'" name="identitas_'.$item_id.'['.$index.'][harga_beli]" value="'.$inventory['harga_beli'].'"></td></tr>';
...
function handleConfirmSave() {
    $('a#confirm_save_modal').click(function(){
        ...
        $('input[name$="[qty]"]', $row).attr('value', $('input#total_jml').val());

        var harga_item = $('input[name$="[harga]"]', $row).val();

        var sub_total = parseInt($('input#total_jml').val() * harga_item);

        $('div#item_sub_total', $row).text(mb.formatRp(sub_total));
        $('input[name$="[sub_total]"]', $row).val(sub_total);

        var $itemSubTotIn = $('input[name$="[sub_total]"]', $('table#tabel_tambah_item'));
        ...
        $('th#total', $('table#tabel_tambah_item')).text(mb.formatRp(grand_total));
        $('input#total_hidden', $('table#tabel_tambah_item')).val(grand_total);


        $('th#grand_total', $('table#tabel_tambah_item')).text(mb.formatRp(grand_total));
        $('input#grand_total_hidden', $('table#tabel_tambah_item')).val(grand_total);
