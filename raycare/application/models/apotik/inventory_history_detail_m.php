...
class Inventory_history_detail_m extends MY_Model {

	protected $_table        = 'inventory_history_detail';
	...
	protected $datatable_columns = array(
		...
		'inventory_history_detail.created_date'             => 'tanggal',
		'item.kode'                                         => 'kode_item',
		'item.id'                                         	=> 'id_item',
		'item.nama'                                         => 'nama_item',
		'item_satuan.nama'                                  => 'nama_satuan',
		'item_satuan.id'                                  	=> 'id_item_satuan',
		'SUM(inventory_history_detail.change_stock * (-1))' => 'jumlah',
		'inventory_history_detail.item_id'                  => 'item_id',
		'inventory_history_detail.item_satuan_id'           => 'item_satuan_id',
		'(select SUM(jumlah) from inventory where item_id = item.id and item_satuan_id = item_satuan.id and gudang_id = GROUP BY item_id, item_satuan_id)' => 'stock'
	...
	{		
		...
		//column di table  => alias
			...
			'inventory_history_detail.item_id'                  => 'item_id',
			'inventory_history_detail.item_satuan_id'           => 'item_satuan_id',
			"(select SUM(jumlah) from inventory where item_id = item.id and item_satuan_id = item_satuan.id and gudang_id = '".$gudang_id."' GROUP BY item_id, item_satuan_id)" => 'stock'
		...
		$this->db->where('date(inventory_history_detail.created_date) >= ',$tgl_awal);
		$this->db->where('date(inventory_history_detail.created_date) <= ',$tgl_akhir);
		$this->db->where('inventory_history_detail.gudang_id',$gudang_id);
		...
		$this->db->group_by('date(inventory_history_detail.created_date)');
