...
(function(o){
     ...
     var 
        ...
        $tableAddAccount        = $('#table_add_item_terdaftar', $form),
        $tableAddAccountTitipan = $('#table_add_item_tidak_terdaftar', $form),
       ...
       var initForm = function(){
        ...
        var 
            ...
            $btnInfo                   = $('.pilih-item-terdaftar-user', $tableAddAccount),
            $btnStock                  = $('.stock', $tableAddAccount),
            $btnPermintaan             = $('.permintaan', $tableAddAccount),
            $btnInfoItemTidakTerdaftar = $('.pilih-item-tidak-terdaftar-user', $tableAddAccountTitipan),
        ...
        // handleBtnSearchAccount($btnInfo);  

        // handleBtnSearchAccountTitipan($btnSearchAccountTitipan);  
        // handleBtnSearchAccountTitipan($btnInfoItemTidakTerdaftar);  
        ...
        // $.each($btnStock, function(idx, btn){
        //     semua_stock($(btn), $(this).data('row'));  
        //     // alert($(this).data('row'));
        // });

        // $.each($btnPermintaan, function(idx, btn){
        //     semua_permintaan($(btn), $(this).data('row'));  
        //     // alert($(this).data('row'));
        // });
        ...
        // info_item_terdaftar();
        // info_item_tidak_terdaftar();


    };

    var semua_stock = function(){

        $btnStock = $('.stock');

        $btnStock.on('click', function(e){

            $row        = $('#item_row_' + $(this).data('row'), $tableAddAccount),
            $itemJumlahKirimIn = $('input[name$="[jumlah_kirim]"]', $row);
            $itemJumlahKirimEl = $('label[name$="[jumlah_kirim]"]', $row);

            // alert('klik');
            $itemStockIn    = $('input[name$="[stock]"]', $row).val();
            $itemJumlahKirimIn.val($itemStockIn);
            $itemJumlahKirimEl.text($itemStockIn);
            // alert($itemJumlahKirimEl.text($itemStockIn));
            // alert($itemJumlahKirimIn.val($itemStockIn));
                    
        });

    }

    var semua_permintaan = function($btn){


        $btnPermintaan = $('.permintaan');

        $btnPermintaan.on('click', function(e){
            ...
            // alert('klik');
            $itemPermintaanIn    = $('input[name$="[permintaan]"]', $row).val();
            // alert($itemPermintaanIn);
            $itemJumlahKirimIn.val($itemPermintaanIn);
            $itemJumlahKirimEl.text($itemPermintaanIn);
            // alert($itemJumlahKirimIn.val($itemPermintaanIn));
                    
        });

    }

    var info_item_terdaftar = function(){
        ...
        $btnInfo.on('click', function(){
            ...
            var $anchor  = $(this),
                ...
                // alert('klik');
            oTablePilihItem.api().ajax.url(baseAppUrl + 'listing_pilih_item_user/' + id + '/' + item_id).load();
            // oTablePilihItemTidakTerdaftar.api().ajax.url(baseAppUrl + 'listing_pilih_item_tidak_terdaftar/' + id).load();
                    
        });

        var $btnSearchItem  = $('.pilih-item-terdaftar-user');
    ...
    var info_item_tidak_terdaftar = function(){

        var 
            $btnInfo = $('.pilih-item-tidak-terdaftar-user', $tableAddAccountTitipan)
        ;
        ...
        $btnInfo.on('click', function(){
            ...
            var $anchor  = $(this),
                id       = $anchor.data('id');
                level_id = $anchor.data('level-id');
                item_id  = $anchor.data('item-other');
                // alert('klik');
            oTablePilihItemTidakTerdaftar.api().ajax.url(baseAppUrl + 'listing_pilih_item_user_tidak_terdaftar/' + id + '/' + item_id).load();
            // oTablePilihItemTidakTerdaftar.api().ajax.url(baseAppUrl + 'listing_pilih_item_tidak_terdaftar/' + id).load();
    ...
    var handlePilihItem = function(){
       ...
       // user_level_id = $('input#user_level_id').val();
        oTablePilihItem = $tablePilihItem.dataTable({
            ...
            'ajax'                  : {
                'url' : baseAppUrl + 'listing_pilih_item_user',
                'type' : 'POST',
            },          
            'pageLength'            : 10,
    ...
    var handlePilihItemTidakTerdaftar = function(){
       ...
       // user_level_id = $('input#user_level_id').val();
        oTablePilihItemTidakTerdaftar = $tablePilihItemTidakTerdaftar.dataTable({
            ...
            'pagingType'            : 'full_numbers',
            'ajax'                  : {
                'url' : baseAppUrl + 'listing_pilih_item_user_tidak_terdaftar',
                'type' : 'POST',
            },          
            'pageLength'            : 10,
            'lengthMenu'            : [[10, 25, 50, 100], [10, 25, 50, 100]],
    ...
    var handleDataTableItems = function(){
       ...
       oTable = $tableItemTerdaftar.dataTable({
            ...
            'pagingType'            : 'full_numbers',
            'ajax'                  : {
                'url' : baseAppUrl + 'listing_item/' + pk_value + '/' + user_level_id,
                'type' : 'POST',
            },          
            'pageLength'            : 10,
            'lengthMenu'            : [[10, 25, 50, 100], [10, 25, 50, 100]],
       ...
       //      oTable.api().ajax.url(baseAppUrl + 'listing_alat_obat/' +$(this).val()).load();
       //    // alert($(this).val());
       //  });
            // oTable.api().ajax.url(baseAppUrl + 'listing_alat_obat/' + id).load();
                ...
                $tableItemTerdaftar.on('draw.dt', function (){
                    ...
                    $('a[name="info_item_terdaftar_user[]"]', this).click(function(){
                        // alert('klik');
                    var $anchor = $(this),
                          id    = $anchor.data('id');
                          level_id = $anchor.data('level-id');

                    oTablePilihItem.api().ajax.url(baseAppUrl + 'listing_pilih_item_user/' + id + '/' + level_id).load();
                    // oTablePilihItemTidakTerdaftar.api().ajax.url(baseAppUrl + 'listing_pilih_item_tidak_terdaftar/' + id).load();
    ...
    var handleBtnSearchItem = function($btn){
     ...
     jQuery('#table_add_item_terdaftar .group-checkable').change(function () {
    ...
    //   $('#table_item_terdaftar').on('change', 'tbody tr .checkboxes', function(){
    ...
    // });
     ...
     jQuery('#table_add_item_tidak_terdaftar .group-checkable').change(function () {
        var set = jQuery(this).attr("data-set");
        var checked = jQuery(this).is(":checked");
    ...
    var handleDataTableItemsTitipan = function(){
       ...
       user_level_id = $('input#user_level_id').val();
        oTable2 = $tableItemTidakTerdafar.dataTable({
            ...
            'pagingType'            : 'full_numbers',
            'ajax'                  : {
                'url' : baseAppUrl + 'listing_item_tidak_terdaftar/' + pk_value + '/' + user_level_id,
                'type' : 'POST',
            },          
            'pageLength'            : 10,
            'lengthMenu'            : [[10, 25, 50, 100], [10, 25, 50, 100]],
        ...
        $tableItemTidakTerdafar.on('draw.dt', function (){
            ...
            $('a[name="info_item_tidak_terdaftar_user[]"]', this).click(function(){
                // alert('klik');
            var $anchor = $(this),
                  id    = $anchor.data('id');
                  level_id = $anchor.data('level-id');
            ...
            oTablePilihItemTidakTerdaftar.api().ajax.url(baseAppUrl + 'listing_pilih_item_user_tidak_terdaftar/' + id + '/' + level_id).load();
    ...
    var handleDatePickers = function () {
        var time = new Date($('#tanggal_transfer').val());
...
var handleValidation = function() {
    ...
    var handleInfoIdentitas = function(){
        ...
        $btnCheckIdentitas.on('click', function(){
            ...
            // alert(id);
            if ($('label#items_jumlah_kirim_'+id).text() == 0) {
                // alert('masih 0');
    ...
    o.init = function(){
        baseAppUrl = mb.baseUrl() + 'gudang/transfer_item/';
        handleValidation();
        calculateTotalKeseluruhan();
        initForm();
        handleDatePickers();
