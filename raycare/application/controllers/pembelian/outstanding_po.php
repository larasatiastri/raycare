...
class Outstanding_po extends MY_Controller {

    // protected $menu_parent_id = 2;           // deprecated, pake $menu_tree
    protected $menu_id = 'f86b1092f6ccc5a36e25f3dd66fd6d08';  
    // protected $menu_id = 'pembelian/outstanding_po';                  // untuk check bit_access
    ...
    {
        $assets = array();
        $config = 'assets/pembelian/outstanding_po/index';
        $this->config->load($config, true);
        $assets = $this->config->item('assets', $config);
        
        // die(dump( $assets['css'] ))

        $data_kasbon = $this->permintaan_biaya_barang_m->get_data_group(1)->result_array();
        ...
        $data = array(
            ...
            'content_view'   => 'pembelian/outstanding_po/index',
            );
    ...
    {
        $assets = array();
        $assets_config = 'assets/pembelian/outstanding_po/add';
        ...
        $data = array(
            ...
            'css_files'      => $assets['css'],
            'js_files'       => $assets['js'],
            'content_view'   => 'pembelian/outstanding_po/add',
        );
    ...
    {               
        ...
        foreach($records->result_array() as $row)
        {
            ...
            foreach ($supplier as $sup) {
                ...
                $data_supplier = $this->supplier_m->get_by(array('id' => $sup->supplier_id, 'is_active' => 1), true);
    ...
    public function generate_po()
    {
        if($this->input->is_ajax_request()){
            ...
            foreach ($array_input['data_kasbon_id'] as $key_kasbon => $kasbon_value) {
                $kasbon_id[$key_kasbon] = $kasbon_value;
            }
            ...
            foreach ($array_input['data_supplier_id'] as $key => $supplier_id) {
                ...
                $supplier[$supplier_id]['kasbon_id'][] = $kasbon_id[$key];
            }
            ...
            foreach ($array_input['data_supplier_id'] as $key => $supplier_id) {
                ...
                $supplier[$supplier_id]['kasbon_id_array'] = urlencode(base64_encode(serialize($supplier[$supplier_id]['kasbon_id'])));
            }
    ...
    {
        $this->load->view('pembelian/outstanding_po/modals/tolak_permintaan');
