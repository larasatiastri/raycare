...
class Grafik_tindakan_visitasi extends MY_Controller {

    // protected $menu_parent_id = 2;           // deprecated, pake $menu_tree
    protected $menu_id = 'af4c68ab09ff086fe9c75e18cf312e3a';                  // untuk check bit_access
    private $menus = array();
    ...
    {       
        ...
        $this->load->model('pegawai/fee_dokter_m');
    ...
    {
        $assets = array();
        $config = 'assets/laporan/grafik_tindakan_visitasi/index';
        ...
        $data = array(
            ...
            'js_files'       => $assets['js'],
            'content_view'   => 'laporan/grafik_tindakan_visitasi/index',
        );
    ...
    function get_day_trans($start, $penjamin_id)
    {
        ...
        $sql .= " AND a.status != 1 AND a.status != 4 AND a.is_manual IS NULL AND a.is_active = 1 GROUP BY
                    date(`tanggal`)";
    ...
    //die(dump($records));
        ...
        {         
            ...
            $pegawai = $this->pegawai_m->get_by(array('cabang_id' => $cabang_id, 'jabatan_id' => 21), true);

            $fee_dokter = $this->fee_dokter_m->get_current_salary($pegawai->id)->result_array();
            // die(dump($this->db->last_query()));
            $sub_total = ($row['trans_count']*$fee_dokter[0]['nominal']);
            ...
            if($i == $count)
            {
                ...
                $input_total    = '<input type="hidden" id="input_total" value="'.$total.'">';
            }

           

            $output['data'][] = array(
                '<div class="text-center"><b><a href="'.base_url().'laporan/grafik_tindakan_visitasi/get_tindakan/'.date('d-M-Y', strtotime($row['date'])).'/'.$penjamin_id.'" data-toggle="modal" data-target="#modal_laporan">'.date('d', strtotime($row['date'])).'</a></b></div>',
                ...
                '<div class="text-right">'.formatrupiah($sub_total).$input_total.'</div>', 
    ...
    public function cetak_data_pasien($start, $penjamin_id,$tipe)
    {
        ...
        $data = array(
            ...
            'css_files'    => $assets['css'],
            'js_files'     => $assets['js'],
            'content_view' => 'laporan/grafik_tindakan_visitasi/cetak_data_pasien',
        );
    ...
    public function get_tindakan($tgl,$penjamin_id)
    {
        ...
        $this->load->view('laporan/grafik_tindakan_visitasi/data_tindakan', $data);
