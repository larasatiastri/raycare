...
class Transaksi_dokter3_m extends MY_Model {
	...
	{	
	 	...
	 	$join1 = array('dokumen', $this->_table . '.dokumen_id = dokumen.id');
		...
		$this->db->where('pasien_dokumen.pasien_id',$id);
		$this->db->where('dokumen.is_active',1);
		...
		$this->db->where('pasien_dokumen.pasien_id',$id);
		$this->db->where('dokumen.is_active',1);
		// $this->db->where('simpan_item_hd.pasien_id',$id);
		// $this->db->where('item_satuan.is_primary',1);
 ...
 // die(dump($result->records));
	...
	public function get_data_dokumen($pasien_id)
	{
		$SQL = "SELECT * FROM dokumen WHERE id NOT IN (SELECT dokumen_id FROM pasien_dokumen WHERE pasien_id = ?) AND dokumen.is_active = 1";
		return $this->db->query($SQL,array($pasien_id));
	}
