...
<form action="#" method="post" id="form_identitas" class="form-horizontal">
    ...
    <div class="modal-body">
        <div class="portlet light">
            <div class="portlet-title">
                <div class="caption">
                    <span class="caption-subject font-blue-sharp bold uppercase"><?=translate("Identias Item", $this->session->userdata("language"))?></span>
                </div>
            ...
            <div class="portlet-body">
                <input type="text" class="hidden" id="item_id" value="<?=$item_id?>">
                <input type="text" class="hidden" id="row_id" value="<?=$row_id?>">
                <input type="text" class="hidden" id="identitasCounter" value="1">
                    ...
                    foreach ($group_indetitas as $group) {
                        ...
                        if (!empty($cek_identitas)) {
                            ...
                            foreach ($item_identitas as $data_item_identitas) {
                                ...
                                $get_item_identitas_detail = $this->inventory_m->get_item_identitas1($data_item_identitas['inventory_identitas_id']);
                                ...
                                foreach ($item_identitas_detail as $data) {
                                    if ($data['inventory_identitas_id'] != NULL) {
                                        $type .= '<td>
                                                    ...
                                                    <input type="hidden" id="identitas_id_'.$data['inventory_identitas_id'].'_'.$i.'" name="identitas_detail_'.$item_id.'_'.$i.'['.$z.'][id]" value="'.$data['inventory_identitas_id'].'">
                                                    <input type="hidden" id="identitas_judul_'.$data['inventory_identitas_id'].'_'.$i.'" name="identitas_detail_'.$item_id.'_'.$i.'['.$z.'][judul]" value="'.$data['judul'].'">
                                                    <input type="hidden" id="identitas_value_'.$data['inventory_identitas_id'].'_'.$i.'" name="identitas_detail_'.$item_id.'_'.$i.'['.$z.'][value]" value="'.$data['value'].'">
                                                  </td>';  
                                    }
                                $z++;
                                ...
                                $type .= '<td class="text-center">
                                        ...
                                        <input type="hidden" id="identitas_gudang_id_'.$data['inventory_identitas_id'].'_'.$i.'" name="identitas_'.$item_id.'['.$i.'][gudang_id]" value="'.$group['gudang_id'].'">
                                        <input type="hidden" id="identitas_pmb_id_'.$data['inventory_identitas_id'].'_'.$i.'" name="identitas_'.$item_id.'['.$i.'][pmb_id]" min="1" value="'.$group['pmb_id'].'">
                                        <input type="hidden" id="identitas_inventory_id_'.$data['inventory_identitas_id'].'_'.$i.'" name="identitas_'.$item_id.'['.$i.'][inventory_id]" value="'.$group['inventory_id'].'">
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function(){
            ...
            tambahIdentitasRow();
            // selectIndentitas();
            baseAppUrl = mb.baseUrl()+'gudang/transfer_item/proses/'
        ...
        function modalOK()
        {
            ...
            $.each($('input[name$="[harga_beli]"]', $('#table_identitas')), function(idx, value){
                harga_beli = parseInt(this.value);
            ...
            $row        = $('#'+row_id, $('#table_add_item_terdaftar'));

            // alert(row_id);
            $('input[name$="[jumlah_kirim]"]', $row).val(total_jumlah);
            $('label[name$="[jumlah_kirim]"]', $row).text(total_jumlah);
            $('input[name$="[jumlah_kirim]"]', $row).attr('value', total_jumlah);

            $('input[name$="[harga_beli]"]', $row).val(harga_beli);
            $('label[name$="[harga_beli]"]', $row).text(harga_beli);
            $('input[name$="[harga_beli]"]', $row).attr('value', harga_beli);
            ...
            var jumlah = parseInt($('input[name$="[jumlah_kirim]"]', $row).val()),
                harga = parseInt($('input[name$="[harga_titipan]"]', $row).val());

            // total = jumlah * harga;

            // subtotal = $('input[name$="[sub_total_titipan]"]', $row).val(total);
            // subtotalIn = $('input[name$="[sub_total_titipan]"]', $row).attr('value', total);
            // subtotalEl = $('label[name$="[sub_total_titipan]"]', $row).text(mb.formatTanpaRp(total));
        ...
        function setHarga()
        {
            $classrow   = $('.row_plus', $('#table_daftar_obat'));
            hargaAll    = $('input[name$="[sub_total_titipan]"]', $classrow);
            ...
            // alert(harga);
            $('label#total').text(mb.formatTanpaRp(harga));
            $('input#total').val(harga);
        ...
        function setJumlah($btn){
            ...
            $btn.on('change', function(){
                ...
                if (jumlah > stock) {
                    ...
                    $('input#identitas_jumlah_'+rowId).val($(this).val());
                    $('input#identitas_jumlah_'+rowId).attr('value', $(this).val());
                }else{
                    $('input#identitas_jumlah_'+rowId).val($(this).val());
                    $('input#identitas_jumlah_'+rowId).attr('value', $(this).val());
                }
