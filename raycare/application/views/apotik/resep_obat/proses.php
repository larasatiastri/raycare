		...
		<div class="form-body">
			...
			<div class="row">
				...
				<div class="col-md-9">
											...
											foreach ($form_data_item as $item) {
												...
												if ($item['is_identitas'] == 1) {
													$input_jumlah = '<div class="input-group">
																<input readonly type="number" min="0" max="'.$item['jumlah'].'" class="form-control text-right" value="0" name="items['.$i.'][jumlah_kirim]" id="jumlah_kirim_'.$i.'">
																<input type="hidden" value="'.$item['jumlah'].'" class="form-control text-right" name="items['.$i.'][jumlah_pesan]" id="jumlah_pesan_'.$i.'">
																<span class="input-group-btn"><button type="button" data-toggle="modal" data-target="#popup_modal_jumlah_keluar" href="'.base_url().'apotik/resep_obat/add_identitas/item_row_'.$i.'/'.$item['item_id'].'/'.$form_data['cabang_id'].'" class="btn blue-chambray add-identitas" name="item['.$i.'][identitas]" title="Tambah Jumlah"><i class="fa fa-info"></i></button></span>
															</div>';
												}
					...
					<?php 
						if(count($form_data_item_pulang) != 0){
							?>
					<div class="portlet light bordered">
						<div class="portlet-title">
							<div class="caption">
								<?=translate("Daftar Item Dibawa Pulang", $this->session->userdata("language"))?>
							</div>
						</div>
						<div class="portlet-body form">

						<div class="table-scrollable">
								<table class="table table-bordered table-hover table-striped responsive" id="table_item_pulang">
									<thead>
										<tr>
											<th class="text-center" width="10%"><?=translate("Kode Item", $this->session->userdata("language"))?></th>
											<th class="text-center"><?=translate("Nama Item", $this->session->userdata("language"))?></th>
											<th class="text-center" width="10%"><?=translate("Jumlah", $this->session->userdata("language"))?></th>
											<th class="text-center" width="10%"><?=translate("Aturan Pakai", $this->session->userdata("language"))?></th>
											<th class="text-center" width="10%"><?=translate("Jumlah Kirim", $this->session->userdata("language"))?></th>
										</tr>
									</thead>
									<tbody>
										...
										<?php 
											...
											$i = 0;
											foreach ($form_data_item_pulang as $item) {

												$aturan = '';
												if($item['aturan'] == 1) $aturan = 'PC';
												if($item['aturan'] == 2) $aturan = 'AC';
												if($item['aturan'] == 3) $aturan = 'SL';
												if($item['aturan'] == 4) $aturan = 'SC';
												if($item['aturan'] == 5) $aturan = 'IV';
												if($item['aturan'] == 6) $aturan = 'IM';
												if($item['aturan'] == 7) $aturan = 'UE';
												...
												// Set aksi jika identitas dan jika bukan identitas
												$input_jumlah = '<div class="input-group"><div style="width:80px;"><input type="number" min="0" class="form-control text-right" value="0" name="items_pulang['.$i.'][jumlah_kirim]" id="jumlah_kirim_'.$i.'"></div><span class="input-group-addon">
															'.form_dropdown('items_pulang['.$i.'][item_satuan_id_kirim]', $satuan_option, '', 'id="id_satuan_item_'.$i.'" class="form_control"').'
														</span></div>';
												if ($item['is_identitas'] == 1) {
													$input_jumlah = '<div class="input-group">
																<input readonly type="number" min="0" max="'.$item['jumlah'].'" class="form-control text-right" value="0" name="items_pulang['.$i.'][jumlah_kirim]" id="jumlah_kirim_'.$i.'">
																<input type="hidden" value="'.$item['jumlah'].'" class="form-control text-right" name="items_pulang['.$i.'][jumlah_pesan]" id="jumlah_pesan_'.$i.'">
																<span class="input-group-btn"><button type="button" data-toggle="modal" data-target="#popup_modal_jumlah_keluar" href="'.base_url().'apotik/resep_obat/add_identitas_pulang/item_pulang_'.$i.'/'.$item['item_id'].'/'.$form_data['cabang_id'].'" class="btn blue-chambray add-identitas" name="item['.$i.'][identitas]" title="Tambah Jumlah"><i class="fa fa-info"></i></button></span>
															</div>';
												}

												echo '<tr id="item_pulang_'.$i.'" >
														<td class="text-center"><input class="form-control hidden" id="tindakan_resep_obat_detail_id" value="'.$item['id'].'" name="items_pulang['.$i.'][tindakan_resep_obat_detail_id]"><input class="form-control hidden" id="item_id" value="'.$item['item_id'].'" name="items_pulang['.$i.'][item_id]"><input class="form-control hidden" id="satuan_id" value="'.$item['satuan_id'].'" name="items_pulang['.$i.'][item_satuan_id]">'.$item['item_kode'].'<div id="identitas_pulang_row" class="hidden"></td>
														<td class="text-left">'.$item['item_nama'].' <div id="modal_identitas_temp"></div> </td>
														<td class="text-center">'.$item['jumlah'].' '.$item['satuan'].'</td>
														<td class="text-center">'.$item['dosis'].' '.$aturan.'</td>
														<td class="text-center">
															'.$input_jumlah.'
														</td>
