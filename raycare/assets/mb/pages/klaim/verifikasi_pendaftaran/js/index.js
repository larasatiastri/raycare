...
(function(o){
    ...
    var 
        baseAppUrl              = '',
        $tableVerifikasiPendaftaran = $('#table_verifikasi_pendaftaran');

    var handleDataTable = function() 
    {
    	$tableVerifikasiPendaftaran.dataTable({
           	'processing'            : true,
			'serverSide'            : true,
			'stateSave'             : true,
			...
			'columns'               : [
				...
				{ 'name' : 'pendaftaran_tindakan.created_date tanggal_daftar', 'visible' : true, 'searchable': true, 'orderable': true },
				...
				{ 'name' : '`user`.nama nama_user', 'visible' : true, 'searchable': false, 'orderable': false },
        		]
        });
        $tableVerifikasiPendaftaran.on('draw.dt', function (){
			$('.btn', this).tooltip();
