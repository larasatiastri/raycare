	...
	<div class="modal-body" id="section-identitas">
				<div class="form-body">
	                ...
	                if($data_inventory != ''){
						...
						foreach ($data_inventory as $inventory) {
							...
							$data_satuan = $this->item_satuan_m->get_by(array('item_id' => $inventory['item_id']));
				            $data_satuan_primary = $this->item_satuan_m->get_by(array('item_id' => $inventory['item_id'],'is_primary' => 1), true);

				            // $harga_item = $this->item_harga_m->get_harga_item_satuan($inventory['item_id'],$data_satuan_primary->id)->row(0);
							...
							$type .='<td>'.$inventory['nama_gudang'].'<input type="hidden" class="form-control" name="identitas_pulang_'.$item_id.'_'.$index.'['.$i.'][gudang_id]" value="'.$inventory['gudang_id'].'"></td>';
							$type .='<td>'.$inventory['nama_satuan'].'<input type="hidden" class="form-control" name="identitas_pulang_'.$item_id.'_'.$index.'['.$i.'][item_id]" value="'.$inventory['item_id'].'"><input type="hidden" class="form-control" name="identitas_pulang_'.$item_id.'_'.$index.'['.$i.'][item_satuan]" value="'.$inventory['item_satuan_id'].'"><input type="hidden" class="form-control" name="identitas_pulang_'.$item_id.'_'.$index.'['.$i.'][item_harga]" value="'.$harga_item->harga.'"></td>';
							$type .='<td><label>'.$inventory['bn_sn_lot'].'</label><input type="hidden" id="bn_sn_lot_'.$i.'" name="identitas_pulang_'.$item_id.'_'.$index.'['.$i.'][bn_sn_lot]" value="'.$inventory['bn_sn_lot'].'"></td>';

							$type .='<td><label>'.date('d M Y', strtotime($inventory['expire_date'])).'</label><input type="hidden" id="expire_date_'.$i.'" name="identitas_pulang_'.$item_id.'_'.$index.'['.$i.'][expire_date]" value="'.$inventory['expire_date'].'"></td>';
					

							$type .='<td>'.$inventory['jumlah'].'<input type="number" class="form-control text-right stock_item hidden" id="identitas_stock_'.$i.'" name="identitas_pulang_'.$item_id.'_'.$index.'['.$i.'][stock]" min="1" value="'.$inventory['jumlah'].'">';


							$type .='<td><input type="number" class="form-control jumlah" name="identitas_pulang_'.$item_id.'_'.$index.'['.$i.'][jumlah_identitas]" min="0" value="0" max="'.$inventory['jumlah'].'" data-konversi="'.$konversi.'"></td></tr>';
				...
				?>
		                ...
		                <table class="table table-striped table-bordered table-hover" id="table_identitas">
		                    ...
		                    <tbody id="content_identitas_pulang">
		                        <?php
...
function handleCheckHtml(){
	...
	html_content = $('div#identitas_pulang_row', $row).html();
	if(html_content != ''){
		$('tbody#content_identitas_pulang').html($('div#identitas_pulang_row', $row).html());
		calculateTotal();
		$table = $('#table_identitas');
		$inputJumlah = $('input.jumlah', $table);
		$inputJumlah.on('change keyup', function() {
...
function handleConfirmSave() {
	$('a#confirm_save_modal').click(function(){
		...
		if(jumlah_isi <= jumlah_pesan){
			$('div#identitas_pulang_row', $row).html($('tbody#content_identitas_pulang').html());
