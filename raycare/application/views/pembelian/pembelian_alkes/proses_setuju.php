<?php
	...
	if($form_data_detail != '')
	{
		...
		$grand_total = 0;
		$hidden_btn = '';
		if($form_data[0]['is_single'] == 1) $hidden_btn = 'hidden';
		foreach ($form_data_detail as $detail) 
		{
			...
			$btn_jumlah_db        = '<a class="btn blue-chambray add-jumlah '.$hidden_btn.'" href="'.base_url().'pembelian/pembelian_alkes/edit_jumlah_edit/item_row_'.$i.'/'.$detail['id'].'" data-original-title="Tambah Jumlah" title="'.translate('Tambah Jumlah', $this->session->userdata('language')).'" data-toggle="modal" data-target="#popup_modal_jumlah" ><i class="fa fa-truck"></i></a>';
			...
			$item_cols_db = array(// style="width:156px;
				...
				'item_total'         => '<div class="text-right"><label class="control-label" name="items['.$i.'][item_sub_total]">'.formatrupiah(($detail['harga_beli'] - (($detail['diskon']/100)*$detail['harga_beli'])) * $detail['jumlah_disetujui']).'</label>'.form_input($attrs_item_total_db).'</div>',
			);
...
<div class="portlet light">
	<div class="portlet-title">
		<div class="caption">
			...
			<span class="caption font-blue-sharp bold uppercase"><?=translate("Verifikasi Pembelian", $this->session->userdata("language"))?></span>
		</div>
		<div class="actions">
			...
			<a id="confirm_save" class="btn btn-primary btn-circle" href="#" data-confirm="<?=$msg?>" data-toggle="modal"><i class="fa fa-check"></i> <?=translate("Verifikasi", $this->session->userdata("language"))?></a>
	...
	<div class="note note-danger note-bordered">
		<p>
			 <b>NOTE :</b> Jumlah pesan dan subtotal pesan akan mengikuti jumlah yang sudah disetujui.
		</p>
	</div>
									...
									<div class="tab-pane active" id="tab_supplier">
										<div class="form-body">
											...
											<div class="form-group hidden">
												<label class="col-md-12"><?=translate("Supplier ID", $this->session->userdata("language"))?> :</label>
												<div class="col-md-12">
													<?php
														$supplier_id = array(
															"id"			=> "supplier_id",
										...
										<div class="form-body">
											...
											<div class="form-group">
												...
												<?php 
													...
													if($form_data['is_single_kirim'] == 1){
														$check_ya = 'Ya';
														$hidden = '';
													}if($form_data['is_single_kirim'] == 0){
														$check_ya = 'Tidak';
														$hidden = 'hidden';
													}
												?>
											...
											<div class="form-group hidden">
												<label class="col-md-12"><?=translate('Master Pembayaran', $this->session->userdata('language'))?> :</label>
												<input type="hidden" name="master_tipe_bayar" id="master_tipe_bayar" value="<?=$tipe_bayar[0]['id_master_bayar']?>">
									...
									<?php
										if(count($data_penawaran))
										{
											...
											foreach ($data_penawaran as $penawaran)
											{
												?>
												<tr>
													<td><a target="_blank" href="<?=base_url()?>assets/mb/pages/pembelian/pembelian_alkes/doc/penawaran/<?=$pk_value?>/<?=$penawaran['id']?>/<?=$penawaran['url']?>"><?=($penawaran['nomor_penawaran'] != '')?$penawaran['nomor_penawaran']:$penawaran['url']?></a></td>
												</tr>
												<?php
											}
										}else{
							...
							<table class="table table-striped table-bordered table-hover" id="table_detail_pembelian">
								...
								<tfoot>
									...
									<tr>
										...
										<td >
											<a class="btn blue-chambray add-biaya" title="<?=translate('Tambah Biaya', $this->session->userdata('language'))?>" href="<?=base_url()?>pembelian/pembelian_alkes/edit_biaya/<?=$pk_value?>" data-toggle="modal" data-target="#modal_biaya">
									...
									<tr>
										...
										<td class="text-right bold"><?=formatrupiah($grand_total_non_biaya)?>
												...
												<input type="hidden" class="form-control text-right" value="<?=$grand_total_non_biaya?>" id="grand_total_biaya_hidden" name="grand_total_biaya_hidden">
												<input class="form-control text-right hidden" readonly value="<?=formattanparupiah($grand_total_non_biaya)?>" id="grand_total_biaya" name="grand_total_biaya">
										</td>
									</tr>
									<tr>
