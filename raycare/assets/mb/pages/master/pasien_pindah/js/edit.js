...
mb.app.pasien_pindah.edit = mb.app.pasien_pindah.edit || {};
(function(o){

    var 
        baseAppUrl             = '',
        $form_pasien_pindah = $('#form_pasien_pindah');
    ...
    var handleValidation = function() {
        var error1   = $('.alert-danger', $form_pasien_pindah);
        var success1 = $('.alert-success', $form_pasien_pindah);

        $form_pasien_pindah.validate({
    ...
    var handlePilihPasien = function(){
        $('#table_pilih_pasien').dataTable({
            ...
            'ajax'                  : {
                'url' : mb.baseUrl() + 'master/pasien_pindah/listing_pilih_pasien',
                'type' : 'POST',
            },          
            'pageLength'            : 10,
    ...
    var handleConfirmSave = function(){
        $('a#confirm_save', $form_pasien_pindah).click(function() {
            if (! $form_pasien_pindah.valid()) return;
            ...
            bootbox.confirm(msg, function(result) {
                ...
                if (result==true) {
                    i = parseInt(i) + 1;
                    $('a#confirm_save', $form_pasien_pindah).attr('disabled','disabled');
                    if(i === 1)
                    {
                      $('#save').click();
                    }
    ...
    var searchPasienByNomorAndFill = function(NomorPasien)
    {
        $.ajax({
            type     : 'POST',
            url      : mb.baseUrl() + 'master/pasien_pindah/search_pasien_by_nomor',
    ...
    var handleDatePickers = function () {
        if (jQuery().datepicker) {
            $('.date', $form_pasien_pindah).datepicker({
                rtl: Metronic.isRTL(),
                format : 'dd-M-yyyy',
                autoclose: true
            });
 ...
 }(mb.app.pasien_pindah.edit));


// initialize  mb.app.home.table
$(function(){
    mb.app.pasien_pindah.edit.init();
});