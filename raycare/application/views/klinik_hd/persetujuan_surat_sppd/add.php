...
<div class="portlet light">
	<div class="portlet-title">
		<div class="caption">
			...
			<span class="caption-subject font-blue-sharp bold uppercase"><?=translate("Persetujuan Surat Pengantar HD 3x", $this->session->userdata("language"))?></span>
		...
		<div class="actions">
            ...
            <?php $msg = translate("Apakah anda yakin akan menyetujui surat dokter sppd ini?",$this->session->userdata("language"));?>
				...
				<a id="confirm_save" class="btn btn-circle btn-primary" href="#" data-confirm="<?=$msg?>" data-toggle="modal">
					...
					<?=translate("Setujui", $this->session->userdata("language"))?>
				</a>
									...
									if(count($form_data_gambar)){
										...
										foreach ($form_data_gambar as $gambar) {
											...
											if($gambar['url'] != '')
										    {
										        ...
										        // die(dump(FCPATH.config_item('site_img_sppd_dir').$gambar['surat_dokter_sppd_id'].'/'.$gambar['url']));
										        if (file_exists(FCPATH.config_item('site_img_sppd_dir').$gambar['surat_dokter_sppd_id'].'/'.$gambar['url']) && is_file(FCPATH.config_item('site_img_sppd_dir').$gambar['surat_dokter_sppd_id'].'/'.$gambar['url'])) 
										        {
										            $image_user = base_url().config_item('site_img_sppd_dir').$gambar['surat_dokter_sppd_id'].'/'.$gambar['url'];
										        	$file_img = $gambar['url'];
										        }
										        else
										        {
										            $image_user = base_url().config_item('site_img_sppd_dir').'global/global.png';
										            $file_img = 'global.png';
										        }
										    }
										    else
