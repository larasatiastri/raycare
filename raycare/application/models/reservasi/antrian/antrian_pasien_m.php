...
class Antrian_pasien_m extends MY_Model {
	...
	{
		$date = date('Y-m-d');
		$format = "SELECT * FROM `antrian` WHERE posisi_loket = $loket_id AND date(created_date) = '$date' ORDER BY no_urut ASC;";	
		return $this->db->query($format);
	}
	...
	public function get_data_loket_panggil($loket_id)
	{
		...
		$format = "SELECT * FROM `antrian` WHERE posisi_loket = $loket_id AND status = 0 AND is_panggil IS NULL AND date(created_date) = '$date' OR posisi_loket = $loket_id AND status = 0 AND is_panggil = 1 AND date(created_date) = '$date' OR posisi_loket = $loket_id AND status = 0 AND is_panggil = 0 AND date(created_date) = '$date' ORDER BY no_urut ASC;";	
		return $this->db->query($format);
	}
	...
	{
		$date = date('Y-m-d');
		$format = "SELECT * FROM `antrian` WHERE posisi_loket = $loket_id AND no_urut >= $no_urut AND date(created_date) = '$date' ORDER BY no_urut ASC;";	
		return $this->db->query($format);
	}
