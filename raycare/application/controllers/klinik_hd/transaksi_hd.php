...
class Transaksi_perawat extends MY_Controller {
    ...
    private $menu_tree = array(0, 5);       // untuk keperluan item menu dengan class 'open', 'selected'
    ...
    public function index()
    {
        ...
        // die(dump( $assets['css'] ));
        $data = array(
            'title'          => config_item('site_name').' | '.translate('Master Spesialis', $this->session->userdata('language')), 
            ...
            'js_files'       => $assets['js'],
            'content_view'   => 'master/spesialis/index',
            );
    ...
    {
        $assets = array();
        $assets_config = 'assets/master/spesialis/add';
        $this->config->load($assets_config, true);

        $assets = $this->config->item('assets', $assets_config);

        $data = array(
            'title'          => config_item('site_name'). ' | '.translate("Tambah Spesialis", $this->session->userdata("language")), 
            'header'         => translate("Tambah Spesialis", $this->session->userdata("language")), 
            'header_info'    => config_item('site_name'), 
            'breadcrumb'     => TRUE,
            // 'menus'          => $this->menus,
            // 'menu_tree'      => $this->menu_tree,
            'css_files'      => $assets['css'],
            'js_files'       => $assets['js'],
            'content_view'   => 'master/spesialis/add',
            'flag'           => 'add',
        );
    ...
    public function edit($id)
    {
        ...
        $assets = array();
        $config = 'assets/master/spesialis/edit';
        $this->config->load($config, true);
        ...
        $form_data = $this->spesialis_m->get($id);

        $data = array(
            'title'          => config_item('site_name').' | '. translate("Edit Spesialis", $this->session->userdata("language")), 
            'header'         => translate("Edit Spesialis", $this->session->userdata("language")), 
            'header_info'    => config_item('site_name'), 
            'breadcrumb'     => TRUE,
            // 'menus'          => $this->menus,
            // 'menu_tree'      => $this->menu_tree,
            'css_files'      => $assets['css'],
            'js_files'       => $assets['js'],
            'content_view'   => 'master/spesialis/edit',
    ...
    {        
        $result = $this->spesialis_m->get_datatable();
        ...
        foreach($records->result_array() as $row)
        {
                ...
                if ($row['is_active'] == '1')
                {
                    $action = '<a title="'.translate('Edit', $this->session->userdata('language')).'" href="'.base_url().'master/spesialis/edit/'.$row['id'].'" class="btn btn-xs blue-chambray"><i class="fa fa-edit"></i></a>
                               <a title="'.translate('Delete', $this->session->userdata('language')).'" data-confirm="'.translate('Anda yakin akan menghapus data spesialis ini?', $this->session->userdata('language')).'" name="delete[]" data-action="delete" data-id="'.$row['id'].'" class="btn btn-xs red"><i class="fa fa-times"></i> </a>';
                }else{
                    $action = '<a title="'.translate('Restore', $this->session->userdata('language')).'" data-confirm="'.translate('Anda yakin akan mengembalikan data spesialis ini?', $this->session->userdata('language')).'" name="restore[]" data-action="restore" data-id="'.$row['id'].'" class="btn btn-xs yellow"><i class="fa fa-undo"></i> </a>';
                }
