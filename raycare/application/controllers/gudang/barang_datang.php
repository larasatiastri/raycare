...
class Barang_datang extends MY_Controller {

    // protected $menu_parent_id = 2;           // deprecated, pake $menu_tree
    protected $menu_id = '5130c0eb8520ffd81e8704f986aff938';                  // untuk check bit_access
    ...
    {
        $assets = array();
        $config = 'assets/gudang/barang_datang/index';
        ...
        $data = array(
            ...
            'js_files'       => $assets['js'],
            'content_view'   => 'gudang/barang_datang/index',
    ...
    {
        $assets = array();
        $config = 'assets/gudang/barang_datang/add_terima_barang';
        $this->config->load($config, true);
        $assets = $this->config->item('assets', $config);
        ...
        // Load the view
        $this->load->view('gudang/barang_datang/add_terima_barang', $data);
    }
    ...
    public function proses($gudang_id, $supplier_id, $data)
    {
        ...
        {
            $assets = array();
            $config = 'assets/gudang/barang_datang/proses';
            $this->config->load($config, true);
            ...
            $data = array(
                ...
                'css_files'    => $assets['css'],
                'js_files'     => $assets['js'],
                'content_view' => 'gudang/barang_datang/proses',                  
                'draft_pmb_id' => $data,                   
                'supplier_id'  => $supplier_id,                   
                'gudang_id'    => $gudang_id,                   
            );
    ...
    public function proses_tambah_po($data, $supplier_id, $gudang_id)
    {
        ...
        // $id || redirect(base_Url());
            $assets = array();
            $config = 'assets/gudang/barang_datang/proses_draft';
            $this->config->load($config, true);
            ...
            $data = array(
                ...
                'css_files'    => $assets['css'],
                'js_files'     => $assets['js'],
                'content_view' => 'gudang/barang_datang/proses_draft',                  
                'draft_pmb_id' => $data,                   
                'supplier_id'  => $supplier_id,                   
                'gudang_id'    => $gudang_id,                   
            );
    ...
    public function view($id, $supplier_id, $gudang_id, $flag)
    {
        ...
        $assets = array();
        $config = 'assets/gudang/barang_datang/view';
        $this->config->load($config, true);
        ...
        $data = array(
            ...
            'css_files'    => $assets['css'],
            'js_files'     => $assets['js'],
            'content_view' => 'gudang/barang_datang/view',
    ...
    {        
        ...
        {
            $action = '<a title="'.translate('View', $this->session->userdata('language')).'" href="'.base_url().'gudang/barang_datang/view/'.$row['id'].'/'.$row['supplier_id'].'/'.$row['gudang_id'].'/'.$flag.'" name="view[]" class="btn grey-cascade pecah-item" data-item="'.htmlentities(json_encode($row)).'"><i class="fa fa-search"></i></a>';
    ...
    {        
        $result = $this->pembelian_detail_tanggal_kirim_m->get_datatable_datang(3);
        // die_dump($this->db->last_query());
        $output = array(
            'draw'                 => intval($this->input->post('draw', true)),
            'iTotalRecords'        => $result->total_records,
        ...
        foreach($records->result_array() as $row)
        {
            $action = '<a href="'.base_url().'gudang/barang_datang/select_po/'.$row['po_id'].'/'.$row['supplier_id'].'/WH-11-2017-001" title="'.translate('Pilih', $this->session->userdata('language')).'" class="btn btn-primary"><i class="fa fa-check"></i></a>';
    ...
    {        
        ...
        foreach($records->result_array() as $row)
        {
            ...
            // {
                $action = '<a title="'.translate('View', $this->session->userdata('language')).'" href="'.base_url().'gudang/barang_datang/view/'.$row['id'].'/'.$row['supplier_id'].'/'.$row['gudang_id'].'/'.$flag.'" name="view[]" class="btn grey-cascade pecah-item" data-item="'.htmlentities(json_encode($row)).'"><i class="fa fa-search"></i></a>
                       <a title="'.translate('Edit', $this->session->userdata('language')).'" href="'.base_url().'gudang/barang_datang/proses_tambah_po/'.$row['id'].'/'.$row['supplier_id'].'/'.$row['gudang_id'].'" name="view[]" class="btn blue-chambray pecah-item" data-item="'.htmlentities(json_encode($row)).'"><i class="fa fa-edit"></i></a>
                       <a title="'.translate('Tambah PO', $this->session->userdata('language')).'" href="'.base_url().'gudang/barang_datang/modal_add_po/'.$row['id'].'/'.$row['supplier_id'].'/'.$row['gudang_id'].'" name="view[]" data-toggle="modal" data-target="#popup_modal_add_po" class="btn btn-primary pecah-item" data-item="'.htmlentities(json_encode($row)).'"><i class="fa fa-plus"></i></a>
    ...
    public function save()
    {
        ...
        {
            $path_dokumen = '../cloud/'.config_item('site_dir').'pages/gudang/barang_datang/images/'.str_replace(' ', '_', $id_pmb);
            ...
            copy(config_item('base_dir').config_item('user_img_temp_dir_new').$temp_filename, $_SERVER['DOCUMENT_ROOT'] .config_item('site_img_faktur_pmb').$real_file);
        }
    ...
    public function modal_add()
    {
        ...
        $this->load->view('gudang/barang_datang/modal/modal_add.php');
    ...
    public function modal_add_po($id_draft, $supplier_id, $gudang_id)
    {
        ...
        $this->load->view('gudang/barang_datang/modal/modal_add_po.php', $data);
    ...
    public function modal_jumlah_pesan($supplier_id, $item_id, $item_satuan_id, $po_id)
    {
        ...
        $this->load->view('gudang/barang_datang/modal/modal_jumlah_pesan.php', $data);
    ...
    public function modal_jumlah_pesan_supplier_lain($supplier_id, $item_id, $item_satuan_id)
    {
        ...
        $this->load->view('gudang/barang_datang/modal/modal_jumlah_pesan_supplier_lain.php', $data);
    ...
    public function modal_jumlah_terima_supplier_lain($supplier_id, $item_id, $item_satuan_id)
    {
        ...
        $this->load->view('gudang/barang_datang/modal/modal_jumlah_terima_supplier_lain.php', $data);
    ...
    public function modal_identitas($item_id, $item_satuan_id, $row)
    {
        ...
        $this->load->view('gudang/barang_datang/modal/modal_identitas.php', $data);
    ...
    public function modal_identitas_view($pmb_id, $item_id, $item_satuan_id, $row)
    {
        ...
        $this->load->view('gudang/barang_datang/modal/modal_identitas_view.php', $data);
    ...
    public function modal_pemisahan_item($item_id, $item_satuan_id, $jumlah, $row)
    {
        ...
        $this->load->view('gudang/barang_datang/modal/modal_pemisahan_item.php', $data);
    ...
    public function select_po($po_id, $supplier_id, $gudang_id)
    {
        ...
        redirect("gudang/barang_datang/proses/$gudang_id/$supplier_id/$draft_pmb_id_last");
        // die_dump($url);
    }
