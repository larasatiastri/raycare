...
(function(o){
     ...
     var 
        ...
        $tableItemSearch        = $('#table_add_account', $form),
        $tableItemSearchTitipan = $('#table_add_account_titipan', $form),
    ...
    var handleDataTableItems = function(){
        ...
        oTable = $tableItemSearch.dataTable({
            ...
            'ajax'                  : {
                'url' : baseAppUrl + 'listing_paket_item/' + paket_id,
                'type' : 'POST',
            },          
            'pageLength'            : 10,
        ...
        $tableItemSearch.on('draw.dt', function (){
            ...
            var subtotal = $('input#subtotal', this).val();
            // alert(subtotal);
            $('label#sub_total').text(mb.formatRp(parseInt(subtotal)));
            

            });

            var subtotal = parseInt($('input#subtotal', $tableItemSearch).val());
            if(!isNaN(subtotal))
            {
                 $('#sub_total').text(subtotal_titipan);    
            } else {
                 $('#sub_total').text(0);
            }
    ...
    var handleDataTableItemsTitipan = function(){
        ...
        oTable2 = $tableItemSearchTitipan.dataTable({
            ...
            'language'              : mb.DTLanguage(),
            'ajax'                  : {
                'url' : baseAppUrl + 'listing_paket_tindakan/' + paket_id,
                'type' : 'POST',
            },          
            'pageLength'            : 10,
            'lengthMenu'            : [[10, 25, 50, 100], [10, 25, 50, 100]],
        ...
        $tableItemSearchTitipan.on('draw.dt', function (){
            ...
            var subtotal_titipan = $('input#subtotal_tindakan', this).val();
            // alert(subtotal_titipan);
            $('label#sub_total_tindakan').text(mb.formatRp(parseInt(subtotal_titipan)));
            

            });

            var subtotal_titipan = parseInt($('input#subtotal_tindakan', $tableItemSearchTitipan).val());
            if(!isNaN(subtotal_titipan))
            {
                 $('#sub_total_tindakan').text(subtotal_titipan);    
            } else {
                 $('#sub_total_tindakan').text(0);
            }
