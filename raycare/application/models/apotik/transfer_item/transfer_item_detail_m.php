...
class Transfer_item_detail_m extends MY_Model {
	...
	public function get_id_transfer_detail()
	{
		$format = "SELECT MAX(SUBSTRING(`id`,13,4)) AS max_id FROM `transfer_item_detail` WHERE SUBSTRING(`id`,5,7) = DATE_FORMAT(NOW(), '%m-%Y')";	
		return $this->db->query($format);
	}
	...
	public function get_data($id)
	{
		$sql = "SELECT
				...
				transfer_item.dari_gudang_id AS dari_gudang_id,
				transfer_item_detail.bn_sn_lot AS bn_sn_lot,
				transfer_item_detail.expire_date AS expire_date,
				transfer_item_detail.jumlah AS jumlah_kirim
				FROM
				transfer_item_detail
