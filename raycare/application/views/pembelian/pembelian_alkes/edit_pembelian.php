<?php
	...
	if($form_data_detail != '')
	{
		...
		foreach ($form_data_detail as $detail) 
		{
			...
			$btn_jumlah_db        = '<a class="btn blue-chambray add-jumlah '.$hidden.'" href="'.base_url().'pembelian/pembelian_alkes/edit_jumlah_edit/item_row_'.$i.'/'.$detail['id'].'" data-original-title="Tambah Jumlah" title="'.translate('Tambah Jumlah', $this->session->userdata('language')).'" data-toggle="modal" data-target="#popup_modal_jumlah" ><i class="fa fa-truck"></i></a>';
			...
			$item_cols_db = array(// style="width:156px;
				'item_kode'          => '<label class="control-label hidden" name="items['.$i.'][item_kode]">'.$detail['kode'].'</label><div class="input-group">'.form_input($attrs_item_kode_db).form_input($attrs_item_id_db).form_input($attrs_id_db).'<span class="input-group-btn">'.$btn_search.'</span></div>',
