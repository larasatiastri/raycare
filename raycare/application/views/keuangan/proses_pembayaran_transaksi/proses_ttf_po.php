<?php
	...
	if($form_data_detail != '')
	{
		...
		foreach ($form_data_detail as $detail) 
		{
			...
			$item_cols_db = array(// style="width:156px;
				...
				'item_jml_terima'     => '<div class="text-center"><label class="control-label" name="items['.$i.'][jumlah_diterima]"><a class="jumlah_terima" data-toggle="modal" data-target="#popup_modal_jumlah_terima" href="'.base_url().'pembelian/history/modal_jumlah_terima/'.$detail['id'].'/'.$detail['item_id'].'/'.$detail['item_satuan_id'].'">'.$jml_diterima.'</a> </label></div>',
	...
	$pkp = ($form_data[0]['is_pkp'] == 1)?'PKP':'Non PKP';
?>
										...
										<table class="table table-striped table-bordered table-hover" id="table_detail_pembelian">
											<thead>
												<tr>
													...
													<th class="text-center" width="5%"><?=translate("JmlPesan", $this->session->userdata("language"))?> </th>
													<th class="text-center" width="5%"><?=translate("JmlTerima", $this->session->userdata("language"))?> </th>
		...
		<div class="row">
			<div class="col-md-9">
				<div class="portlet light bordered">
					<div class="portlet-title">
						<div class="caption">
							<span class="caption-subject"><?=translate("Daftar Dokumen", $this->session->userdata("language"))?>&nbsp;<b><?=$pkp?></b></span>
						</div>
					</div>
					<div class="portlet-body">
					    <div class="form-body">
					    	<div class="table-scrollable">
								<?php
									...
									if(count($data_invoice) != 0){
										foreach ($data_invoice as $key => $bon) {
											...
											<td class="text-left"><b>No. Invoice :</b><br>'.$bon['no_invoice'].'<br><b>Tgl Invoice :</b><br>'.date('d M Y', strtotime($bon['tgl_invoice'])).'<br><b>Total Invoice :</b><br><input type="hidden" name="invoice['.$i.'][total_invoice]" id="total_invoice_'.$i.'" value="'.$bon['total_invoice'].'">'.formatrupiah($bon['total_invoice']).'<br><b>Lampiran :<b><br><a class="fancybox-button" title="'.$bon['url'].'" href="'.config_item('site_img_bayar').$data_bayar['id'].'/'.$bon['url'].'" data-rel="fancybox-button"><img src="'.config_item('site_img_bayar').$data_bayar['id'].'/'.$bon['url'].'" alt="Smiley face" class="img-responsive" ></a></td>
											<td class="text-left"><b>No. PO :</b><br>'.$bon['no_pembelian'].'<br><b>Tgl PO :</b><br>'.date('d M Y', strtotime($bon['tanggal_pesan'])).'<br><b>Total PO :</b><br>'.formatrupiah($grand_total_pph + $form_data[0]['biaya_tambahan']).'</td>
											<td class="text-left"><b>No. DO :</b><br>'.$bon['no_surat_jalan'].'<br><b>Tgl DO :</b><br>'.date('d M Y', strtotime($bon['tanggal_do'])).'<br><b>Lampiran :<b><br><a class="fancybox-button" title="'.$bon['url_surat_jalan'].'" href="'.config_item('site_img_faktur_pmb_farmasi').$bon['pmb_id'].'/'.$bon['url_surat_jalan'].'" data-rel="fancybox-button"><img src="'.config_item('site_img_faktur_pmb_farmasi').$bon['pmb_id'].'/'.$bon['url_surat_jalan'].'" alt="Dokumen belum diupload" class="img-responsive" ></a></td>
											';

											if($form_data[0]['is_pkp'] == 1){
												$form_upload_bon .= '<td><b>No. Faktur Pajak :</b> <br>'.$bon['no_faktur_pajak'].'<br><b>DPP :</b> <br>'.formatrupiah($bon['dpp']).'<br><b>PPN Nominal :</b> <br>'.formatrupiah($bon['ppn_nominal']).'<br><b>Tgl Faktur Pajak :</b><br>'.date('d M Y', strtotime($bon['tgl_invoice'])).'<br><b>Lampiran :</b><a class="fancybox-button" title="'.$bon['url_faktur_pajak'].'" href="'.config_item('site_img_bayar').$data_bayar['id'].'/'.$bon['url_faktur_pajak'].'" data-rel="fancybox-button"><img src="'.config_item('site_img_bayar').$data_bayar['id'].'/'.$bon['url_faktur_pajak'].'" alt="Smiley face" class="img-responsive" ></a></td>';
											}
								...
								<table class="table table-hover" id="table_invoice">
									...
									<tfoot>
										...
										<tr>
											<th colspan="13" class="text-left" >Terbilang : # <?=terbilang($total_invoice)?> Rupiah #</th>
										</tr>
									</tfoot>
								</table>    
							</div>
