...
(function(o){
    ...
    var 
        ...
        itemCounter                     = parseInt($('input#jumlah_item').val()),
        penawaranCounter                = 0
        ;
    ...
    {    
        ...
        var is_flexible = $('input[name="is_harga_flexible"]').val();
        ...
        if(is_flexible == 1){
            $('input[name$="[item_harga]"]',  $tableDetailPembelian).removeAttr('readonly');
        }else if(is_flexible == 0){
            $('input[name$="[item_harga]"]',  $tableDetailPembelian).attr('readonly','readonly');
        }
    ...
    var calculateTotal = function()
    {
        ...
        $.each($rows, function(idx, row)
        {
            ...
            if (itemCode != '' ){
                ...
                if(isPPH == 1){
                    ...
                    pph23 = pph23 + (parseFloat($('input#pph23').val())/100) * totalCost;
        ...
        grandTotalAll =  grandTotalAT - parseFloat($('input#total_pph_23').val());       
    ...
    var calculateTotalDiskon = function()
    {
        ...
        grandTotalAll =  grandTotalAll - parseFloat($('input#total_pph_23').val());
        $('input#grand_total').val(mb.formatTanpaRp(grandTotalAll));
        $('input#grand_total_hidden').val(grandTotalAll);
        dp = parseFloat($('input#dp').val());
                ...
                if(found == false)
                {
                    ...
                    ({ 
                        ...
                        { 
                            ...
                            $itemHarga.val(parseFloat(harga));
                            $itemHargaLama.val(parseFloat(harga));
                    ...
                    if ($itemKode.val() == "") {
                        ...
                        $isPPH.val($(this).data('item').is_pph);
                        $isPPH.attr('value',$(this).data('item').is_pph);
                        ...
                        $isPPH.val($(this).data('item').is_pph);
                        $isPPH.attr('value',$(this).data('item').is_pph);
                        // $itemHarga.val(parseFloat($(this).data('item').harga));
                        $SearchSatuan.val(name);
                        $SearchSatuanId.val(primary);
                        $itemLabelKode.text($(this).data('item').item_kode);
                        $itemLabelNama.text($(this).data('item').item_nama);
    ...
    {   
        ...
        $('select.satuan').on('change', function(){
            ...
            $itemLabelHarga.text(mb.formatRp(parseFloat($harga)));
        ...
        //     $max = parseFloat($total_stock / $konversi);
    ...
    var handlePilihSupplierSelect = function($btn){
        $btn.on('click', function(e){
            var 
                ...
                $IsHargaFlex    = $('input[name="is_harga_flexible"]'),
                $IsPkp          = $('input[name="is_pkp"]'),               
                $IsPPH          = $('input[name="is_pph_supp"]'),               
            ...
            var is_flexible = parseInt($(this).data('item').is_harga_flexible);
            var is_pkp = parseInt($(this).data('item').is_pkp);
            var is_pph = parseInt($(this).data('item').is_pph);

            $IdSupplier.val($(this).data('item').id);
            $IsHargaFlex.val($(this).data('item').is_harga_flexible);
            $IsHargaFlex.attr('value', $(this).data('item').is_harga_flexible);
            $IsPkp.val($(this).data('item').is_pkp);
            $IsPkp.attr('value', $(this).data('item').is_pkp);
            $IsPPH.val($(this).data('item').is_pph);
            $IsPPH.attr('value', $(this).data('item').is_pph);
            ...
            if(is_pkp == 1){
                $('input[name="pph"]',  $form).attr('value','10');
                $('input[name="pph"]',  $form).val('10');
            }else{
                $('input[name="pph"]',  $form).attr('value','0');
                $('input[name="pph"]',  $form).val('0');
            }

            if(is_pkp == 1 && is_pph == 1){
                $('input[name="pph23"]',  $form).attr('value','2');
                $('input[name="pph23"]',  $form).val('2');
            }else if(is_pkp == null && is_pph == 1){
                $('input[name="pph23"]',  $form).attr('value','4');
                $('input[name="pph23"]',  $form).val('4');
            }else{
                $('input[name="pph23"]',  $form).attr('value','0');
                $('input[name="pph23"]',  $form).val('0');
