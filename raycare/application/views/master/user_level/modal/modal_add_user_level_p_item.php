...
function handleDataTableUserLevel() {
    ...
    $tableUserLevel.on('draw.dt', function (){
		...
		var row = $('input#numRow').val();
		// alert(row);
		$('a#select-p-item', this).click(function() 
		{
			$('input#user_level_name_item'+ row).val($(this).data('item').nama);
			$('input#user_level_id_item'+ row).val($(this).data('item').id);
			$('label#user_level_lblname_item'+ row).text($(this).data('item').nama);
			$('a#addRowItem').click();
