...
(function(o){
     ...
     var 
        ...
        $tableItemTerdaftar     = $('#table_item_search'),
       ...
       var initForm = function(){
        ...
        var 
            ...
            $btnDeletesDb              = $('.del-item-db', $tableAddAccount),
    ...
    var handleDataTableItems = function(){
       ...
       // pk_value = $('input#pk_value').val();
       // user_level_id = $('input#user_level_id').val();
       gudang_id = $('select#gudang_dari').val();
       ...
       // $( '#gudang_ke').on( 'change', function () {
       //      // alert($(this).val());
            
       //      oTable.api().ajax.url(baseAppUrl + 'listing_item/' + $(this).val()).load();
       //      // alert($(this).val());
       //  });
    ...
    var handleAccountSelect = function($btn){
        $btn.on('click', function(e){
            ...
            // console.log(itemTarget);
                ...
                $itemStockEl    = $('label[name$="[stock]"]', $row);
                $itemStockIn    = $('input[name$="[stock]"]', $row);
                $itemStockTersediaEl    = $('label[name$="[stock_tersedia]"]', $row);
                $itemStockTersediaIn    = $('input[name$="[stock_tersedia]"]', $row);
                $itemSatuanEl    = $('select[name$="[satuan]"]', $row);
                $btnStock       = $('a.stock', $row);
            ...
            // alert(found);
                ...
                if(found == false)
                {
                    ...
                    $itemStockEl.text($stock);
                    $itemStockIn.val($stock);

                    if($(this).data('item')['stock_tersedia'] == null)
                    {
                        $stock_tersedia = 0;

                    } else {

                        $stock_tersedia = $(this).data('item')['stock_tersedia'];
                    }

                    $itemStockTersediaEl.text($stock_tersedia);
                    $itemStockTersediaIn.val($stock_tersedia);
                    // $btnIdentitas.attr('href', baseAppUrl + 'modal_identitas/' + $(this).data('item').item_id + '/' + $(this).data('item').item_satuan_id + '/' + rowId);
    ...
    var semua_stock = function($btn){

        var 
            rowId       = $btn.closest('tr').prop('id'),
            $row        = $('#' + rowId, $tableAddAccount),
            $itemJumlahKirim = $('input[name$="[jumlah_kirim]"]', $row);

        $btn.on('click', function(e){

            $itemStockIn    = $('input[name$="[stock]"]', $row).val();
            // alert($itemStockIn);
            $itemJumlahKirim.val($itemStockIn);
    ...
    var handleBtnDeleteDB = function($btn){
        ...
        $btn.on('click', function(e){
            bootbox.confirm('Apakah anda yakin menghapus Item ini?', function(result){
                if (result==true) {
...
var handleValidation = function() {
    ...
    o.init = function(){
        baseAppUrl = mb.baseUrl() + 'gudang/request_item/';
        handleValidation();
        calculateTotalKeseluruhan();
        initForm();
        handleDatePickers();
