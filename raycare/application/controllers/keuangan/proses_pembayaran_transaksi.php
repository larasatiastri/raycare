...
class Proses_pembayaran_transaksi extends MY_Controller {

    // protected $menu_parent_id = 2;           // deprecated, pake $menu_tree
    protected $menu_id = '8923debb8b947fa27b46ed0ffc54118d';                  // untuk check bit_access
    ...
    {       
        ...
        $this->load->model('keuangan/rangkuman_cek_pembayaran_m');
    ...
    public function index()
    {
        ...
        $assets = array();
        $config = 'assets/keuangan/proses_pembayaran_transaksi/index';
        $this->config->load($config, true);
        $assets = $this->config->item('assets', $config);
        
        // die(dump( $assets['css'] ));
        $data = array(
            'title'          => config_item('site_name').' | '.translate('Pembayaran Keluar', $this->session->userdata('language')), 
            'header'         => translate('Pembayaran Keluar', $this->session->userdata('language')), 
            ...
            'js_files'       => $assets['js'],
            'content_view'   => 'keuangan/proses_pembayaran_transaksi/index',
            );
    ...
    public function history()
    {
        ...
        // die(dump( $assets['css'] ));
        $data = array(
            'title'          => config_item('site_name').' | '.translate('History Pembayaran Keluar', $this->session->userdata('language')), 
            'header'         => translate('History Pembayaran Keluar', $this->session->userdata('language')), 
            'header_info'    => config_item('site_name'), 
            'breadcrumb'     => true,
            'menus'          => $this->menus,
            'menu_tree'      => $this->menu_tree,
            'css_files'      => $assets['css'],
            'js_files'       => $assets['js'],
            'content_view'   => 'keuangan/proses_pembayaran_transaksi/history',
            );
    ...
    {       
        ...
        foreach($records->result_array() as $row)
        {
            ...
            if($row['tipe_transaksi'] == 1){
                ...
                {
                    $status = '<div class="text-center"><span class="label label-md label-primary">Proses Persetujuan</span></div>';
                    $action = '<a title="'.translate('View', $this->session->userdata('language')).'" class="btn grey-cascade" href="'.base_url().'pembelian/history/view/'.$row['transaksi_id'].'"><i class="fa fa-search"></i></a>';
                }
                else if($row['status'] == 3)
                {
                    $status = '<div class="text-center"><span class="label label-md label-warning">Proses Verif</span></div>';
                    $action = '<a title="'.translate('View', $this->session->userdata('language')).'" class="btn grey-cascade" href="'.base_url().'pembelian/history/view/'.$row['transaksi_id'].'"><i class="fa fa-search"></i></a>';

                }
                else if($row['status'] == 4)
                {
                    $status = '<div class="text-center"><span class="label label-md label-success">Upload Proforma Invoice</span></div>';
                     $action = '<a title="'.translate('View', $this->session->userdata('language')).'" class="btn grey-cascade" href="'.base_url().'pembelian/history/view/'.$row['transaksi_id'].'"><i class="fa fa-search"></i></a>';
                }
                ...
                {                    
                    $status = '<div class="text-center"><span class="label label-md label-danger">Konfirmasi Ditolak</span></div>';
                }
                else if($row['status'] == 9)
                {                    
                    $status = '<div class="text-center"><span class="label label-md label-danger">Verifikasi Proforma Invoice</span></div>';
                    
                    $action = '<a title="'.translate('Process', $this->session->userdata('language')).'" class="btn btn-primary" href="'.base_url().'keuangan/proses_pembayaran_transaksi/verifikasi_dokumen/'.$row['transaksi_id'].'"><i class="fa fa-check"></i></a>';
                }else if($row['status'] == 10)
                {                    
                    $status = '<div class="text-center"><span class="label label-md label-danger">Menunggu Diproses</span></div>';
                    
                    $action = '<a title="'.translate('Process', $this->session->userdata('language')).'" class="btn btn-primary" href="'.base_url().'keuangan/proses_pembayaran_transaksi/proses_po/'.$row['transaksi_id'].'"><i class="fa fa-check"></i></a>';
                }
                else if($row['status'] == 11)
                {                    
                    $status = '<div class="text-center"><span class="label label-md label-info">Pencairan</span></div>';
                    
                    $action = '<a title="'.translate('View', $this->session->userdata('language')).'" class="btn grey-cascade" href="'.base_url().'pembelian/history/view/'.$row['transaksi_id'].'"><i class="fa fa-search"></i></a>';
                }else if($row['status'] == 13)
                {                    
                    $status = '<div class="text-center"><span class="label label-md label-danger">Verifikasi Invoice</span></div>';
                    
                    $action = '<a title="'.translate('Process', $this->session->userdata('language')).'" class="btn btn-primary" href="'.base_url().'keuangan/proses_pembayaran_transaksi/verifikasi_dokumen_asli/'.$row['transaksi_id'].'"><i class="fa fa-check"></i></a>';
                }
                ...
                {
                    $status = '<div class="text-center"><span class="label label-md label-warning">Menunggu Persetujuan</span></div>';
                    $action .= '<a title="'.translate('View', $this->session->userdata('language')).'" href="'.base_url().'keuangan/proses_pembayaran_transaksi/view_kasbon/'.$row['transaksi_id'].'"  class="btn grey-cascade"><i class="fa fa-search"></i></a>';
                    ...
                    $action .= '<a title="'.translate('Proses', $this->session->userdata('language')).'" href="'.base_url().'keuangan/proses_pembayaran_transaksi/proses_persetujuan/'.$row['transaksi_id'].'"  class="btn btn-primary"><i class="fa fa-check"></i></a>';
                }if($row['status'] == 12){
                    ...
                    $action .= '<a title="'.translate('View', $this->session->userdata('language')).'" href="'.base_url().'keuangan/proses_pembayaran_transaksi/view_proses/'.$row['transaksi_id'].'"  class="btn grey-cascade"><i class="fa fa-search"></i></a>';
                    ...
                    $action .= '<a title="'.translate('Proses', $this->session->userdata('language')).'" href="'.base_url().'keuangan/proses_pembayaran_transaksi/view_kasbon/'.$row['transaksi_id'].'"  class="btn btn-primary"><i class="fa fa-check"></i></a>';
                }
                if($row['status'] == 21){
    ...
    {       
        ...
        foreach($records->result_array() as $row)
        {
            ...
            if($row['tipe_transaksi'] == 1){
                $tipe = 'Pembelian';
                if($row['status'] == 12 && $row['status_cancel'] == 0)
                {                    
                    $status = '<div class="text-left"><span class="label label-md label-success">Selesai</span></div>';
                    $action = '<a title="'.translate('Proses', $this->session->userdata('language')).'" class="btn default" href="'.base_url().'keuangan/proses_pembayaran_transaksi/view_proses_bayar_po/'.$row['id'].'/'.$row['transaksi_id'].'"><i class="fa fa-search"></i></a>';
                }if($row['status'] == 12 && $row['status_cancel'] == 1)
                {                    
                    $status = '<div class="text-left"><span class="label label-md label-danger">Dibatalkan</span></div>';
                    $action = '<a title="'.translate('Proses', $this->session->userdata('language')).'" class="btn default" href="'.base_url().'pembelian/history/view/'.$row['id'].'/'.$row['transaksi_id'].'"><i class="fa fa-search"></i></a>';
                }
            }if($row['tipe_transaksi'] == 2){
                $tipe = 'Kasbon';
                if($row['status'] == 5){
                    
                    $status = '<div class="text-left"><span class="label label-md label-success">Diproses</span></div>';
                    $action .= '<a title="'.translate('View', $this->session->userdata('language')).'" href="'.base_url().'keuangan/proses_pembayaran_transaksi/view_proses/'.$row['transaksi_id'].'"  class="btn default"><i class="fa fa-search"></i></a>';
                }
                ...
                {                    
                    $status = '<div class="text-left"><span class="label label-md label-success">Selesai</span></div>';
                    $action = '<a title="'.translate('View', $this->session->userdata('language')).'" class="btn default" href="'.base_url().'keuangan/proses_pembayaran_transaksi/view_proses_bayar_po_ttf/'.$row['id'].'/'.$row['transaksi_id'].'"><i class="fa fa-search"></i></a>';
                }
            }
    ...
    public function listing_supplier_bank($supplier_id)
    {
        $result = $this->supplier_bank_m->get_datatable($supplier_id);
        ...
        foreach($records->result_array() as $row)
        {
            $action = '<a title="'.translate('Select', $this->session->userdata('language')).'" data-item="'.htmlentities(json_encode($row)).'" class="btn btn-primary select-supplier-bank"><i class="fa fa-check"></i></a>';
    ...
    public function listing_pegawai_bank($pegawai_id)
    {
    ...
    public function verifikasi_dokumen($id)
    {
        $assets = array();
        $config = 'assets/keuangan/proses_pembayaran_transaksi/verifikasi_dokumen';
        $this->config->load($config, true);
        ...
        $data_biaya = $this->pembelian_biaya_m->get_data($id)->result_array();
        $data_bayar = $this->pembayaran_status_m->get_by(array('transaksi_id' => $id, 'tipe_transaksi' => 1), true);
        ...
        $data = array(
            ...
            'css_files'           => $assets['css'],
            'js_files'            => $assets['js'],
            'content_view'        => 'keuangan/proses_pembayaran_transaksi/verifikasi_dokumen',
    ...
    public function verifikasi_dokumen_asli($id)
    {
        $assets = array();
        $config = 'assets/keuangan/proses_pembayaran_transaksi/verifikasi_dokumen';
        $this->config->load($config, true);
        ...
        $data = array(
            ...
            'css_files'           => $assets['css'],
            'js_files'            => $assets['js'],
            'content_view'        => 'keuangan/proses_pembayaran_transaksi/verifikasi_dokumen_asli',
            'form_data'           => object_to_array($form_data),
            'data_biaya'          => object_to_array($data_biaya),
            'data_invoice'        => object_to_array($data_invoice),
            'data_bayar'          => object_to_array($data_bayar),
    ...
    {
        $assets = array();
        $config = 'assets/keuangan/proses_pembayaran_transaksi/proses_po';
        $this->config->load($config, true);
        ...
        $data = array(
            ...
            'css_files'           => $assets['css'],
            'js_files'            => $assets['js'],
            'content_view'        => 'keuangan/proses_pembayaran_transaksi/proses_po',
            'form_data'           => object_to_array($form_data),
            'data_biaya'          => object_to_array($data_biaya),
            'data_invoice'        => object_to_array($data_invoice),
            'data_bayar'        => object_to_array($data_bayar),
    ...
    public function proses_ttf_po($ttf_id)
    {
        $assets = array();
        $config = 'assets/keuangan/proses_pembayaran_transaksi/proses_po';
        $this->config->load($config, true);
        ...
        $form_ttf = $this->tanda_terima_faktur_m->get_by(array('id' => $ttf_id), true);
        $id = $form_ttf->pembelian_id;
        $form_data = $this->pembelian_m->get_data($id)->result_array();
        $supplier_tipe_bayar = $this->supplier_tipe_pembayaran_m->get_pembayaran($form_data[0]['id'])->result_array();
        $tipe_bayar = $this->supplier_tipe_pembayaran_m->get_tipe_pembayaran($form_data[0]['tipe_pembayaran'])->result_array();
        $form_data_detail = $this->pembelian_detail_m->get_data_detail($id);
        // $data_invoice = $this->pembelian_invoice_m->get_by(array('pembelian_id' =>$id));
        $data_invoice = $this->pembelian_invoice_m->get_invoice_faktur($id)->result_array();
        // die_dump($this->db->last_query());
        $data_biaya = $this->pembelian_biaya_m->get_data($id)->result_array();
        $data_bayar = $this->pembayaran_status_m->get_by(array('transaksi_id' => $ttf_id, 'tipe_transaksi' => 4), true);

        // die_dump($data_invoice);
        ...
        $data = array(
            ...
            'css_files'           => $assets['css'],
            'js_files'            => $assets['js'],
            'content_view'        => 'keuangan/proses_pembayaran_transaksi/proses_ttf_po',
            ...
            'form_data_detail'    => (count($form_data_detail) != 0)?object_to_array($form_data_detail):'',
            'pk_value'            => $id,                      //table primary key value
            'ttf_id'            => $ttf_id,                      //table primary key value
        );
    ...
    public function save()
    {
        ...
        if($command === 'proses_po'){
            ...
            $data_bayar = array(
                ...
                'status'          => 11,
                'user_level_id'   => 9,
                'nominal'         => $array_input['total_bayar'],
            ...
            $supplier = $this->supplier_m->get($array_input['supplier_id']);

            $last_id_bayar = $this->pembelian_pembayaran_m->get_max_id_bayar_pembelian()->result_array();
            $last_id_bayar = intval($last_id_bayar[0]['max_id'])+1;

            $format_id_bayar   = 'PAY-'.date('m').'-'.date('Y').'-%04d';
            $id_po_bayar       = sprintf($format_id_bayar, $last_id_bayar, 4);


            $last_nomor_bayar = $this->pembelian_pembayaran_m->get_no_bayar_pembelian()->result_array();
            $last_nomor_bayar = intval($last_nomor_bayar[0]['max_no_pembayaran'])+1;
            
            $format           = '#PAY#%03d/RI-'.strtoupper($supplier->kode).'/'.romanic_number(date('m'), true).'/'.date('Y');
            $no_po_bayar      = sprintf($format, $last_nomor_bayar, 3);

            $total = 0;
            if($array_input['invoice']){
                foreach ($array_input['invoice'] as $key => $bon) {
                    if($bon['bank_id'] != ''){
                        $total = $total + $bon['total_invoice'];
                    }
                }
            }
            $data_pembelian_bayar = array(
                'id'                   => $id_po_bayar,
                'pembayaran_status_id' => $pembayaran_status_id->id,
                'pembelian_id'         => $pembelian_id,
                'no_pembayaran'        => $no_po_bayar,
                'supplier_id'          => $array_input['supplier_id'],
                'tipe_supplier'        => $supplier->tipe,
                'keuangan_id'          => $user_id,
            ...
            $pembayaran_po = $this->pembelian_pembayaran_m->add_data($data_pembelian_bayar);

            if($array_input['invoice']){
                foreach ($array_input['invoice'] as $key => $bon) {
                        $last_id_bayar_detail = $this->pembelian_pembayaran_detail_m->get_max_id_bayar_detail_pembelian()->result_array();
                        $last_id_bayar_detail = intval($last_id_bayar_detail[0]['max_id'])+1;

                        $format_id_bayar_detail   = 'PAYD-'.date('m').'-'.date('Y').'-%04d';
                        $id_po_bayar_detail      = sprintf($format_id_bayar_detail, $last_id_bayar_detail, 4);

                        $no_tipe = '';
                        if($bon['tipe'] == 1){
                            $no_tipe = $bon['bank_no_cek'];
                        }if($bon['tipe'] == 2){
                            $no_tipe = $bon['bank_no_giro'];
                        }if($bon['tipe'] == 3){
                            $no_tipe = $bon['bank_supp_nomor'];
                        }
                        $data_pembelian_bayar_detail = array(
                            'id'                      => $id_po_bayar_detail,
                            'pembelian_pembayaran_id' => $id_po_bayar,
                            'pembelian_invoice_id'    => $bon['id'],
                            'pembayaran_tipe'         => $array_input['payment_type'],
                            'nomor_tipe'              => $no_tipe,
                            'penerima'              => $array_input['bank_penerima_cek'],
                            'bank_id'                 => $array_input['bank_id'],
                            'supplier_bank_id'        => $array_input['bank_supp_id'],
                            'biaya_id'                => $array_input['biaya_id'],
                            'jumlah_biaya'            => $bon['nominal_biaya'],
                            'jumlah'                  => $bon['total_invoice'],
                            'total'                   => $bon['nominal'] + $bon['total_invoice'],
                            'tanggal'                 => date('Y-m-d', strtotime($array_input['tanggal'])),
                            'jatuh_tempo'             => date('Y-m-d H:i:s', strtotime($array_input['jatuh_tempo'])),
                            'created_by'              => $this->session->userdata('user_id'),
                            'created_date'            => date('Y-m-d H:i:s')
                        );

                        $pembayaran_po = $this->pembelian_pembayaran_detail_m->add_data($data_pembelian_bayar_detail);
                }
            }

            $date = date('Y-m-d', strtotime($array_input['jatuh_tempo']));
            ...
            $data_arus_kas_bank = array(
                'tanggal'      => $date,
                'tipe'         => 5,
                'keterangan'   => 'Pembayaran '.$array_input['no_pembelian'],
                'bank_id'      => $array_input['bank_id'],
                'user_id'      => $this->session->userdata('user_id'),
                'debit_credit' => 'C',
                'rupiah'       => $array_input['nominal'],
            ...
            $data = array(
                ...
                'tanggal'              => date('Y-m-d H:i:s', strtotime($array_input['jatuh_tempo'])),
                'bank_id'              => $array_input['bank_id'],
                'jumlah'               => $array_input['nominal'],
                'keterangan'           => 'Pembayaran '.$array_input['no_pembelian'],
            ...
            $format_no_voucher = '%03d/'.date('m/y').'/B-'.$bank->nob.'/OUT/RC-OFC';
            ...
            if($array_input['pph_23_nominal_hidden'] != '' && $array_input['pph_23_nominal_hidden'] != 0){

                $last_id_pph       = $this->pembelian_pph_23_m->get_max_id_pembelian_pph_23()->result_array();
                $last_id_pph       = intval($last_id_pph[0]['max_id'])+1;
                
                $format_id_po_pph  = 'PPH-'.date('m').'-'.date('Y').'-%04d';
                $id_po_pph         = sprintf($format_id_po_pph, $last_id_pph, 4);

                $format_no_pph  = 'PPH#'.date('mY').'#%04d';
                $nomor_pph         = sprintf($format_no_pph, $last_id_pph, 4);

                $data_po_pph = array(
                    'id'        => $id_po_pph,
                    'nomor_pph'        => $nomor_pph,
                    'pembelian_id'        => $pembelian_id,
                    'nomor_pembelian'        => $array_input['no_pembelian'],
                    'pph_23'        => $array_input['pph_23_persen_hidden'],
                    'pph_23_nominal'        => $array_input['pph_23_nominal_hidden'],
                    'pph_23_nominal'        => $array_input['pph_23_nominal_hidden'],
                    'status'        => 1,
                    'created_by'          => $this->session->userdata('user_id'),
                    'created_date'        => date('Y-m-d H:i:s')
                ...
                $save_po_pph = $this->pembelian_pph_23_m->add_data($data_po_pph);
                ...
                $data_status = array(
                    'id'              => $id_status,
                    'transaksi_id'    => $id_po_pph,
                    'transaksi_nomor' => $nomor_pph,
                    'tipe_transaksi'  => 7,
                    'nominal'         => $array_input['pph_23_nominal_hidden'],
                    'status'          => 1,
                    'user_level_id'   => 14,
                    'divisi'          => 15,
                    'is_active'       => 1,
        ...
        if($command === 'proses_ttf_po'){
            ...
            $user_kasir = $this->user_level_m->get(5);
            $data_bayar = array(
                'status_keuangan' => 4,
                'status'          => 3,
                'user_level_id'   => 5,
            ...
            if($array_input['payment_type'] == 1){
                ...
                $no_tipe = $array_input['bank_supp_nomor'];
            }

            $last_id_cek = $this->rangkuman_cek_pembayaran_m->get_max_id_cek()->result_array();
            $last_id_cek = intval($last_id_cek[0]['max_id'])+1;

            $format_id_bayar   = 'CK-'.date('m').'-'.date('Y').'-%04d';
            $id_cek_bayar       = sprintf($format_id_bayar, $last_id_cek, 4);
            ...
            $data_cek = array(
                ...
                'transaksi_id'         => $pembelian_id,
                'pembayaran_status_id' => $pembayaran_status_id->id,
                'transaksi_nomor'      => $pembayaran_status_id->transaksi_nomor,
                'pembayaran_tipe'      => $array_input['payment_type'],
                'nomor_tipe'           => $no_tipe,
                'penerima'             => $array_input['bank_penerima_cek'],
                'nama_bank'            => $array_input['nama_bank'],
                'bank_id'              => $array_input['bank_id'],
                'biaya_tambahan_id'    => $array_input['biaya_id'],
                'jumlah_biaya'         => $array_input['nominal_biaya'],
                'jumlah'               => $array_input['total_bayar'],
            ...
            $rangkuman = $this->rangkuman_cek_pembayaran_m->add_data($data_cek);
        ...
        if($command === 'persetujuan_biaya'){
            ...
            $user_level_data = $this->user_level_m->get(9);
            $data_status = array(
                'status'        => $status,
                'user_level_id' => 9,
        ...
        if($command === 'persetujuan_biaya_besar'){
            ...
            foreach ($biaya_bon as $rows) {
                ...
                $date = date('Y-m-d', strtotime($rows['jatuh_tempo']));
                $last_saldo_bank = $this->keuangan_arus_kas_m->get_saldo_before($date, $rows['bank_id'])->result_array();
                $after_saldo_bank = $this->keuangan_arus_kas_m->get_after_after($date, $rows['bank_id'])->result_array();
                ...
                $data_arus_kas_bank = array(
                    ...
                    'tipe'         => 5,
                    'transaksi_id' => $id,
                    'keterangan'   => 'Pembayaran Kasbon No.'.$array_input['nomor_permintaan']."\n".$array_input['keperluan'],
                    'user_id'      => $this->session->userdata('user_id'),
                    'bank_id'      => $rows['bank_id'],
                    'debit_credit' => 'C',
                    'rupiah'       =>  $rows['nominal'] + $rows['total_biaya'],
                    'saldo'        => ($saldo_before_bank +  $rows['nominal'] + $rows['total_biaya']),
                    'status'       => 1
                );
                ...
                if(count($after_saldo_bank) != 0){
                    foreach ($after_saldo_bank as $after) {
                        $data_arus_kas_after_bank = array(
                            'saldo'        => ($after['saldo'] - ( $rows['nominal'] + $rows['total_biaya'])),
                        );
                ...
                $data = array(
                    'id'                   => $id_rek_koran,
                    'nomor'            => $array_input['nomor_rk'],
                    'tanggal'              => date('Y-m-d H:i:s', strtotime($rows['jatuh_tempo'])),
                    'bank_id'              => $rows['bank_id'],
                    'jumlah'               => $rows['nominal'] + $rows['total_biaya'],
                    'keterangan'           => 'Pembayaran Kasbon No.'.$array_input['nomor_permintaan']."\n".$array_input['keperluan'],
                    'keuangan_arus_kas_id' => $keuangan_arus_kas_id,
                    'is_active'            => 1,
                    'created_by'           => $this->session->userdata('user_id'),
        ...
        redirect('keuangan/proses_pembayaran_transaksi');
    }
    ...
    public function proses_persetujuan($id)
    {
        ...
        }if($form_data->status == 16 || $form_data->status == 20){
            $data_minta = array(
                'status' => 17
            );
        ...
        $data = array(
            ...
            'css_files'        => $assets['css'],
            'js_files'         => $assets['js'],
            'content_view'     => 'keuangan/proses_pembayaran_transaksi/proses_persetujuan',
            'flag'             => 'proses',
            'pk_value'         => $id,
    ...
    public function reject_proses($permintaan_biaya_id,$tipe)
    {
        ...
        $this->load->view('keuangan/proses_pembayaran_transaksi/modal_reject', $data);
    }

    public function tolak_dokumen($pembelian_id)
    {
        ...
        $this->load->view('keuangan/proses_pembayaran_transaksi/modal_reject_dokumen', $data);
    }

    public function tolak_dokumen_asli($pembelian_id)
    {
        $data_po = $this->pembelian_m->get_by(array('id' => $pembelian_id), true);
        ...
        $this->load->view('keuangan/proses_pembayaran_transaksi/modal_reject_dokumen_asli', $data);
    }
    ...
    public function tolak_proforma_invoice()
    {
        $array_input = $this->input->post();
        $user        = $this->session->userdata('user_id');
        $date        = date('Y-m-d H:i:s');
        ...
        if($array_input['jenis_pembelian'] == 1){
            $user_level_back = 32;
        }if($array_input['jenis_pembelian'] == 3){
            $user_level_back = 9;
        }
        $user_level_data = $this->user_level_m->get($user_level_back);
        $data_status = array(
            'status'        => $status,
            'user_level_id' => $user_level_back,
            'divisi'        => $user_level_data->divisi_id,
            'keterangan_tolak'     => $array_input['keterangan_tolak'],
            'modified_by'   => $user,
            'modified_date' => $date
        ...
        $wheres_status = array(
            'transaksi_id'   => $array_input['pembelian_id'],
            'tipe_transaksi' => $array_input['tipe_transaksi'],
        );    
        ...
        if ($update || $history_persetujuan_permintaan_biaya_id) 
        {
            $flashdata = array(
                "type"     => "error",
                "msg"      => translate("Proforma Invoice Ditolak.", $this->session->userdata("language")),
                "msgTitle" => translate("Sukses", $this->session->userdata("language"))    
                );
            $this->session->set_flashdata($flashdata);
        }
    ...
    public function tolak_invoice()
    {
        ...
        if ($update || $history_persetujuan_permintaan_biaya_id) 
        {
            $flashdata = array(
                "type"     => "error",
                "msg"      => translate("Invoice Ditolak.", $this->session->userdata("language")),
                "msgTitle" => translate("Sukses", $this->session->userdata("language"))    
                );
            $this->session->set_flashdata($flashdata);
        }
    ...
    public function view_proses_bayar_po_ttf($id,$id_ttf)
    {
        $assets = array();
        $config = 'assets/keuangan/pembayaran_transaksi/proses_bayar_po';
        $this->config->load($config, true);
        ...
        $data = array(
            ...
            'css_files'                 => $assets['css'],
            'js_files'                  => $assets['js'],
            'content_view'              => 'keuangan/proses_pembayaran_transaksi/proses_bayar_po_ttf',
            'form_ttf'                 => object_to_array($form_ttf),
            'form_data'                 => object_to_array($form_data),
            'data_biaya'                => object_to_array($data_biaya),
            'data_invoice'              => object_to_array($data_invoice),
...
/* Location: ./application/controllers/keuangan/proses_pembayaran_transaksi.php */