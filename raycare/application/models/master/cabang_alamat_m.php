...
class Cabang_alamat_m extends MY_Model {
	...
	public function get_alamat_lengkap($cabang_id)
	{
		$this->db->select('cabang_alamat.*, inf_lokasi.nama_propinsi as propinsi, inf_lokasi.nama_kabupatenkota as kabkot, inf_lokasi.nama_kecamatan kecamatan, inf_lokasi.nama_kelurahan');
		$this->db->join('inf_lokasi',$this->_table.'.kode_lokasi = inf_lokasi.lokasi_kode','left');
		$this->db->where($this->_table.'.cabang_id',$cabang_id);
		$this->db->where($this->_table.'.is_active',1);
		$this->db->where($this->_table.'.is_primary',1);
