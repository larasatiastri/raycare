...
(function(o){
    ...
    var handlePilihPasienSelect = function($btn){
        $btn.on('click', function(e){
            ...
            $('#label_umur_pasien').text(parseFloat($(this).data('item').umur)+' Tahun');
        ...
        $.ajax({
            ...
            success : function(result){
                if(result.success === true)
                {
                    ...
                    $('#label_umur_pasien').text(parseFloat(data.umur)+' Tahun');
        ...
        $btn.on('click', function(e){
            ...
            var 
                ...
                if($(this).data('harga')['harga'] == undefined){
                    ...
                    harga_item = parseFloat($(this).data('harga')['harga']);
                }
                ...
                $itemSatuanEl.on('change', function(){
                    ...
                    $.ajax({
                        ...
                        success  : function( results ) {
                            ...
                            if(results.success == true){
                                harga = parseFloat(results.harga);
                ...
                $('input[name$="[qty]"]', $row).on('change keyup', function() {
                    var jumlah = $(this).val(),
                        harga =  $('input[name$="[harga]"]', $row).val(),
                        diskon = parseFloat($('input[name$="[diskon]"]', $row).val());

                    sub_total = parseFloat(jumlah * harga);

                    if(diskon != 0){
                        sub_total = sub_total - diskon;
                    }

                     $('input[name$="[sub_total]"]', $row).val(sub_total);
                     $('div#item_sub_total', $row).text(mb.formatRp(sub_total));
                     handleCountSubTotalItem();

                });

                $('input[name$="[diskon_persen]"]', $row).on('change keyup', function() {
                    var sub_total = $('input[name$="[sub_total]"]', $row).val(),
                        disc = parseFloat($(this).val()),
                        jumlah = $('input[name$="[qty]"]', $row).val(),
                        harga =  $('input[name$="[harga]"]', $row).val(),
                        diskon = $('input[name$="[diskon]"]', $row).val();

                    sub_total = parseFloat(jumlah * harga);
                    diskon = parseFloat(sub_total * (disc/100));

                    $('input[name$="[diskon]"]', $row).val(diskon);
                ...
                $('input[name$="[diskon]"]', $row).on('change keyup', function() {
                    var sub_total = $('input[name$="[sub_total]"]', $row).val(),
                        disc = parseFloat($(this).val()),
                        jumlah = $('input[name$="[qty]"]', $row).val(),
                        harga =  $('input[name$="[harga]"]', $row).val(),
                        diskon = $('input[name$="[diskon_persen]"]', $row).val();

                    sub_total = parseFloat(jumlah * harga);
                    diskon = parseFloat((disc*100)/sub_total);

                    $('input[name$="[diskon_persen]"]', $row).val(diskon);


                    if(disc != 0){
                        sub_total = sub_total - disc;
                    }
    ...
    var handleCountSubTotalItem = function() {
        ...
        if($diskon.val() == ''){
            total_all = grand_total - parseFloat(0);
        }else{
            total_all = grand_total - parseFloat($diskon.val());
        }
    ...
    var handleDiskon = function(){
        var $diskon = $('input#diskon', $tableTambahItem);
         ...
         total_all = 0;
         $('input#diskon_persen', $tableTambahItem).on('change keyup', function() {

            grand_total = $('input#total_hidden', $tableTambahItem).val(); 
            //alert(grand_total);

            diskon_rupiah = parseFloat(grand_total) * parseFloat($(this).val()/100);

            $('input#diskon', $tableTambahItem).val(diskon_rupiah);
            total_all = grand_total - parseFloat(diskon_rupiah);
        ...
        total_all = 0;
        $('input#diskon', $tableTambahItem).on('change keyup', function() {

            grand_total = $('input#total_hidden', $tableTambahItem).val(); 
            total_all = grand_total - parseFloat($(this).val());

            diskon_persen = parseFloat($(this).val() * 100)/grand_total;
            diskon_persen = diskon_persen;

             $('input#diskon_persen', $tableTambahItem).val(diskon_persen);
        ...
        total_all = total_all;
        $('input#biaya_tambahan', $tableTambahItem).on('change', function() {



            grand_total = $('input#grand_total_hidden', $tableTambahItem).val(); 
            total_all = parseFloat(grand_total) + parseFloat($(this).val());
            $('th#grand_total', $tableTambahItem).text(mb.formatRp(total_all));
            $('input#grand_total_hidden', $tableTambahItem).val(total_all);
            $('input[name$="[nominal]"]', $form).val(total_all);  
