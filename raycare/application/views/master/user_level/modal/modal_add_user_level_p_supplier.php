...
function handleDataTableUserLevel() {
    ...
    $tableUserLevel.on('draw.dt', function (){
		...
		var row = $('input#numRowSupplier').val();
		// alert(row);
		$('a#select-p-supplier', this).click(function() 
		{
			$('input#user_level_persetujuan_supplier_name'+ row).val($(this).data('item').nama);
			$('input#user_level_persetujuan_supplier_id'+ row).val($(this).data('item').id);
			$('label#user_level_supplier_lblname'+ row).text($(this).data('item').nama);
			$('a#addRowSupplier').click();
