...
class Kelompok_penjamin extends MY_Controller {
    ...
    {       
        ...
        $this->load->model('master/kelompok_penjamin_m');
    ...
    {
        $assets = array();
        $config = 'assets/master/Kelompok_penjamin/index';
        $this->config->load($config, true);
        $assets = $this->config->item('assets', $config);
        
        // die(dump( $assets['css'] ));
        $data = array(
            'title'          => config_item('site_name').' &gt;'.translate('Master Kelompok Penjamin', $this->session->userdata('language')), 
            'header'         => translate('Master kelompok Penjamin', $this->session->userdata('language')), 
            ...
            'js_files'       => $assets['js'],
            'content_view'   => 'master/Kelompok_penjamin/index',
            'flag'           => 2,
        );
    ...
    {
        $assets = array();
        $assets_config = 'assets/master/kelompok_penjamin/add';
        $this->config->load($assets_config, true);

        $assets = $this->config->item('assets', $assets_config);

        $data = array(
            'title'          => config_item('site_name').' &gt;'. translate("Tambah Penjamin", $this->session->userdata("language")), 
            'header'         => translate("Tambah Penjamin", $this->session->userdata("language")), 
            'header_info'    => config_item('site_name'), 
            'breadcrumb'     => TRUE,
            'menus'          => $this->menus,
            'menu_tree'      => $this->menu_tree,
            ...
            'content_view'   => 'master/kelompok_penjamin/add',
        );
    ...
    public function edit($id)
    {
        ...
        $assets = array();
        $config = 'assets/master/kelompok_penjamin/edit';
        $this->config->load($config, true);
        ...
        $form_data=$this->kelompok_penjamin_m->get($id);
      //  $form_data2=$this->poliklinik_tindakan_m->getdata($id)->result_array();

        $data = array(
            'title'          => config_item('site_name').' &gt;'. translate("Edit Kelompok Penjamin", $this->session->userdata("language")), 
            'header'         => translate("Edit Kelompok Penjamin", $this->session->userdata("language")), 
            'header_info'    => config_item('site_name'), 
            'breadcrumb'     => TRUE,
            'menus'          => $this->menus,
            'menu_tree'      => $this->menu_tree,
            'css_files'      => $assets['css'],
            'js_files'       => $assets['js'],
            'content_view'   => 'master/kelompok_penjamin/edit',
            'form_data'      => $form_data,
    ...
    {        
        $result = $this->kelompok_penjamin_m->get_datatable();
        ...
        foreach($records->result_array() as $row)
        {
            ...
            $action = ' 
                        <a title="'.translate('Edit', $this->session->userdata('language')).'" href="'.base_url().'master/kelompok_penjamin/edit/'.$row['id'].'" class="btn btn-xs blue-chambray"><i class="fa fa-edit"></i></a>
                        
                        <a title="'.translate('Hapus', $this->session->userdata('language')).'" data-confirm="'.translate('Anda yakin akan menghapus data kelompok penjamin ini?', $this->session->userdata('language')).'" name="delete[]" data-action="delete" data-id="'.$row['id'].'" class="btn btn-xs red"><i class="fa fa-times"></i> </a>';
    ...
    public function save()
    {
        ...
        {  
           ...
           // $data = $this->cabang_m->array_from_post( $this->cabang_m->fillable_add());
            ...
            $data['is_active'] = 1;
            $kelompok_id = $this->kelompok_penjamin_m->save($data);
            ...
            if ($kelompok_id) 
            {
                $flashdata = array(
                    "type"     => "success",
                    "msg"      => translate("Data kelompok penjamin berhasil ditambahkan.", $this->session->userdata("language")),
        ...
        elseif ($command === 'edit')
        {
            ...
            $kelompok_id = $this->kelompok_penjamin_m->save($data, $this->input->post('pk'));
 
            if ($kelompok_id) 
            {
                $flashdata = array(
                    "type"     => "success",
                    "msg"      => translate("Data kelompok penjamin berhasil diubah.", $this->session->userdata("language")),
                    "msgTitle" => translate("Sukses", $this->session->userdata("language"))    
                    );
                $this->session->set_flashdata($flashdata);
            }
        ...
        redirect("master/kelompok_penjamin");
    }
    ...
    public function deleteajax()
    {
        ...
        // save data
        $user_id = $this->kelompok_penjamin_m->save($data, $id);
...
/* End of file kelompok_penjamin.php */
/* Location: ./application/controllers/master/kelompok_penjamin.php */