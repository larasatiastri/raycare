...
class Inventory_m extends MY_Model {
	...
	public function get_data_inventory($item_id,$item_satuan_id)
	{
		$format = "SELECT gudang.nama as nama_gudang, inventory.inventory_id, inventory.gudang_id, inventory.bn_sn_lot, inventory.expire_date, sum(inventory.jumlah) as jumlah, inventory.harga_beli as harga_beli
					FROM 
					inventory
					LEFT JOIN item_satuan on inventory.item_satuan_id = item_satuan.id
					...
					GROUP BY inventory.item_satuan_id,inventory.bn_sn_lot, inventory.expire_date, inventory.gudang_id
					ORDER BY inventory.expire_date ASC";
		return $this->db->query($format);
	}

	public function get_data_inventory_gudang($item_id,$item_satuan_id,$gudang_id)
	{
		$format = "SELECT gudang.nama as nama_gudang, inventory.inventory_id, inventory.gudang_id, inventory.bn_sn_lot, inventory.expire_date, sum(inventory.jumlah) as jumlah, inventory.harga_beli as harga_beli
					FROM 
					inventory
	...
	public function get_data_stock($item_id, $item_satuan_id, $gudang_id)
	{
		$SQL = "SELECT
					item.kode,
					item.nama,
	...
	public function get_data_fifo($item_id, $item_satuan_id)
	{
		$SQL = "SELECT
					item.kode,
					item.nama,
					inventory.jumlah AS jumlah,
					item_satuan.nama AS nama_satuan
				FROM
					inventory
				JOIN item ON inventory.item_id = item.id
				...
				WHERE
					inventory.item_id = $item_id
				AND inventory.item_satuan_id = $item_satuan_id
				ORDER BY inventory.tanggal_datang ASC";
		return $this->db->query($SQL);
	}
