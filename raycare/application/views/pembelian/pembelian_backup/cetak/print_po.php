...
<body>
	...
	<div id="body">
		...
		<table id="body_content" class="table">
			<tr>
				<th width="3%;" class="text-center">No</th>
				<th width="35%;" class="text-center">Description</th>
				<th width="15%;" class="text-center">Code</th>
				<th width="15%;" colspan="2" class="text-center">Qty</th>
				<th width="20%;" colspan="2" class="text-center">Price</th>
				<th width="5%;" class="text-center">Disc</th>
				<th width="25%;" colspan="2" class="text-center">Total</th>
			</tr>
			<?php
				$total = 0;
				$tgl_kirim = array();
			...
			?>
				<tr>
					...
					<td style="padding-left:5px;"><?=$detail['nama']?></td>
					<td class="text-center" width="7%"><?=$detail['kode']?></td>
					<td class="text-right" width="6%;" style="border-right:1px solid #FFFFFF;"><?=$detail['jumlah_disetujui']?></td>
					<td class="text-left" width="9%;"><?=$detail['nama_satuan']?></td>
					<td style="padding-left:5px;border-right:1px solid #FFFFFF;" width="4%;">IDR</td>
					<td class="text-right" width="16%;"><?=formattanparupiahnol($detail['harga_beli'])?></td>
					<td style="padding-left:5px;"><?=$detail['diskon']?> %</td>
					<td style="padding-left:5px;border-right:1px solid #FFFFFF;" width="4%;">IDR</td>
					<td class="text-right" width="21%;"><?=formattanparupiahnol(($detail['jumlah_disetujui'] * $detail['harga_beli']) - ($detail['jumlah_disetujui'] * $detail['harga_beli'])*$detail['diskon']/100) ?></td>
				</tr>
			<?php
		...
		<div style="background:red;border-bottom:1.5px solid #000;width:116px;margin-bottom:5px;"><b>Term and Conditions</b></div>
		<table id="body_info" border="0">
			<tr>
				<td width="12%">POD</td>
				<td width="1%">:</td>
				<td colspan="3">Franco KCC - Kalideres Jakarta</td>
			</tr>
			<tr>
					...
					<td width="23%" colspan="3">
						...
						?>
							...
							?>
								<tr>
								<?php 
									if(date('Y-m-d', strtotime($kirim_detail['tanggal_kirim'])) == date('Y-m-d', strtotime($data_kirim_detail[$idy+1]['tanggal_kirim']))){
								?>
									<td rowspan="<?=count($data_kirim_detail)?>" valign="top"><?=date('d M Y', strtotime($data_kirim['tanggal_kirim']))?></td>
								<?php	
									}
									if(date('Y-m-d', strtotime($kirim_detail['tanggal_kirim'])) == date('Y-m-d', strtotime($data_kirim_detail[$idy-1]['tanggal_kirim']))){
								?>
								...
								<?php
									}if(date('Y-m-d', strtotime($kirim_detail['tanggal_kirim'])) != date('Y-m-d', strtotime($data_kirim_detail[$idy+1]['tanggal_kirim'])) && date('Y-m-d', strtotime($kirim_detail['tanggal_kirim'])) != date('Y-m-d', strtotime($data_kirim_detail[$idy-1]['tanggal_kirim']))){
								?>
									<td><?=date('d M Y', strtotime($data_kirim['tanggal_kirim']))?></td>
								<?php		
									}
								?>
								<td>&nbsp;-&nbsp;</td>
			...
			<tr>
				<td>Payment</td>
				<td>:</td>
				<td colspan="3"><?=($tempo->tipe_bayar_id != 3)?$nama_pembayaran->nama:$tempo->lama_tempo.' days after received the invoice'?></td>
			...
			<tr>
				...
				<td>Validity</td>
				<td>:</td>
				<td><?=($pembelian['tanggal_kadaluarsa'] != '1970-01-01')?date('d M Y', strtotime($pembelian['tanggal_kadaluarsa'])):'-' ?></td>
			</tr>
			<tr>
				<td></td>
				<td></td>
				<td colspan="3"><?=$customer_alamat[0]['alamat'].', '.ucwords(strtolower($customer_alamat[0]['nama_kelurahan']))?></td>
			</tr>
			<tr>
				<td></td>
				<td></td>
				<td colspan="3"><?=ucwords(strtolower($customer_alamat[0]['kabkot'])).' '.ucwords(strtolower($customer_alamat[0]['kodepos']))?> - Indonesia</td>
			</tr>
		</table>
				<?php 
		...
		<div class="signature">
			<table id="signature">
				<tr>
					<td width="30%"><b>Issued By,</b></td>
					<td width="30%"><b>Approval By,</b></td>
					<td width="30%"><b>Accepted By,</b></td>
				</tr>
				<tr >
					<td style="height: 75px;"><img style="height:75px;" src="<?=$tanda_tangan_issued?>"></td>
					<td style="height: 75px;"><img style="height:75px;" src="<?=$tanda_tangan_approval?>"></td>
			...
			<div class="stempel"><img src="<?=$image_header?>"></div>			
		</div>

		<div style="border-bottom:1.5px solid #000;width:55px;margin-bottom:5px;margin-top:200px;"><b>Catatan :</b></div>
		<br>
		- Tukar faktur hanya dapat dilakukan setiap hari SELASA pukul 08:00 s/d 17:00<br>
		- Tidak melayani proses tukar faktur diluar ketentuan di atas
