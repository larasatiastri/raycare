<div class="portlet light">
	<div class="portlet-body form">
		...
		<div class="form-body">
				...
				<div class="portlet-body form">
					<div class="form-body">
						...
						<div class="form-group hidden">
							<label class="control-label col-md-2"><?=translate("Racik Obat Id", $this->session->userdata("language"))?> :</label>
							<div class="col-md-2">
								<?php
									$racik_obat_id = array(
										"id"			=> "racik_obat_id",
										...
										"placeholder"	=> translate("Racik Obat Id", $this->session->userdata("language")), 
										"value"			=> $form_data['id'],
										"help"			=> $flash_form_data['racik_obat_id'],
									);
									echo form_input($racik_obat_id);
								?>
								<label id="racik_obat_id" class="control-label"><?=$form_data['id']?></label>
							</div>
						</div>
							...
							<div class="col-md-2">
								<?php
									$resep_obat_racikan_id = array(
										...
										"value"			=> $form_data['resep_obat_racikan_id'],
						...
						<div class="form-group">
	                        ...
	                        <div class="col-md-2">
	                            <div class="input-group date" id="tanggal">
	                                <input type="text" class="form-control" id="tanggal_kadaluarsa" name="tanggal_kadaluarsa" value="<?=date('d F Y', strtotime($form_data['tanggal_kadaluarsa']))?>">
						...
						<div class="form-group">
							...
							?>
							<label class="control-label col-md-2 <?=$check_dosis?>"><?=translate("Dosis", $this->session->userdata("language"))?> :</label>
							<div class="col-md-2">
								<?php
									...
									echo form_input($input_dosis);
								?>
								<label id="dosis" class="control-label <?=$check_dosis?>"><?=$dosis?></label>
							</div>
						</div>
			...
			<?php
				$check_komposisi_manual = '';
				if ($flag == "racik_obat") {
					$check_komposisi_manual = '';
				}else{
					$check_komposisi_manual = 'hidden';
				}
			?>
			<div class="portlet light <?=$check_komposisi_manual?>">
				<div class="portlet-title">
					<div class="caption">
			...
			<div class="portlet light">
				...
				<?php 
					...
					$item_cols = array(// style="width:156px;
						...
						'item_jumlah_farmasi' => '<div class="input-group text-center">
											...
											<a data-toggle="modal" data-target="#popup_modal" href="'.base_url().'apotik/racik_obat/modal_identitas_view/0/item_row_{0}" class="btn btn-primary identitas"><i class="fa fa-info"></i></a>
					...
					$get_komposisi_racikan = $this->racik_obat_m->get_komposisi_racikan($form_data['id']);
				    ...
				    if(empty($records)){
					    ...
					    foreach ($records as $key=>$data) 
					    {
							...
							if ($item_id == $data['item_id']) {
								...
								$jumlah_farmasi = $this->racik_obat_m->get_jumlah_farmasi($form_data['id'], $data['item_id'], $data['item_satuan_id'])->result_array();
								
								// die_dump($jumlah_farmasi[0]);
								$harga = intval($data['harga']);
								// $jumlah = intval($data['jumlah_farmasi']);
								$jumlah = intval($jumlah_farmasi[0]['jumlah_farmasi']);

								// die_dump($jumlah_farmasi[0]['jumlah_farmasi']);
								$sub_harga = $jumlah * $harga;
								$total_sub_harga = $total_sub_harga + $sub_harga;

								$total = $total_sub_harga + intval($form_data['biaya_tambahan']);
								$biaya_tambahan = intval($form_data['biaya_tambahan']);
						        ...
						        $item_cols = array(// style="width:156px;
									...
									// 				  </div>'.form_input($attrs_jumlah_dokter).form_input($attrs_jumlah_dokter_awal),
									'item_jumlah_farmasi' => '<div class="input-group text-center">
														<label class="control-label" name="items[{0}][item_jumlah]" style="display: table-cell; width: 100%; text-align: center;">'.$jumlah.'</label>
														<a data-toggle="modal" data-target="#popup_modal" href="'.base_url().'apotik/racik_obat/modal_identitas_view/'.$form_data['id'].'/'.$data['item_id'].'/'.$data['item_satuan_id'].'/item_row_{0}" class="btn btn-primary identitas"><i class="fa fa-info"></i></a>
													  </div>'.form_input($attrs_jumlah),
				...
				<div id="form_tambahan">
					...
					<table class="table table-striped table-bordered table-hover" id="table_komposisi_racikan">
						<thead>
							<tr class="heading">
								<th class="text-center" style="width : 15% !important;"><?=translate("Kode", $this->session->userdata("language"))?></th>
								...
								<!-- <th class="text-center" style="width : 5% !important;"><?=translate("Aksi", $this->session->userdata("language"))?></th> -->
							</tr>
						</thead>
							...
							<tr>
								...
								<td class="text-right">
									<div class="input-group">
									    ...
									    <label id="sub_total" class="control-label form-control"><?=number_format($total_sub_harga,0,'','.')?>,-</label>
										<input type="hidden" id="sub_total" name="sub_total" value="<?=$total_sub_harga?>">
									</div>
								</td>
							</tr>
							<tr>
								<td class="text-right" colspan="5"><b><?=translate("Biaya Tambahan", $this->session->userdata("language"))?> : </b></td>
								<td class="text-right">
									<div class="input-group">
										<span class="input-group-addon">
									    	<b>Rp.</b>
									    </span>
									    <label id="biaya_tambahan" class="control-label form-control"><?=number_format($biaya_tambahan,0,'','.')?>,-</label>

										<input type="hidden" min="0" class="form-control text-right" value="0" id="biaya_tambahan" name="biaya_tambahan">
									</div>
								</td>
							</tr>
							<tr>
								<td class="text-right" colspan="5"><b><?=translate("Harga Jual", $this->session->userdata("language"))?> : </b></td>
								<td class="text-right">
									<div class="input-group">
										<span class="input-group-addon">
									    	<b>Rp.</b>
