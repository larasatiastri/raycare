...
class Inventory_m extends MY_Model {
	...
	public function get_datatable_penjualan_obat($status_so_history, $gudang_id)
	{	
		...
		$this->datatable_prepare($join_tables, $params);
		// $this->db->where($wheres);
		if ($status_so_history == 0 && $gudang_id == 0)
		{
		...
		} elseif ($status_so_history != 0 && $gudang_id == 0) {

			$this->db->where('inventory.gudang_id', 1);
			$this->db->where('item_kategori_id', $status_so_history);
			$this->db->or_where('inventory.gudang_id', 2);
			$this->db->where('item_kategori_id', $status_so_history);
			$this->db->or_where('inventory.gudang_id', 3);
			$this->db->where('item_kategori_id', $status_so_history);
			// $this->db->where('item_kategori_id', $status_so_history);
			...
			$this->db->where('item_kategori_id', $status_so_history);
			// $this->db->or_where('inventory.gudang_id', $gudang);
			// $this->db->where('item_kategori_id', $status_so_history);
			$this->db->group_by('id', 'asc');
	...
	public function get_data_inventory($item_id,$item_satuan_id, $gudang_id)
	{
		$format = "SELECT inventory.inventory_id, inventory.bn_sn_lot, inventory.expire_date, sum(inventory.jumlah) as jumlah, inventory.harga_beli
					FROM 
					inventory
					LEFT JOIN item_satuan on inventory.item_satuan_id = item_satuan.id
					WHERE inventory.item_id =  '$item_id'
					...
					GROUP BY inventory.item_id, inventory.item_satuan_id, inventory.bn_sn_lot, inventory.expire_date
