...
(function(o){
    ...
    var 
        ...
        $tableOrderItemTransfusi         = $('#table_order_item_transfusi', $form),
        ...
        regExpTpltindakan          = new RegExp('tindakan_lain[0]', 'g'),   // 'g' perform global, case-insensitive
        $lastPopoverBed          = null,
        ...
        tplFormtindakan          = '<li class="fieldset">' + $('#tpl-form-tindakan_lain', $form).val() + '<hr></li>',
    ...
    var formsTindakan = {
        'tindakan' : {            
            section  : $('#section-tindakan', $form),
            template : $.validator.format( tplFormtindakan.replace(regExpTpltindakan, '{0}') ), //ubah ke format template jquery validator
            counter  : function(){ tindakanCounter++; return tindakanCounter-1; },
            ...
            bootbox.confirm(msg, function(result) {
                if (result==true) {
                   ...
                   // alert(clickcounter1);
                    if(clickcounter1==1)
                    {
                        $('input#counter').val(clickcounter1);
                        $('input#counter').attr('value',clickcounter1);
                        $('a#confirm_save_tindakan').attr('disabled');
                      $.ajax
                      ({        
                        type: 'POST',
                        url: baseAppUrl +  "check_bed",  
                        data:  {bed_id:bed_id},  
                        dataType : 'json',
                        success:function(data)          //on recieve of reply
                        { 
                              ...
                              ({        
                                ...
                                { 
                                  if(data.success == true){
                                    ...
                                    var url = baseAppUrl + 'modal_persetujuan/'+ data.tindakan_id + '/' + data.pasien_id;

                                    window.location = url;
                                    
                                    // var msg = data.msg;
                                    // $('div#modal-message').text(msg);
                                    // $('a#btn-message').attr('href',url);
                                    // $('a#btn-message').attr('data-href',url);
                                    // $('a#btn-message').attr('target','_blank');
                                    // $('a#btn-batal').attr('href', baseAppUrl);

                                    // $('a#print_persetujuan').click();
...
var handlebackbtn5 = function(){
 ...
 var handlebackbtn = function(){
    ...
    var initForm2 = function(){
        ...
        $.each(formsTindakan, function(idx, form){
            var $section           = form.section,
                $fieldsetContainer = $('ul', $section);

            // handle button add
            $('a.add-tindakan', form.section).on('click', function(){
                addFieldsetParentTindakan(form,{});
            });
...
var handlekategori = function(){
            ...
            { 
                if(data.success == true){
                    ...
                    $.each(detail_dokumen, function(idx, dtldok){
                        ...
                        dokhtml += '                <li>';
                        dokhtml += '                <a class="fancybox-button" target="_blank" title="'+dtldok.url+'" href="'+mb.baseDir()+'cloud/raycare/pages/tindakan/input_hasil_lab_manual/images/'+data.data_hasil_lab['id']+'/'+dtldok.url+'" data-rel="fancybox-button">';
                        dokhtml += '<img src="'+mb.baseDir()+'cloud/raycare/pages/tindakan/input_hasil_lab_manual/images/'+data.data_hasil_lab['id']+'/'+dtldok.url+'" alt="Tidak ditemukan">';
    ...
    function addFieldsetParentTindakan(form,data)
    {
        var 
            $section           = form.section,
            $fieldsetContainer = $('ul#list_tindakan_lain', $section),
            counter            = form.counter(),
            $newFieldset       = $(form.template(counter)).prependTo($fieldsetContainer),
            fields             = form.fields,
            prefix             = form.fieldPrefix
        ...
        $('a.del-this', $newFieldset).on('click', function(){
            ...
            handleDeleteFieldsetTindakan($(this).parents('.fieldset').eq(0), id);
        });

        $('select[name$="[tindakan_lain_id]"]', $newFieldset).select2();
    ...
    function handleDeleteFieldsetTindakan($fieldset, id)
    {
        var 
            $parentUl     = $fieldset.parent(),
            fieldsetCount = $('.fieldset', $parentUl).length,
