...
class Akun_m extends MY_Model
{
	...
	public function get_datatable($akun_tipe, $is_suspended)
	{
		...
		$params['sort_by'] = $this->_table.'.no_akun';
		$params['sort_dir'] = 'ASC';
...
public function get_datatable_suspended($suspended)
	...
	public function get_akun_by_tipe($akun_tipe)
	{
		$this->db->where_in('akun_tipe', $akun_tipe);
	...
	{
		$format = "SELECT MAX(RIGHT(`id`,4)) AS max_id FROM `akun` WHERE SUBSTR(`id`,5,7) = DATE_FORMAT(NOW(), '%m-%Y');";	
		return $this->db->query($format);
	}
