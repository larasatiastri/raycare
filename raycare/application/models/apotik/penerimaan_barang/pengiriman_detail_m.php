...
class Pengiriman_detail_m extends MY_Model {
	...
	public function get_data_item($id)
	{
		$this->db->select('pengiriman_detail.id AS id, pengiriman_detail.pengiriman_id AS pengiriman_id, item.id AS item_id, item.nama AS item_nama, 
							item.kode AS item_kode, satuan_a.id AS satuan_id, satuan_a.nama AS satuan, pengiriman_detail.box_paket_id AS box_paket_id, 
							item.is_identitas AS is_identitas, SUM(pengiriman_detail.jumlah) AS jumlah_kirim, SUM(pengiriman_detail.jumlah_konversi) as jumlah_konversi, pengiriman_detail.item_satuan_primary_id, satuan_b.nama AS satuan_primary, pengiriman_detail.bn_sn_lot, pengiriman_detail.expire_date');
		
		$this->db->join('item',$this->_table.'.item_id = item.id', 'left');
		$this->db->join('item_satuan satuan_a',$this->_table.'.item_satuan_id = satuan_a.id', 'left');
		$this->db->join('item_satuan satuan_b',$this->_table.'.item_satuan_primary_id = satuan_b.id', 'left');
		...
		$this->db->group_by($this->_table.'.item_satuan_primary_id');
		$this->db->group_by($this->_table.'.bn_sn_lot');
		$this->db->group_by($this->_table.'.expire_date');
