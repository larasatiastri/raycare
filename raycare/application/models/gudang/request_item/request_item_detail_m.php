...
class Request_item_detail_m extends MY_Model {

	protected $_table        = 'request_item_detail';
	...
	{
		$sql = "SELECT
				request_item_detail.request_item_id AS request_item_id,
				request_item_detail.item_id AS item_id,
				request_item_detail.item_satuan_id AS item_satuan_id,
				request_item_detail.jumlah AS jumlah,
				item_satuan.nama AS nama_satuan,
				item.kode AS item_kode,
				item.nama AS item_nama,
				request_item.dari_gudang_id AS dari_gudang_id,
				Sum(inventory.jumlah) AS stock,
				inventory.inventory_id
				FROM
				request_item_detail
				LEFT JOIN request_item ON request_item_detail.request_item_id = request_item.id
				LEFT JOIN item ON request_item_detail.item_id = item.id
				LEFT JOIN item_satuan ON request_item_detail.item_satuan_id = item_satuan.id
				LEFT JOIN inventory ON request_item.dari_gudang_id = inventory.gudang_id AND request_item_detail.item_id = inventory.item_id AND request_item_detail.item_satuan_id = inventory.item_satuan_id
				WHERE
				request_item_detail.request_item_id = $id
				";
		return $this->db->query($sql);
	}
