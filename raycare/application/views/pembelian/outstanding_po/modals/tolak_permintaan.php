<form action="#" id="form_tolak_os" class="form-horizontal">
        <div class="modal-header">
            ...
            <div class="caption">
                <span class="caption-subject font-blue-sharp bold uppercase"><?=translate("Tolak Permintaan Barang Umum ".$data_po['no_pembelian'], $this->session->userdata("language"))?></span>
            </div>
        </div>
        <div class="modal-body">

            <table class="table table-striped table-bordered table-hover" id="table_tolak_outstanding_po">
                <thead>
                    <tr>
                        ...
                        <th class="text-center" width="15%"><?=translate("Diminta Oleh", $this->session->userdata("language"))?> </th>
                        <th class="text-center" width="20%"><?=translate("Tgl", $this->session->userdata("language"))?> </th>
                        <th class="text-center" width="10%"><?=translate("Jml Minta", $this->session->userdata("language"))?> </th>
                        <th class="text-center" width="30%"><?=translate("Jml Tolak", $this->session->userdata("language"))?> </th>
                    </tr>
                </thead>
                <tbody>
...
<script>
    ...
    {
        var baseAppUrl = mb.baseUrl() + 'pembelian/outstanding_po/';
        $('#table_tolak_outstanding_po').dataTable({
            'processing'            : true,
            'serverSide'            : true,
            'info'            : true,
            'paginate'            : false,
            ...
            'ajax'                  : {
                'url' : baseAppUrl + 'listing_tolak_permintaan/1',
                'type' : 'POST',
            },          
            'pageLength'            : 10,
    ...
    function handleSaveTolak(){
        $('a#modal_ok').click(function() {
            ...
            $.ajax({
                type     : 'POST',
                url      : mb.baseUrl() + 'pembelian/outstanding_po/tolak_os',
                ...
                success  : function( data ) {
                    ...
                    if(data.success == true){
                        ...
                        $('#closeModal').click();              
                        location.href = mb.baseUrl() + 'pembelian/outstanding_po';
