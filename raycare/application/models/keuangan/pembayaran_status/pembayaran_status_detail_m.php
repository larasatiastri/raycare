...
class Pembayaran_status_detail_m extends MY_Model {
	...
	{
		$SQL = "SELECT user_level.nama as nama_level, user.inisial, DATE_FORMAT(pembayaran_status_detail.tanggal_proses,'%d %b %Y, %H:%i') as tanggal_proses, pembayaran_status_detail.waktu_tunggu,pembayaran_status_detail.status, pembayaran_status_detail.keterangan
				FROM pembayaran_status_detail LEFT JOIN user_level ON pembayaran_status_detail.user_level_id = user_level.id
				LEFT JOIN user ON pembayaran_status_detail.user_proses = user.id WHERE pembayaran_status_detail.pembayaran_status_id = '$pembayaran_id' AND tipe_pengajuan = $tipe ORDER BY `order` ASC";
