...
class Grafik_hd_bulan_manual extends MY_Controller {

    // protected $menu_parent_id = 2;           // deprecated, pake $menu_tree
    protected $menu_id = '7f955045e5efa9c94529ca96f86bc6e9';                  // untuk check bit_access
    private $menus = array();
    ...
    {       
        ...
        $this->load->model('laporan/tindakan_hd_manual_m');
    ...
    {
        $assets = array();
        $config = 'assets/laporan/grafik_hd_bulan_manual/index';
        ...
        $data = array(
            ...
            'js_files'       => $assets['js'],
            'content_view'   => 'laporan/grafik_hd_bulan_manual/index',
        );
    ...
    function get_day_trans($start, $penjamin_id)
    {
        ...
        $sql .= " AND a.status != 1 AND a.status != 4 AND a.is_manual = 1 AND a.is_active = 1 GROUP BY
                    date(`tanggal`)";
    ...
    function get_day_patient($start, $penjamin_id)
    {
        ...
        $sql .= " AND a.status != 1 AND a.status != 4 AND a.is_manual = 1 AND a.is_active = 1";
    ...
    {        
        ...
        $result = $this->tindakan_hd_manual_m->get_datatable_report($param_month,$param_year,$penjamin_id);
    ...
    //die(dump($records));
        ...
        {         
            ...
            $output['data'][] = array(
                '<div class="text-center"><b><a href="'.base_url().'laporan/grafik_hd_bulan_manual/get_tindakan/'.date('d-M-Y', strtotime($row['date'])).'/'.$penjamin_id.'" data-toggle="modal" data-target="#modal_laporan">'.date('d', strtotime($row['date'])).'</a></b></div>',
    ...
    public function cetak_pdf($tgl_awal=null, $tgl_akhir=null)
    {
        ...
        $this->tindakan_hd_manual_m->csv($tgl_awal,$tgl_akhir);
    }
    ...
    public function cetak_data_pasien($start, $penjamin_id,$tipe)
    {
        ...
        $data = array(
            ...
            'css_files'    => $assets['css'],
            'js_files'     => $assets['js'],
            'content_view' => 'laporan/grafik_hd_bulan_manual/cetak_data_pasien',
        );
    ...
    public function get_tindakan($tgl,$penjamin_id)
    {
        ...
        $data_per_tgl = $this->tindakan_hd_manual_m->get_data_pertanggal($param,$penjamin_id)->row(0);
        $data_per_bulan = $this->tindakan_hd_manual_m->get_data_perbulan($param,$param_month,$param_year,$penjamin_id)->row(0);

        $data_per_tgl_past = $this->tindakan_hd_manual_m->get_data_pertanggal($param_past,$penjamin_id)->row(0);
        $data_per_bulan_past = $this->tindakan_hd_manual_m->get_data_perbulan($param_past,$param_past_month,$param_past_year,$penjamin_id)->row(0);

        $data_per_tgl_past2 = $this->tindakan_hd_manual_m->get_data_pertanggal($param_past2,$penjamin_id)->row(0);
        $data_per_bulan_past2 = $this->tindakan_hd_manual_m->get_data_perbulan($param_past2,$param_past2_month,$param_past2_year,$penjamin_id)->row(0);

        $data_per_tgl_past3 = $this->tindakan_hd_manual_m->get_data_pertanggal($param_past3,$penjamin_id)->row(0);
        $data_per_bulan_past3 = $this->tindakan_hd_manual_m->get_data_perbulan($param_past3,$param_past3_month,$param_past3_year,$penjamin_id)->row(0);
        ...
        $grand_total_all = $this->tindakan_hd_manual_m->get_data_all(0,$penjamin_id)->result_array();
        $grand_total_year = $this->tindakan_hd_manual_m->get_data_all($param_year,$penjamin_id)->result_array();
        ...
        $this->load->view('laporan/grafik_hd_bulan_manual/data_tindakan', $data);
