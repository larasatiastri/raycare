...
class Arus_kas_eksternal extends MY_Controller {

    // protected $menu_parent_id = 2;           // deprecated, pake $menu_tree
    protected $menu_id = '2aa90d2ffa29fa30259a0e6c73f97ee2';                  // untuk check bit_access
    ...
    {
        $assets = array();
        $config = 'assets/kasir/arus_kas_eksternal/index';
        $this->config->load($config, true);
        $assets = $this->config->item('assets', $config);
        
        // die(dump( $assets['css'] ));
        $data = array(
            'title'          => config_item('site_name').' | '.translate('Arus Kas Eksternal', $this->session->userdata('language')), 
            'header'         => translate('Arus Kas Eksternal', $this->session->userdata('language')), 
            ...
            'js_files'       => $assets['js'],
            'content_view'   => 'kasir/arus_kas_eksternal/index',
            );
    ...
    public function listing($tgl_awal=null,$tgl_akhir=null, $kasir_id=null)
    {
        ...
        $result = $this->external_arus_kas_m->get_datatable($tgl_awal,$tgl_akhir,$kasir_id);
        // die_dump($result);
...
/* End of file arus_kas_eksternal.php */
/* Location: ./application/controllers/keuangan/arus_kas_eksternal.php */