...
(function(o){
    ...
    var 
        ...
        $tableHistoryTransferItem 	= $('#table_history_terima_item'),
    ...
    var handleDataTable = function() 
    {
    	oTable_history = $tableHistoryTransferItem.dataTable({
			...
			'ajax'              	: {
				'url' : baseAppUrl + 'listing_history_terima_item',
				'type' : 'POST',
			},			
			'pageLength'			: 10,
        ...
        $( '#gudang_terima').on( 'change', function () {
            // alert($(this).val());
            // var item_kategori_id = $('select#select_so_history').val();
            // alert(item_kategori_id);
            oTable_history.api().ajax.url(baseAppUrl + 'listing_history_terima_item/' + $(this).val()).load();
          // alert($(this).val());
        });
