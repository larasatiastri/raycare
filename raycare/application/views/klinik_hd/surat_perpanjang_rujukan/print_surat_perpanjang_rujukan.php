<html>
	...
	<?php
		...
		if (file_exists(FCPATH.config_item('site_logo')) && is_file(FCPATH.config_item('site_logo'))) 
        {
            $image_header = base_url().config_item('site_logo');
        }
        else 
        {
	...
	?>
		...
		<div id="body_surat">
			...
			<div id="tanggal_surat">
				Jakarta, <?=date('d M Y', strtotime($pengantar['created_date']));?>
			</div>
			<br>
			<div id="tujuan_surat">
				Yth. <?=$nama_dokter?>
			...
			<div id="isi_surat">
				...
				<table border=0 width="100%" style="border: 0px solid black; padding-left:10px;">
					...
					<tr>
						...
						<td width="65%"><?=ucfirst($form_kel_alamat.$form_kec_alamat.$form_kota_alamat)?></td>
					</tr>
				</table>
				<br>
				<span id="paragraf_surat">Nama diatas merupakan pasien Hemodialisa rutin di Klinik "Raycare" 2x seminggu.
				<br>
				Pasien dirujuk karena alasan <?=$pengantar['deskripsi']?>
				<br>
				<br>
				Atas perhatiannya kami ucapkan terimakasih.</span>
			</div>
			<br>
			<br>
