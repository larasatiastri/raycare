...
class Inventory_m extends MY_Model {
	...
	{
		$format = "SELECT MAX(id) as max_id FROM inventory";
	...
	public function get_data($item_id, $item_satuan_id, $gudang_id)
	{
		$this->db->select('item.kode, item.nama, item_satuan.nama as nama_satuan, sum(inventory.jumlah) as jumlah, inventory.harga_beli, inventory.gudang_id, inventory.pmb_id, inventory.inventory_id, inventory.item_id, inventory.item_satuan_id, inventory.bn_sn_lot, inventory.expire_date');
		$this->db->join('item','inventory.item_id = item.id','left');
		$this->db->join('item_satuan','inventory.item_satuan_id = item_satuan.id','left');
		...
		$this->db->order_by('inventory.expire_date', 'asc');
