...
class Tindakan_hd_item_m extends MY_Model {

	protected $_table        = 'tindakan_hd_item';
	...
	protected $datatable_columns = array(
		...
		'tindakan_hd_item.waktu'                  => 'waktu',
		'item.nama'                               => 'item_nama',
		'tindakan_hd_item.tipe_pemberian'         => 'tipe_pemberian',
		'tindakan_hd_item.jumlah'	              => 'jumlah',
		'tindakan_hd_item.item_satuan_id'         => 'item_satuan_id',
		'item_satuan.nama'                        => 'item_satuan_nama',
		'user.nama'                               => 'user_nama',
		'tindakan_hd_item.id'                     => 'id',
		'tindakan_hd_item.item_id'                => 'item_id',
		'tindakan_hd_item.bn_sn_lot'                => 'bn_sn_lot',
		'tindakan_hd_item.expire_date'                => 'expire_date',
		'tindakan_hd_item.transaksi_pemberian_id' => 'transaksi_pemberian_id',
		'tindakan_hd_item.created_by'             => 'created_by',
	...
	{		
		...
		$params['sort_by'] = 'tindakan_hd_item.waktu';
		$params['sort_dir'] = 'desc';
		// prepare buat total record tanpa filter dan limit
		// filter = false, limit= false 
		$this->datatable_prepare($join_tables, $params);
		$this->db->where('tindakan_hd_item.tindakan_hd_id', $tindakan_id );
		$this->db->where('tindakan_hd_item.is_active', 1 );
		$this->db->where('tindakan_hd_item.jumlah !=', 0 );
	...
	public function get_data_group($tindakan_hd_id, $item_id, $item_satuan_id, $harga_beli)
	{
		...
		$format = "SELECT * FROM
						tindakan_hd_item
					WHERE
						tindakan_hd_item.tindakan_hd_id = ".$tindakan_hd_id."
					AND tindakan_hd_item.item_id = ".$item_id."
					AND tindakan_hd_item.item_satuan_id = ".$item_satuan_id."
					AND tindakan_hd_item.harga_beli = ".$harga_beli."
				";
	...
	{
		$format="SELECT tindakan_hd_item.id, item.nama, item.is_show_assessment
					FROM tindakan_hd_item
					JOIN  item ON tindakan_hd_item.item_id = item.id
					WHERE tindakan_hd_item.tindakan_hd_id = ".$tindakan_hd_id."
					AND item.is_show_assessment = 1 AND item.is_active = 1 AND tindakan_hd_item.jumlah != 0
					GROUP BY item.id";
	...
	{
		$SQL = 'SELECT * FROM `tindakan_hd_item` where tindakan_hd_id = ? and jumlah != 0 and item_id IN '.config_item('dialyzer_id').' and is_active = 1';
	...
	public function get_item_invoice($wheres)
	{
		...
		$this->db->join('item','tindakan_hd_item.item_id = item.id');
		$this->db->where($wheres);
