...
class Pembelian_obat extends MY_Controller {

    // protected $menu_parent_id = 2;           // deprecated, pake $menu_tree
    protected $menu_id = 'a3ff7b6dfae72d897da1088d7a71be7a';                  // untuk check bit_access
    ...
    {
        $assets = array();
        $config = 'assets/pembelian/pembelian_obat/index';
        ...
        $data = array(
            ...
            'js_files'       => $assets['js'],
            'content_view'   => 'pembelian/pembelian_obat/index',
    ...
    {
        $assets = array();
        $config = 'assets/pembelian/pembelian_obat/history';
        $this->config->load($config, true);
        $assets = $this->config->item('assets', $config);
        ...
        $data = array(
            ...
            'css_files'      => $assets['css'],
            'js_files'       => $assets['js'],
            'content_view'   => 'pembelian/pembelian_obat/history',
            );
    ...
    {
        $assets = array();
        $assets_config = 'assets/pembelian/pembelian_obat/add';
        ...
        $data = array(
            ...
            'content_view'   => 'pembelian/pembelian_obat/add',
        );
    ...
    public function edit_draft_po($id, $id_sup)
    {
        ...
        $assets = array();
        $config = 'assets/pembelian/pembelian_obat/edit';
        $this->config->load($config, true);
        ...
        $data = array(
            ...
            'css_files'      => $assets['css'], 
            'js_files'       => $assets['js'],
            'content_view'   => 'pembelian/pembelian_obat/edit_draft_po',
    ...
    public function edit_pembelian($id)
    {
        ...
        $data = array(
            ...
            'css_files'           => $assets['css'], 
            'js_files'            => $assets['js'],
            'content_view'        => 'pembelian/pembelian_obat/edit_pembelian',
            'form_data'           => object_to_array($form_data),
    ...
    public function edit_pembelian_tolak($id)
    {
        ...
        $data = array(
            ...
            'css_files'           => $assets['css'], 
            'js_files'            => $assets['js'],
            'content_view'        => 'pembelian/pembelian_obat/edit_pembelian_tolak',
            'form_data'           => object_to_array($form_data),
            'supplier_tipe_bayar' => $supplier_tipe_bayar,
            'form_data_detail'    => (count($form_data_detail) != 0)?object_to_array($form_data_detail):'',
            'pk_value'            => $id                         //table primary key value
    ...
    public function view($id)
    {
        ...
        $assets = array();
        $config = 'assets/pembelian/pembelian_obat/view';
        $this->config->load($config, true);
        ...
        $data = array(
            ...
            'css_files'           => $assets['css'],
            'js_files'            => $assets['js'],
            'content_view'        => 'pembelian/pembelian_obat/view',
            'form_data'           => object_to_array($form_data),
            'supplier_tipe_bayar' => $supplier_tipe_bayar,
    ...
    public function view_proses($id)
    {
        ...
        $assets = array();
        $config = 'assets/pembelian/pembelian_obat/view_proses';
        $this->config->load($config, true);
        ...
        $data = array(
            ...
            'css_files'           => $assets['css'],
            'js_files'            => $assets['js'],
            'content_view'        => 'pembelian/pembelian_obat/view_proses',
            'form_data'           => object_to_array($form_data),
            'supplier_tipe_bayar' => $supplier_tipe_bayar,
            'tipe_bayar'          => $tipe_bayar,
            'form_data_detail'    => (count($form_data_detail) != 0)?object_to_array($form_data_detail):'',
    ...
    {        
        $result = $this->pembelian_m->get_datatable($tipe, $stat, 1);
        // die_dump($result);
        $output = array(
    ...
    //die_dump($records);
        ...
        foreach($records->result_array() as $row)
        {
            ...
            if($row['status'] == 1)
            {
                $action = '<a title="'.translate('View', $this->session->userdata('language')).'" class="btn grey-cascade" href="'.base_url().'pembelian/pembelian_obat/view/'.$row['id'].'"><i class="fa fa-search"></i></a>
                <a title="'.translate('Edit', $this->session->userdata('language')).'" href="'.base_url().'pembelian/pembelian_obat/edit_pembelian/'.$row['id'].'" class="btn blue-chambray"><i class="fa fa-edit"></i></a>
            ...
            {
                $action = '<a title="'.translate('Finish Process', $this->session->userdata('language')).'" class="btn btn-primary" href="'.base_url().'pembelian/pembelian_obat/proses_setuju/'.$row['id'].'"><i class="fa fa-check"></i></a>';
            ...
            {
                $action = '<a title="'.translate('View', $this->session->userdata('language')).'" class="btn grey-cascade" href="'.base_url().'pembelian/pembelian_obat/view/'.$row['id'].'"><i class="fa fa-search"></i></a>
                <a title="'.translate('Cancel PO', $this->session->userdata('language')).'" data-target="#modal_cancel_po" data-toggle="modal" href="'.base_url().'pembelian/pembelian_obat/batalkan_po/'.$row['id'].'" class="btn red cancel"><i class="fa fa-undo"></i></a>';
            ...
            $output['data'][] = array(
                ...
                '<div class="text-left inline-button-table">'.$row['nama_sup'].' <strong>['.$row['kode_sup'].']</strong></div>',
    ...
    {      
        ...
        foreach($records->result_array() as $row)
        {
            ...
            if($row['nama_sup'] != null && $row['kode_sup'] != null && $row['tanggal_pesan'] != null && $row['tanggal_kadaluarsa'] != null)
            {
                ...
                $action = '<a title="'.translate('Send PO', $this->session->userdata('language')).'" data-target="#modal_send_po" data-toggle="modal" href="'.base_url().'pembelian/pembelian_obat/send_po/'.$row['id'].'" class="btn btn-success"><i class="fa fa-send"></i></a>
                 <a title="'.translate('View', $this->session->userdata('language')).'" href="'.base_url().'pembelian/pembelian_obat/view_proses/'.$row['id'].'" class="btn grey-cascade"><i class="fa fa-search"></i></a>';
                if($row['is_kirim'] == 1){
                     $action .= '<a title="'.translate('Finish PO', $this->session->userdata('language')).'" data-target="#modal_finish_po" data-toggle="modal" href="'.base_url().'pembelian/pembelian_obat/selesaikan_po/'.$row['id'].'" class="btn btn-primary perpanjang"><i class="fa fa-check"></i></a> ';
                }if($row['is_kirim'] == 0){
                     $action .= '<a title="'.translate('Buat PO Pengganti', $this->session->userdata('language')).'" href="'.base_url().'pembelian/pembelian_obat/add_pengganti/'.$row['id'].'" class="btn btn-primary"><i class="fa fa-file"></i></a><a title="'.translate('Cancel PO', $this->session->userdata('language')).'" data-target="#modal_cancel_po" data-toggle="modal" href="'.base_url().'pembelian/pembelian_obat/batalkan_po/'.$row['id'].'" class="btn red cancel"><i class="fa fa-undo"></i></a>';
                }

                 $action .= '<a title="'.translate('Print PO', $this->session->userdata('language')).'" href="'.base_url().'pembelian/pembelian_obat/cetak_po/'.$row['id'].'" target="_blank" class="btn default"><i class="fa fa-print"></i></a>
                <a title="'.translate('Print SP', $this->session->userdata('language')).'" href="'.base_url().'pembelian/pembelian_obat/cetak_po_prekursor/'.$row['id'].'" target="_blank" class="btn default"><i class="fa fa-print"></i></a>
                <a title="'.translate('Print SP PSI', $this->session->userdata('language')).'" href="'.base_url().'pembelian/pembelian_obat/cetak_po_psiktropika/'.$row['id'].'" target="_blank" class="btn default"><i class="fa fa-print"></i></a>';
    ...
    {      
        $result = $this->pembelian_m->get_datatable_history($tipe, 1);
        // die_dump($result);
        $output = array(
            'draw'                 => intval($this->input->post('draw', true)),
            'iTotalRecords'        => $result->total_records,
        ...
        foreach($records->result_array() as $row)
        {
            ...
            if($row['nama_sup'] != null && $row['kode_sup'] != null && $row['tanggal_pesan'] != null && $row['tanggal_kadaluarsa'] != null)
            {
                ...
                $tgl_kadaluarsa = date('d M Y', strtotime($row['tanggal_kadaluarsa']));
                 ...
                 $action = '<a title="'.translate('View', $this->session->userdata('language')).'" href="'.base_url().'pembelian/pembelian_obat/view_proses/'.$row['id'].'" class="btn grey-cascade"><i class="fa fa-search"></i></a>
                 <a title="'.translate('Send PO', $this->session->userdata('language')).'" data-target="#modal_send_po" data-toggle="modal" href="'.base_url().'pembelian/pembelian_obat/send_po/'.$row['id'].'" class="btn btn-success"><i class="fa fa-send"></i></a><a title="'.translate('Print PO', $this->session->userdata('language')).'" href="'.base_url().'pembelian/pembelian_obat/cetak_po/'.$row['id'].'" target="_blank" class="btn default"><i class="fa fa-print"></i></a>';
    ...
    public function listing_draft($tipe=null)
    {
        ...
        foreach($records->result_array() as $row)
        {
            ...
            $action = '<a title="'.translate('Edit', $this->session->userdata('language')).'" href="'.base_url().'pembelian/pembelian_obat/edit_draft_po/'.$row['id'].'/'.$row['id_sup'].'" class="btn blue-chambray"><i class="fa fa-edit"></i></a>';
    ...
    public function listing_supplier($tipe=null)
    {
        ...
        $result = $this->supplier_m->get_datatable($tipe, 1);
        // die_dump($this->db->last_query());
       ...
       // die(dump($records));
        ...
        foreach($records->result_array() as $row)
        {
            ...
            if($row['is_active']==1)
            {
                ...
                $supplier_item = $this->supplier_item_m->get_by(array('supplier_id' => $id_supplier, 'item_id' => $row['id']), true);
    ...
    public function listing_permintaan_terdaftar($item_id=null, $row_id=null, $satuan_id=null)
    {
        ...
        foreach($records->result_array() as $row)
        {
            ...
            if($row['is_active']==1)
            {
                ...
                $satuan_primary = $this->item_satuan_m->get_by(array('item_id' => $row['id'], 'is_primary' => 1),true);
                $action = '<a href="'.base_url().'pembelian/pembelian_obat/data_daftar_link/'.$item_id.'/'.$row['id'].'/'.$row_id.'/'.$satuan_id.'/'.$row['id_detail'].'/'.$row['id'].'/'.$i.'" data-target="#ajax_notes2" data-toggle="modal" data-tipe="1" data-tipe_pembelian="2" title="'.translate('Select', $this->session->userdata('language')).'"  data-item="'.htmlentities(json_encode($row)).'" data-satuan_primary="'.htmlentities(json_encode($satuan_primary)).'" class="btn btn-primary select"><i class="fa fa-check"></i></a>';                
    ...
    public function modal_perpanjang($id)
    {
        ...
        $this->load->view('pembelian/pembelian_obat/modals/perpanjang_tanggal.php', $data);
    }
    ...
    public function daftar_link($id, $rowId, $satuan_id)
    {
        ...
        $this->load->view('pembelian/pembelian_obat/modals/daftar_link.php', $data);
    }
    ...
    public function data_daftar_link($item_id, $row, $row_id, $satuan_id, $id_detail, $id, $i)
    {
        ...
        // die_dump($data);
        $this->load->view('pembelian/pembelian_obat/modals/data_daftar_link.php', $data);
    }
    ...
    public function save()
    {
        ...
        if($command == 'add')
        {
            if($saveDraft == "")
            {
                ...
                $data_user_level_persetujuan = $this->setting_persetujuan_po_m->get_by(array('tipe_po' => 1, 'is_active' => 1));
                ...
                if($tempo->tipe_bayar_id == 1 || $tempo->tipe_bayar_id == 2){
                    ...
                    if(count($user_level_persetujuan) != 0){
                        ...
                        $posisi = 32;
                        $divisi_posisi = $this->user_level_m->get(32);
                    ...
                    for($i=0; $i<3; $i++){
                        ...
                        if($i == 0 || $i == 2){
                            $user_level = $this->user_level_m->get(32);
                ...
                foreach ($beliItem as $key => $item) 
                {
                    if($item['item_id'] != ""  && $item['item_nama'] != "" && $item['satuan_id'] != ""  )
                    {
                        ...
                        $data_item = array(
                            ...
                            'jumlah_diterima'             => 0,
                            'jumlah_belum_diterima'       => ($item['jumlah']*$nilai_konversi),
                            ...
                            'zat_aktif'                  => $item['item_zat_aktif'],
                            'bentuk_kekuatan'                  => $item['item_bentuk_kekuatan'],
                            'urutan'                      => $y,
                            'is_active'                   => 1,
                            'created_by'                  => $this->session->userdata('user_id'),
                            'created_date'                => date('Y-m-d H:i:s')
                        ...
                        if(count($user_level_persetujuan) != 0){
                            ...
                            foreach ($user_level_persetujuan as $kategori) 
                            {
                                ...
                                $data_persetujuan = array(
                                    ...
                                    '`order`'                  => $kategori['level_order'],
                                    '`status`'                 => 1,
                                    'email_notif'              => $kategori['email_notif'],
                                    'is_active'                => 1,
                                    'created_by'               => $this->session->userdata('user_id'),
                                    'created_date'             => date('Y-m-d H:i:s')
                                );
                        ...
                        foreach ($data_os_pesan as $row_os_pesan) {
                            $last_id_os_po_det = $this->o_s_pmsn_po_det_m->get_max_id_os_po()->result_array();
                            $last_id_os_po_det = intval($last_id_os_po_det[0]['max_id'])+1;

                            $format_id_detail_kirim   = 'OSPD-'.date('m').'-'.date('Y').'-%04d';
                            $id_os_po_det       = sprintf($format_id_detail_kirim, $last_id_os_po_det, 4);
                            ...
                            if($x == 1 && $item['jumlah'] >= $row_os_pesan['jumlah']){
                                ...
                                $save_os_po_det = $this->o_s_pmsn_po_det_m->add_data($data_os_po_det);
                ...
                if($penawaran)
                {
                    foreach ($penawaran as $row) 
                    {
                        if($row['nomor'] != '' || $row['url'] != '')
                        {
                            ...
                            $path_dokumen = '../cloud/'.config_item('site_dir').'pages/pembelian/pembelian_obat/doc/penawaran/'.str_replace(' ', '_', $id_po).'/'.str_replace(' ', '_', $id_po_penawaran);
               ...
               sent_notification_po(1,$nama_supplier, 1);
        ...
        else if($command == 'edit')
        {
            if($saveDraft == "")
            {
                ...
                if($tempo->tipe_bayar_id == 1 || $tempo->tipe_bayar_id == 2){
                    ...
                    $data_bayar_status = $this->pembayaran_status_m->get_by(array('transaksi_id' => $pembelian_id));

                    if(count($data_bayar_status) == 0){
                        $data_status['nominal'] = $array_input['sisa_bayar_hidden'];
                        ...
                        if(count($user_level_persetujuan) != 0){
                            ...
                            foreach ($user_level_persetujuan as $kategori) {
                                ...
                                $data_status_detail = array(
                                    ...
                                    'transaksi_nomor'   => $array_input['no_pembelian'],
                                    'tipe_transaksi' => 1,
                                    '`order`'              => $kategori['level_order'],
                                    'divisi'               => $user_level->divisi_id,
                                    'user_level_id'        => $kategori['user_level_menyetujui_id'],
                ...
                if($penawaran)
                {
                    foreach ($penawaran as $row) 
                    {
                        if($row['id'] == '' && ($row['nomor'] != '' || $row['url'] != ''))
                        {
                            ...
                            $path_dokumen = '../cloud/'.config_item('site_dir').'pages/pembelian/pembelian_obat/doc/penawaran/'.str_replace(' ', '_', $pembelian_id).'/'.str_replace(' ', '_', $id_po_penawaran);
                            if (!is_dir($path_dokumen)){mkdir($path_dokumen, 0777, TRUE);}
                        ...
                        elseif($row['id'] != '')
                        {
                            ...
                            {
                                $path_dokumen = '../cloud/'.config_item('site_dir').'pages/pembelian/pembelian_obat/doc/penawaran/'.str_replace(' ', '_', $pembelian_id).'/'.str_replace(' ', '_', $row['id']);
                                if (!is_dir($path_dokumen)){mkdir($path_dokumen, 0777, TRUE);}
        ...
        else if($command == 'proses')
        {
            ...
            foreach ($beliItem as $item) 
            {
                if($item['item_id'] != "" && $item['item_satuan'] != ""  )
                {
                    ...
                    $harga_satuan = $tax_tambahan - $pembulatan;
                    $data_item = array(
                        'jumlah_belum_diterima'       => ($item['jumlah_setujui']*$nilai_konversi),
                        'diskon'                      => $item['item_diskon'],
                        'diskon_tambahan_primary'     => $diskon_tambahan,
                        'ppn_tambahan_primary'        => $ppn_tambahan,
                        'pembulatan_tambahan_primary' => $pembulatan,
            ...
            redirect("pembelian/pembelian_obat");
        }
        if($command == 'add_ganti')
        {
            if($saveDraft == "")
            {
                $supplier      = $this->supplier_m->get($idSupplier);
                ...
                $data = array(
                    'id'                        => $id_po,
                    'po_ref_id'                        => $array_input['pembelian_id'],
                    'no_pembelian'              => $no_member,
                    'jenis_pembelian'              => 1,
                    'supplier_id'               => $idSupplier,
                    'customer_id'               => $penerima,
                    ...
                    'diskon'                    => $array_input['diskon'],
                    'pph'                       => $array_input['pph'],
                    'pph_23'                    => $array_input['pph23'],
                    'pph_23_nominal'            => $array_input['total_pph_23_hidden'],
                    'ket_tambahan'              => $array_input['ket_tambahan'],
                    'biaya_tambahan'            => $array_input['biaya_tambahan'],
                    'pembulatan'                => $array_input['pembulatan'],
                    'grand_total_po'            => $array_input['grand_total_hidden'],
                ...
                $id_po_sebelum = $array_input['pembelian_id'];
                $data_cancel_po = array(
                    'status_cancel' => 1,
                    'keterangan_batal' => 'Diganti dengan '.$no_member,
                );

                $cancel_po_sebelumnya = $this->pembelian_m->edit_data($data_cancel_po, $id_po_sebelum);
    ...
    public function proses_setuju($id)
    {
        ...
        $data = array(
            ...
            'css_files'           => $assets['css'],
            'js_files'            => $assets['js'],
            'content_view'        => 'pembelian/pembelian_obat/proses_setuju',
            'form_data'           => object_to_array($form_data),
            'supplier_tipe_bayar' => $supplier_tipe_bayar,
            'tipe_bayar'          => $tipe_bayar,
            'form_data_detail'    => (count($form_data_detail) != 0)?object_to_array($form_data_detail):'',
   ...
   public function cetak_po($id)
   {
        ...
        // $html = str_replace('{PAGECNT}', $mpdf->getPageCount(), $this->load->view('pembelian/pembelian_obat/cetak/header', $body, true));
   ...
   public function cetak_po_prekursor($id)
   {
        ...
        // $html = str_replace('{PAGECNT}', $mpdf->getPageCount(), $this->load->view('pembelian/pembelian_obat/cetak/header', $body, true));
        $mpdf->SetHTMLHeader($this->load->view('pembelian/pembelian_obat/cetak/header_prekursor', $body, true));
        $mpdf->SetHTMLFooter($this->load->view('pembelian/pembelian_obat/cetak/footer_prekursor', $body, true));
        $mpdf->writeHTML($this->load->view('pembelian/pembelian_obat/cetak/print_po_prekursor', $body, true));

        $mpdf->Output('invoice_'.date('Y-m-d H:i:s').'.pdf', 'I');
        $mpdf->Output($path_dokumen.'/'.$filename, 'F');
   }

    public function cetak_po_psiktropika($id)
   {
        ...
        // $html = str_replace('{PAGECNT}', $mpdf->getPageCount(), $this->load->view('pembelian/pembelian_obat/cetak/header', $body, true));
        $mpdf->SetHTMLHeader($this->load->view('pembelian/pembelian_obat/cetak/header_psiktropika', $body, true));
        $mpdf->SetHTMLFooter($this->load->view('pembelian/pembelian_obat/cetak/footer_psiktropika', $body, true));
        $mpdf->writeHTML($this->load->view('pembelian/pembelian_obat/cetak/print_po_psiktropika', $body, true));

        $mpdf->Output('invoice_'.date('Y-m-d H:i:s').'.pdf', 'I');
        $mpdf->Output($path_dokumen.'/'.$filename, 'F');
   } 

   public function cetak_po_prekursor_krw($id)
   {
        ...
        // $html = str_replace('{PAGECNT}', $mpdf->getPageCount(), $this->load->view('pembelian/pembelian_obat/cetak/header', $body, true));
        $mpdf->SetHTMLHeader($this->load->view('pembelian/pembelian_obat/cetak/header_prekursor_krw', $body, true));
        $mpdf->SetHTMLFooter($this->load->view('pembelian/pembelian_obat/cetak/footer_prekursor_krw', $body, true));
        $mpdf->writeHTML($this->load->view('pembelian/pembelian_obat/cetak/print_po_prekursor_krw', $body, true));

        $mpdf->Output('invoice_'.date('Y-m-d H:i:s').'.pdf', 'I');
        $mpdf->Output($path_dokumen.'/'.$filename, 'F');
   }

   public function cetak_po_psiktropika_krw($id)
   {
        ...
        // $html = str_replace('{PAGECNT}', $mpdf->getPageCount(), $this->load->view('pembelian/pembelian_obat/cetak/header', $body, true));
        $mpdf->SetHTMLHeader($this->load->view('pembelian/pembelian_obat/cetak/header_psiktropika_krw', $body, true));
        $mpdf->SetHTMLFooter($this->load->view('pembelian/pembelian_obat/cetak/footer_psiktropika_krw', $body, true));
        $mpdf->writeHTML($this->load->view('pembelian/pembelian_obat/cetak/print_po_psiktropika_krw', $body, true));
    ...
    public function finish_po()
    {
        ...
        $data_po_detail = $this->pembelian_detail_m->get_by(array('pembelian_id' => $po_id, 'status' => 2, 'is_active' => 1, 'jumlah_belum_diterima !=' => 0));
        $data_po_detail = object_to_array($data_po_detail);

        foreach ($data_po_detail as $key => $po_detail) {
            
            $item_satuan = $this->item_satuan_m->get_by(array('id' => $po_detail['item_satuan_id_primary']), true);

            $jumlah_belum_kirim = ($po_detail['jumlah_belum_diterima'] / $item_satuan->jumlah);

            $array_data_detail = array(
                'jumlah_pesan' => ($po_detail['jumlah_pesan'] - $jumlah_belum_kirim),
                'jumlah_disetujui' => ($po_detail['jumlah_disetujui'] - $jumlah_belum_kirim),
                'jumlah_belum_diterima' => 0,
            );

            $edit_po_detail = $this->pembelian_detail_m->edit_data($array_data_detail, $po_detail['id']);
        }
    ...
    {
       $this->load->view('pembelian/pembelian_obat/add_biaya');
    }
    ...
    public function edit_biaya($id_po)
    {
        ...
        $this->load->view('pembelian/pembelian_obat/edit_biaya', $data);
    }
    ...
    public function send_po($po_id)
    {
        ...
        $this->load->view('pembelian/pembelian_obat/modals/send_po', $data);
    ...
    public function selesaikan_po($po_id)
    {
        ...
        $this->load->view('pembelian/pembelian_obat/modals/finish_po', $data);
    ...
    public function batalkan_po($po_id)
    {
        ...
        $this->load->view('pembelian/pembelian_obat/modals/cancel_po', $data);
    ...
    public function add_jumlah($id_row, $item_id, $satuan_id)
    {
        ...
        $this->load->view('pembelian/pembelian_obat/modals/add_jumlah', $data);
    }
    ...
    public function edit_jumlah_edit($id_row, $pembelian_detail_id)
    {
        ...
        $this->load->view('pembelian/pembelian_obat/modals/edit_jumlah', $data);
    }

    public function send_notif()
    {
        $nama_supplier = str_replace(' ', '_', "PT. Ravena Indonesia");
        die(dump(sent_notification_po(3,$nama_supplier, 3)));
    }

    public function listing_lunas()
    {      
        $result = $this->pembelian_m->get_datatable_lunas(1);
        // die_dump($result);
        $output = array(
            'draw'                 => intval($this->input->post('draw', true)),
            'iTotalRecords'        => $result->total_records,
    ...
    public function add_pengganti($id)
    {
        $assets = array();
        $assets_config = 'assets/pembelian/pembelian_obat/add_pengganti';
        $this->config->load($assets_config, true);
        ...
        $data = array(
            'title'          => config_item('site_name').' | '. translate("Tambah Pembelian Pengganti", $this->session->userdata("language")), 
            'header'         => translate("Tambah Pembelian Pengganti", $this->session->userdata("language")), 
            'header_info'    => config_item('site_name'), 
            'breadcrumb'     => TRUE,
            'menus'          => $this->menus,
            'menu_tree'      => $this->menu_tree,
            'css_files'      => $assets['css'],
            'js_files'       => $assets['js'],
            'content_view'   => 'pembelian/pembelian_obat/add_pengganti',
            'form_data'           => object_to_array($form_data),
            'supplier_tipe_bayar' => $supplier_tipe_bayar,
            'form_data_detail'    => (count($form_data_detail) != 0)?object_to_array($form_data_detail):'',
            'data_supplier'       => $data_supplier,
