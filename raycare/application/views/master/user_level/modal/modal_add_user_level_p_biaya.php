...
function handleDataTableUserLevel() {
	oTableUserLevel = $tableUserLevel.dataTable({
		...
		'ajax'              	: {
			'url' : baseAppUrl + 'listing_p_biaya/add',
			'type' : 'POST',
		},			
		'pageLength'			: 10,
    ...
    $tableUserLevel.on('draw.dt', function (){
		...
		var row = $('input#numRowBiaya').val();
		// alert(row);
		$('a#select-p-biaya', this).click(function() 
		{
			$('input#user_level_name_biaya'+ row).val($(this).data('item').nama);
			$('input#user_level_id_biaya'+ row).val($(this).data('item').id);
			$('label#user_level_lblname_biaya'+ row).text($(this).data('item').nama);
			$('a#addRowBiaya').click();
