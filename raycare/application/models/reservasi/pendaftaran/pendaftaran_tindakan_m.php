...
class Pendaftaran_tindakan_m extends MY_Model {
	...
	protected $datatable_columns_antri = array(
		...
		'pasien.no_member'                  => 'no_member',
		'pendaftaran_tindakan.dokter_id'  => 'dokter_id',  
		'pendaftaran_tindakan.berat_badan'  => 'berat_badan',  
		'pendaftaran_tindakan.tekanan_darah'  => 'tekanan_darah',  
	...
	{	
		...
		$join7 = array('pasien_penjamin', 'pendaftaran_tindakan.pasien_id = pasien_penjamin.pasien_id AND pendaftaran_tindakan.penjamin_id = pasien_penjamin.penjamin_id', 'left');
		...
		$this->db->where('date(pendaftaran_tindakan.created_date) >= ', $date_awal); 
		$this->db->where('date(pendaftaran_tindakan.created_date) <= ', $date_akhir);
	...
	{	
		...
		$this->db->where('pendaftaran_tindakan.status',4);
 ...
 // die(dump($result->records));
	...
	{	
		...
		$this->db->or_where('pendaftaran_tindakan.cabang_id',$cabang_id);
		$this->db->where('pendaftaran_tindakan.status',2);
		$this->db->where('pendaftaran_tindakan.status_verif',2);
 ...
 // die(dump($result->records));
	...
	{
		$cabang_id = $this->session->userdata('cabang_id');
		$this->db->select('pasien.nama as nama_pasien, a.nama as nama_dokter, penjamin.nama as nama_penjamin, pendaftaran_tindakan.status, pendaftaran_tindakan.status_verif, pendaftaran_tindakan.created_date as created_date');
		...
		$this->db->where($this->_table.'.status_verif != ',3);
		$this->db->where('date(pendaftaran_tindakan.created_date)',date('Y-m-d'));
	...
	{
		$cabang_id = $this->session->userdata('cabang_id');
		$SQL = "SELECT MAX(antrian_dokter) as antrian_dokter FROM pendaftaran_tindakan WHERE shift = $shift AND dokter_id = $dokter_id AND date(created_date) = '".date('Y-m-d')."' AND cabang_id = $cabang_id;";
	...
	{
		$format = "SELECT MAX(SUBSTRING(`id`,12,4)) AS max_id FROM `pendaftaran_tindakan` WHERE SUBSTR(`id`,4,7) = DATE_FORMAT(NOW(), '%m-%Y');";	
