...
<div class="portlet light">
	<div class="portlet-title">
		...
		<div class="actions">
		<a href="<?=base_url()?>gudang/barang_datang_farmasi/modal_add" id="modal_add" class="btn btn-primary hidden" data-toggle="modal" data-target="#popup_modal_add"><?=translate("Tambah", $this->session->userdata("language"))?></a>

			<div class="btn-group">
	            <?php
	            	$user_level_id = $this->session->userdata('level_id');
	            	$data = '<a id="tambah" class="btn default green"> <i class="fa fa-plus"></i> <span class="hidden-480"> '.translate("Tambah", $this->session->userdata("language")).'</span> </a>';
            ...
            <?php 
            echo restriction_button($data,$user_level_id,'gudang_barang_datang_farmasi','add');
            ?>
        </div>
	</div>
	<div class="portlet-body">
		<table class="table table-striped table-hover" id="table_barang_datang_farmasi">
			<thead>
				<tr>
					<th class="text-center"><?=translate("ID", $this->session->userdata("language"))?> </th>
